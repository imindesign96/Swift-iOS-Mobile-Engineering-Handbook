---
title: "Pagination, prefetch và duplicate requests"
phase: "Networking"
difficulty: 4
importance: 5
...
used_later:
  - "HTTP caching, ETag và cache policy"
competencies:
  - "Networking"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "pagination-prefetch-va-duplicate-requests"
  - "global-commerce"
---

# Pagination, prefetch và duplicate requests
...
Trong Global Commerce, request Commerce thành công ở happy path nhưng production gặp mạng yếu, token hết hạn và response đến sai thứ tự. Chapter này tập trung vào **Pagination, prefetch và duplicate requests**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng pagination cần cursor/state machine và dedupe theo stable ID;
- nhận diện failure mode chính: double load, stale cursor hoặc append sau refresh;
- chọn giải pháp bằng rule: serialize transition, cancel stale generation và coalesce same page;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng request ID/cursor logs và item uniqueness;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Single-flight token refresh](10-single-flight-token-refresh.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [HTTP caching, ETag và cache policy](12-http-caching-etag-va-cache-policy.md) dùng contract của chapter này làm building block.
...
View → ViewModel → Repository → API Client → URLSession → HTTP server → mapped result
                 ↓
Focus: Pagination, prefetch và duplicate requests
                 ↓
Evidence: request ID/cursor logs và item uniqueness
```
...
Trọng tâm của **Pagination, prefetch và duplicate requests** là: pagination cần cursor/state machine và dedupe theo stable ID. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng request ID/cursor logs và item uniqueness.
...
Áp dụng khi serialize transition, cancel stale generation và coalesce same page. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là double load, stale cursor hoặc append sau refresh. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Pagination, prefetch và duplicate requests?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Double load, stale cursor hoặc append sau refresh tạo evidence gì?
...
URLSession quản lý task/connection theo configuration; HTTP response và transport completion là hai lớp evidence khác nhau. Với **Pagination, prefetch và duplicate requests**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Pagination, prefetch và duplicate requests** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy double load, stale cursor hoặc append sau refresh.
...
Dùng request ID/cursor logs và item uniqueness; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **serialize transition, cancel stale generation và coalesce same page**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **request ID/cursor logs và item uniqueness**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Pagination, prefetch và duplicate requests** là code an toàn.
>
...
- Double load, stale cursor hoặc append sau refresh → behavior chỉ sai ở scale/lifecycle edge.
...
- Serialize transition, cancel stale generation và coalesce same page.
...
**Hỏi:** Pagination, prefetch và duplicate requests giải quyết vấn đề gì?

**30-second:** Pagination cần cursor/state machine và dedupe theo stable id. Chọn nó khi serialize transition, cancel stale generation và coalesce same page; rủi ro chính là double load, stale cursor hoặc append sau refresh.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao double load, stale cursor hoặc append sau refresh phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **request ID/cursor logs và item uniqueness**.
...
Viết một ví dụ nhỏ minh họa **Pagination, prefetch và duplicate requests** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **double load, stale cursor hoặc append sau refresh** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Pagination, prefetch và duplicate requests**. Thu evidence bằng request ID/cursor logs và item uniqueness, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → pagination cần cursor/state machine và dedupe theo stable ID
Use when  → serialize transition, cancel stale generation và coalesce same page
Risk      → double load, stale cursor hoặc append sau refresh
Evidence  → request ID/cursor logs và item uniqueness
...
3. Usage rule: serialize transition, cancel stale generation và coalesce same page.
4. Mistake nguy hiểm: double load, stale cursor hoặc append sau refresh.
5. Production lesson: kết luận bằng request ID/cursor logs và item uniqueness, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Single-flight token refresh](10-single-flight-token-refresh.md)
- [HTTP caching, ETag và cache policy](12-http-caching-etag-va-cache-policy.md)
