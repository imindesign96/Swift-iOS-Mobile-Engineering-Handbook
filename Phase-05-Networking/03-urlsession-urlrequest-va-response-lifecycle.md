---
title: "URLSession, URLRequest và response lifecycle"
phase: "Networking"
difficulty: 3
importance: 5
...
levels:
  - L3
prerequisites:
  - "HTTP methods, headers, status và idempotency"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "urlsession-urlrequest-va-response-lifecycle"
  - "global-commerce"
---
...
Trong Global Commerce, request Commerce thành công ở happy path nhưng production gặp mạng yếu, token hết hạn và response đến sai thứ tự. Chapter này tập trung vào **URLSession, URLRequest và response lifecycle**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng URLSessionTask có state, cancellation và metrics tách khỏi HTTP result;
- nhận diện failure mode chính: quên validate response hoặc giữ session/task sai lifetime;
- chọn giải pháp bằng rule: inject transport/session configuration qua API client;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng URLSessionTaskMetrics và cancellation trace;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [HTTP methods, headers, status và idempotency](02-http-methods-headers-status-va-idempotency.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Codable, CodingKeys và resilient decoding](04-codable-codingkeys-va-resilient-decoding.md) dùng contract của chapter này làm building block.
...
View → ViewModel → Repository → API Client → URLSession → HTTP server → mapped result
                 ↓
Focus: URLSession, URLRequest và response lifecycle
                 ↓
Evidence: URLSessionTaskMetrics và cancellation trace
```
...
Trọng tâm của **URLSession, URLRequest và response lifecycle** là: URLSessionTask có state, cancellation và metrics tách khỏi HTTP result. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng URLSessionTaskMetrics và cancellation trace.
...
Áp dụng khi inject transport/session configuration qua API client. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là quên validate response hoặc giữ session/task sai lifetime. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa URLSession, URLRequest và response lifecycle?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Quên validate response hoặc giữ session/task sai lifetime tạo evidence gì?
...
URLSession quản lý task/connection theo configuration; HTTP response và transport completion là hai lớp evidence khác nhau. Với **URLSession, URLRequest và response lifecycle**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **URLSession, URLRequest và response lifecycle** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy quên validate response hoặc giữ session/task sai lifetime.
...
Dùng URLSessionTaskMetrics và cancellation trace; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **inject transport/session configuration qua API client**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **URLSessionTaskMetrics và cancellation trace**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **URLSession, URLRequest và response lifecycle** là code an toàn.
>
...
- Quên validate response hoặc giữ session/task sai lifetime → behavior chỉ sai ở scale/lifecycle edge.
...
- Inject transport/session configuration qua api client.
...
**Hỏi:** URLSession, URLRequest và response lifecycle giải quyết vấn đề gì?

**30-second:** Urlsessiontask có state, cancellation và metrics tách khỏi http result. Chọn nó khi inject transport/session configuration qua API client; rủi ro chính là quên validate response hoặc giữ session/task sai lifetime.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao quên validate response hoặc giữ session/task sai lifetime phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **URLSessionTaskMetrics và cancellation trace**.
...
Viết một ví dụ nhỏ minh họa **URLSession, URLRequest và response lifecycle** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **quên validate response hoặc giữ session/task sai lifetime** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **URLSession, URLRequest và response lifecycle**. Thu evidence bằng URLSessionTaskMetrics và cancellation trace, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → URLSessionTask có state, cancellation và metrics tách khỏi HTTP result
Use when  → inject transport/session configuration qua API client
Risk      → quên validate response hoặc giữ session/task sai lifetime
Evidence  → URLSessionTaskMetrics và cancellation trace
...
3. Usage rule: inject transport/session configuration qua API client.
4. Mistake nguy hiểm: quên validate response hoặc giữ session/task sai lifetime.
5. Production lesson: kết luận bằng URLSessionTaskMetrics và cancellation trace, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [HTTP methods, headers, status và idempotency](02-http-methods-headers-status-va-idempotency.md)
- [Codable, CodingKeys và resilient decoding](04-codable-codingkeys-va-resilient-decoding.md)
