---
title: "Access token, refresh token và Keychain boundary"
phase: "Networking"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "access-token-refresh-token-va-keychain-boundary"
  - "global-commerce"
---

# Access token, refresh token và Keychain boundary
...
Trong Global Commerce, request Commerce thành công ở happy path nhưng production gặp mạng yếu, token hết hạn và response đến sai thứ tự. Chapter này tập trung vào **Access token, refresh token và Keychain boundary**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng một refresh in-flight phục vụ nhiều waiter và publish token atomically;
- nhận diện failure mode chính: 20 request 401 tạo 20 refresh rồi random logout;
- chọn giải pháp bằng rule: actor/lock lưu shared refresh task; failure dẫn tới một logout transition;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng refresh count/session và ordered auth logs;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Cancellation từ screen đến URLSession](08-cancellation-tu-screen-en-urlsession.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Single-flight token refresh](10-single-flight-token-refresh.md) dùng contract của chapter này làm building block.
...
View → ViewModel → Repository → API Client → URLSession → HTTP server → mapped result
                 ↓
Focus: Access token, refresh token và Keychain boundary
                 ↓
Evidence: refresh count/session và ordered auth logs
```
...
Trọng tâm của **Access token, refresh token và Keychain boundary** là: một refresh in-flight phục vụ nhiều waiter và publish token atomically. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng refresh count/session và ordered auth logs.
...
Áp dụng khi actor/lock lưu shared refresh task; failure dẫn tới một logout transition. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là 20 request 401 tạo 20 refresh rồi random logout. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Access token, refresh token và Keychain boundary?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. 20 request 401 tạo 20 refresh rồi random logout tạo evidence gì?
...
URLSession quản lý task/connection theo configuration; HTTP response và transport completion là hai lớp evidence khác nhau. Với **Access token, refresh token và Keychain boundary**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Access token, refresh token và Keychain boundary** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy 20 request 401 tạo 20 refresh rồi random logout.
...
Dùng refresh count/session và ordered auth logs; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **actor/lock lưu shared refresh task; failure dẫn tới một logout transition**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **refresh count/session và ordered auth logs**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Access token, refresh token và Keychain boundary** là code an toàn.
>
...
- 20 request 401 tạo 20 refresh rồi random logout → behavior chỉ sai ở scale/lifecycle edge.
...
- Actor/lock lưu shared refresh task; failure dẫn tới một logout transition.
...
**Hỏi:** Access token, refresh token và Keychain boundary giải quyết vấn đề gì?

**30-second:** Một refresh in-flight phục vụ nhiều waiter và publish token atomically. Chọn nó khi actor/lock lưu shared refresh task; failure dẫn tới một logout transition; rủi ro chính là 20 request 401 tạo 20 refresh rồi random logout.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao 20 request 401 tạo 20 refresh rồi random logout phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **refresh count/session và ordered auth logs**.
...
Viết một ví dụ nhỏ minh họa **Access token, refresh token và Keychain boundary** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **20 request 401 tạo 20 refresh rồi random logout** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Access token, refresh token và Keychain boundary**. Thu evidence bằng refresh count/session và ordered auth logs, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → một refresh in-flight phục vụ nhiều waiter và publish token atomically
Use when  → actor/lock lưu shared refresh task; failure dẫn tới một logout transition
Risk      → 20 request 401 tạo 20 refresh rồi random logout
Evidence  → refresh count/session và ordered auth logs
...
3. Usage rule: actor/lock lưu shared refresh task; failure dẫn tới một logout transition.
4. Mistake nguy hiểm: 20 request 401 tạo 20 refresh rồi random logout.
5. Production lesson: kết luận bằng refresh count/session và ordered auth logs, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Cancellation từ screen đến URLSession](08-cancellation-tu-screen-en-urlsession.md)
- [Single-flight token refresh](10-single-flight-token-refresh.md)
