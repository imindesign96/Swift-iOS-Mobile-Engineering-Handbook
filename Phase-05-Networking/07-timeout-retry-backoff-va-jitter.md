---
title: "Timeout, retry, backoff và jitter"
phase: "Networking"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "timeout-retry-backoff-va-jitter"
  - "global-commerce"
---
...
Trong Global Commerce, request Commerce thành công ở happy path nhưng production gặp mạng yếu, token hết hạn và response đến sai thứ tự. Chapter này tập trung vào **Timeout, retry, backoff và jitter**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng retry là policy theo retryability, idempotency, budget và cancellation;
- nhận diện failure mode chính: infinite retry/thundering herd hoặc timeout một mức cho mọi request;
- chọn giải pháp bằng rule: exponential backoff có jitter, cap và deadline;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng attempt count, delay và end-to-end latency;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Error taxonomy: transport/HTTP/decode/business](06-error-taxonomy-transport-http-decode-business.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Cancellation từ screen đến URLSession](08-cancellation-tu-screen-en-urlsession.md) dùng contract của chapter này làm building block.
...
View → ViewModel → Repository → API Client → URLSession → HTTP server → mapped result
                 ↓
Focus: Timeout, retry, backoff và jitter
                 ↓
Evidence: attempt count, delay và end-to-end latency
```
...
Trọng tâm của **Timeout, retry, backoff và jitter** là: retry là policy theo retryability, idempotency, budget và cancellation. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng attempt count, delay và end-to-end latency.
...
Áp dụng khi exponential backoff có jitter, cap và deadline. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là infinite retry/thundering herd hoặc timeout một mức cho mọi request. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Timeout, retry, backoff và jitter?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Infinite retry/thundering herd hoặc timeout một mức cho mọi request tạo evidence gì?
...
URLSession quản lý task/connection theo configuration; HTTP response và transport completion là hai lớp evidence khác nhau. Với **Timeout, retry, backoff và jitter**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Timeout, retry, backoff và jitter** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy infinite retry/thundering herd hoặc timeout một mức cho mọi request.
...
Dùng attempt count, delay và end-to-end latency; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **exponential backoff có jitter, cap và deadline**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **attempt count, delay và end-to-end latency**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Timeout, retry, backoff và jitter** là code an toàn.
>
...
- Infinite retry/thundering herd hoặc timeout một mức cho mọi request → behavior chỉ sai ở scale/lifecycle edge.
...
- Exponential backoff có jitter, cap và deadline.
...
**Hỏi:** Timeout, retry, backoff và jitter giải quyết vấn đề gì?

**30-second:** Retry là policy theo retryability, idempotency, budget và cancellation. Chọn nó khi exponential backoff có jitter, cap và deadline; rủi ro chính là infinite retry/thundering herd hoặc timeout một mức cho mọi request.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao infinite retry/thundering herd hoặc timeout một mức cho mọi request phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **attempt count, delay và end-to-end latency**.
...
Viết một ví dụ nhỏ minh họa **Timeout, retry, backoff và jitter** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **infinite retry/thundering herd hoặc timeout một mức cho mọi request** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Timeout, retry, backoff và jitter**. Thu evidence bằng attempt count, delay và end-to-end latency, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → retry là policy theo retryability, idempotency, budget và cancellation
Use when  → exponential backoff có jitter, cap và deadline
Risk      → infinite retry/thundering herd hoặc timeout một mức cho mọi request
Evidence  → attempt count, delay và end-to-end latency
...
3. Usage rule: exponential backoff có jitter, cap và deadline.
4. Mistake nguy hiểm: infinite retry/thundering herd hoặc timeout một mức cho mọi request.
5. Production lesson: kết luận bằng attempt count, delay và end-to-end latency, rồi bảo vệ bằng test và metric.
