...
Trong Global Commerce, request Commerce thành công ở happy path nhưng production gặp mạng yếu, token hết hạn và response đến sai thứ tự. Chapter này tập trung vào **Single-flight token refresh**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Access token, refresh token và Keychain boundary](09-access-token-refresh-token-va-keychain-boundary.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Pagination, prefetch và duplicate requests](11-pagination-prefetch-va-duplicate-requests.md) dùng contract của chapter này làm building block.
...
View → ViewModel → Repository → API Client → URLSession → HTTP server → mapped result
                 ↓
Focus: Single-flight token refresh
                 ↓
...
Trọng tâm của **Single-flight token refresh** là: một refresh in-flight phục vụ nhiều waiter và publish token atomically. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Single-flight token refresh?
...
URLSession quản lý task/connection theo configuration; HTTP response và transport completion là hai lớp evidence khác nhau. Với **Single-flight token refresh**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Single-flight token refresh** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Single-flight token refresh** là code an toàn.
>
...
**Hỏi:** Single-flight token refresh giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Single-flight token refresh** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Single-flight token refresh**. Thu evidence bằng refresh count/session và ordered auth logs, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Pagination, prefetch và duplicate requests](11-pagination-prefetch-va-duplicate-requests.md)
