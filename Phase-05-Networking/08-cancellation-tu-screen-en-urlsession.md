---
title: "Cancellation từ screen đến URLSession"
phase: "Networking"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "cancellation-tu-screen-en-urlsession"
  - "global-commerce"
---
...
Trong Global Commerce, request Commerce thành công ở happy path nhưng production gặp mạng yếu, token hết hạn và response đến sai thứ tự. Chapter này tập trung vào **Cancellation từ screen đến URLSession**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng cancellation là cooperative signal, không cưỡng bức dừng mọi code;
- nhận diện failure mode chính: bỏ check trong loop hoặc nuốt CancellationError rồi publish state;
- chọn giải pháp bằng rule: check tại boundary, propagate và cleanup idempotently;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng cancel-to-stop latency và state after cancel;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Timeout, retry, backoff và jitter](07-timeout-retry-backoff-va-jitter.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Access token, refresh token và Keychain boundary](09-access-token-refresh-token-va-keychain-boundary.md) dùng contract của chapter này làm building block.
...
View → ViewModel → Repository → API Client → URLSession → HTTP server → mapped result
                 ↓
Focus: Cancellation từ screen đến URLSession
                 ↓
Evidence: cancel-to-stop latency và state after cancel
```
...
Trọng tâm của **Cancellation từ screen đến URLSession** là: cancellation là cooperative signal, không cưỡng bức dừng mọi code. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng cancel-to-stop latency và state after cancel.
...
Áp dụng khi check tại boundary, propagate và cleanup idempotently. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là bỏ check trong loop hoặc nuốt CancellationError rồi publish state. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Cancellation từ screen đến URLSession?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Bỏ check trong loop hoặc nuốt cancellationerror rồi publish state tạo evidence gì?
...
URLSession quản lý task/connection theo configuration; HTTP response và transport completion là hai lớp evidence khác nhau. Với **Cancellation từ screen đến URLSession**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Cancellation từ screen đến URLSession** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy bỏ check trong loop hoặc nuốt CancellationError rồi publish state.
...
Dùng cancel-to-stop latency và state after cancel; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **check tại boundary, propagate và cleanup idempotently**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **cancel-to-stop latency và state after cancel**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Cancellation từ screen đến URLSession** là code an toàn.
>
...
- Bỏ check trong loop hoặc nuốt cancellationerror rồi publish state → behavior chỉ sai ở scale/lifecycle edge.
...
- Check tại boundary, propagate và cleanup idempotently.
...
**Hỏi:** Cancellation từ screen đến URLSession giải quyết vấn đề gì?

**30-second:** Cancellation là cooperative signal, không cưỡng bức dừng mọi code. Chọn nó khi check tại boundary, propagate và cleanup idempotently; rủi ro chính là bỏ check trong loop hoặc nuốt CancellationError rồi publish state.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao bỏ check trong loop hoặc nuốt CancellationError rồi publish state phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **cancel-to-stop latency và state after cancel**.
...
Viết một ví dụ nhỏ minh họa **Cancellation từ screen đến URLSession** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **bỏ check trong loop hoặc nuốt CancellationError rồi publish state** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Cancellation từ screen đến URLSession**. Thu evidence bằng cancel-to-stop latency và state after cancel, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → cancellation là cooperative signal, không cưỡng bức dừng mọi code
Use when  → check tại boundary, propagate và cleanup idempotently
Risk      → bỏ check trong loop hoặc nuốt CancellationError rồi publish state
Evidence  → cancel-to-stop latency và state after cancel
...
3. Usage rule: check tại boundary, propagate và cleanup idempotently.
4. Mistake nguy hiểm: bỏ check trong loop hoặc nuốt CancellationError rồi publish state.
5. Production lesson: kết luận bằng cancel-to-stop latency và state after cancel, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Timeout, retry, backoff và jitter](07-timeout-retry-backoff-va-jitter.md)
- [Access token, refresh token và Keychain boundary](09-access-token-refresh-token-va-keychain-boundary.md)
