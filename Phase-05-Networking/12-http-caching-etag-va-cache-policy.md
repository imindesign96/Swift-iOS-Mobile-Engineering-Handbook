---
title: "HTTP caching, ETag và cache policy"
phase: "Networking"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "http-caching-etag-va-cache-policy"
  - "global-commerce"
---
...
Trong Global Commerce, request Commerce thành công ở happy path nhưng production gặp mạng yếu, token hết hạn và response đến sai thứ tự. Chapter này tập trung vào **HTTP caching, ETag và cache policy**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Pagination, prefetch và duplicate requests](11-pagination-prefetch-va-duplicate-requests.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Kết hợp remote cache và offline data](13-ket-hop-remote-cache-va-offline-data.md) dùng contract của chapter này làm building block.
...
View → ViewModel → Repository → API Client → URLSession → HTTP server → mapped result
                 ↓
Focus: HTTP caching, ETag và cache policy
                 ↓
...
Trọng tâm của **HTTP caching, ETag và cache policy** là: cache cần key, freshness, validation, invalidation và source-of-truth policy. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa HTTP caching, ETag và cache policy?
...
URLSession quản lý task/connection theo configuration; HTTP response và transport completion là hai lớp evidence khác nhau. Với **HTTP caching, ETag và cache policy**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **HTTP caching, ETag và cache policy** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **HTTP caching, ETag và cache policy** là code an toàn.
>
...
**Hỏi:** HTTP caching, ETag và cache policy giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **HTTP caching, ETag và cache policy** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **HTTP caching, ETag và cache policy**. Thu evidence bằng hit rate, age, revalidation và generation, vẽ owner/state graph, xác định root cause và thêm regression test.
