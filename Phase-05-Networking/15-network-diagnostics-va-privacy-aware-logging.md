---
title: "Network diagnostics và privacy-aware logging"
phase: "Networking"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "network-diagnostics-va-privacy-aware-logging"
  - "global-commerce"
---
...
Trong Global Commerce, request Commerce thành công ở happy path nhưng production gặp mạng yếu, token hết hạn và response đến sai thứ tự. Chapter này tập trung vào **Network diagnostics và privacy-aware logging**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng telemetry phải trả lời user impact và hỗ trợ correlation mà không lộ dữ liệu;
- nhận diện failure mode chính: log text tự do, PII hoặc metric không có action;
- chọn giải pháp bằng rule: thiết kế event schema, sampling, retention và ownership;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng dashboard theo SLO và trace/request ID;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [ATS, TLS và certificate pinning trade-offs](14-ats-tls-va-certificate-pinning-trade-offs.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Phase Review: Networking](99-phase-review.md) dùng contract của chapter này làm building block.
...
View → ViewModel → Repository → API Client → URLSession → HTTP server → mapped result
                 ↓
Focus: Network diagnostics và privacy-aware logging
                 ↓
Evidence: dashboard theo SLO và trace/request ID
```
...
Trọng tâm của **Network diagnostics và privacy-aware logging** là: telemetry phải trả lời user impact và hỗ trợ correlation mà không lộ dữ liệu. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng dashboard theo SLO và trace/request ID.
...
Áp dụng khi thiết kế event schema, sampling, retention và ownership. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là log text tự do, PII hoặc metric không có action. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Network diagnostics và privacy-aware logging?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Log text tự do, pii hoặc metric không có action tạo evidence gì?
...
URLSession quản lý task/connection theo configuration; HTTP response và transport completion là hai lớp evidence khác nhau. Với **Network diagnostics và privacy-aware logging**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Network diagnostics và privacy-aware logging** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy log text tự do, PII hoặc metric không có action.
...
Dùng dashboard theo SLO và trace/request ID; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **thiết kế event schema, sampling, retention và ownership**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **dashboard theo SLO và trace/request ID**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Network diagnostics và privacy-aware logging** là code an toàn.
>
...
- Log text tự do, pii hoặc metric không có action → behavior chỉ sai ở scale/lifecycle edge.
...
- Thiết kế event schema, sampling, retention và ownership.
...
**Hỏi:** Network diagnostics và privacy-aware logging giải quyết vấn đề gì?

**30-second:** Telemetry phải trả lời user impact và hỗ trợ correlation mà không lộ dữ liệu. Chọn nó khi thiết kế event schema, sampling, retention và ownership; rủi ro chính là log text tự do, PII hoặc metric không có action.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao log text tự do, PII hoặc metric không có action phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **dashboard theo SLO và trace/request ID**.
...
Viết một ví dụ nhỏ minh họa **Network diagnostics và privacy-aware logging** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **log text tự do, PII hoặc metric không có action** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Network diagnostics và privacy-aware logging**. Thu evidence bằng dashboard theo SLO và trace/request ID, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → telemetry phải trả lời user impact và hỗ trợ correlation mà không lộ dữ liệu
Use when  → thiết kế event schema, sampling, retention và ownership
Risk      → log text tự do, PII hoặc metric không có action
Evidence  → dashboard theo SLO và trace/request ID
...
3. Usage rule: thiết kế event schema, sampling, retention và ownership.
4. Mistake nguy hiểm: log text tự do, PII hoặc metric không có action.
5. Production lesson: kết luận bằng dashboard theo SLO và trace/request ID, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [ATS, TLS và certificate pinning trade-offs](14-ats-tls-va-certificate-pinning-trade-offs.md)
- [Phase Review: Networking](99-phase-review.md)
