---
title: "HTTP methods, headers, status và idempotency"
phase: "Networking"
difficulty: 3
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "http-methods-headers-status-va-idempotency"
  - "global-commerce"
---
...
Trong Global Commerce, request Commerce thành công ở happy path nhưng production gặp mạng yếu, token hết hạn và response đến sai thứ tự. Chapter này tập trung vào **HTTP methods, headers, status và idempotency**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng signature là API contract về intent, input, ownership và failure;
- nhận diện failure mode chính: boolean soup, label mơ hồ hoặc side effect ẩn;
- chọn giải pháp bằng rule: tách function theo một responsibility có tên domain;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng call site đọc như câu và test theo output/side effect;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Một request từ iPhone đến server đi như thế nào?](01-mot-request-tu-iphone-en-server-i-nhu-the-nao.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [URLSession, URLRequest và response lifecycle](03-urlsession-urlrequest-va-response-lifecycle.md) dùng contract của chapter này làm building block.
...
View → ViewModel → Repository → API Client → URLSession → HTTP server → mapped result
                 ↓
Focus: HTTP methods, headers, status và idempotency
                 ↓
Evidence: call site đọc như câu và test theo output/side effect
```
...
Trọng tâm của **HTTP methods, headers, status và idempotency** là: signature là API contract về intent, input, ownership và failure. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng call site đọc như câu và test theo output/side effect.
...
Áp dụng khi tách function theo một responsibility có tên domain. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là boolean soup, label mơ hồ hoặc side effect ẩn. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa HTTP methods, headers, status và idempotency?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Boolean soup, label mơ hồ hoặc side effect ẩn tạo evidence gì?
...
URLSession quản lý task/connection theo configuration; HTTP response và transport completion là hai lớp evidence khác nhau. Với **HTTP methods, headers, status và idempotency**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **HTTP methods, headers, status và idempotency** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy boolean soup, label mơ hồ hoặc side effect ẩn.
...
Dùng call site đọc như câu và test theo output/side effect; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **tách function theo một responsibility có tên domain**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **call site đọc như câu và test theo output/side effect**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **HTTP methods, headers, status và idempotency** là code an toàn.
>
...
- Boolean soup, label mơ hồ hoặc side effect ẩn → behavior chỉ sai ở scale/lifecycle edge.
...
- Tách function theo một responsibility có tên domain.
...
**Hỏi:** HTTP methods, headers, status và idempotency giải quyết vấn đề gì?

**30-second:** Signature là api contract về intent, input, ownership và failure. Chọn nó khi tách function theo một responsibility có tên domain; rủi ro chính là boolean soup, label mơ hồ hoặc side effect ẩn.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao boolean soup, label mơ hồ hoặc side effect ẩn phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **call site đọc như câu và test theo output/side effect**.
...
Viết một ví dụ nhỏ minh họa **HTTP methods, headers, status và idempotency** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **boolean soup, label mơ hồ hoặc side effect ẩn** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **HTTP methods, headers, status và idempotency**. Thu evidence bằng call site đọc như câu và test theo output/side effect, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → signature là API contract về intent, input, ownership và failure
Use when  → tách function theo một responsibility có tên domain
Risk      → boolean soup, label mơ hồ hoặc side effect ẩn
Evidence  → call site đọc như câu và test theo output/side effect
...
3. Usage rule: tách function theo một responsibility có tên domain.
4. Mistake nguy hiểm: boolean soup, label mơ hồ hoặc side effect ẩn.
5. Production lesson: kết luận bằng call site đọc như câu và test theo output/side effect, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Một request từ iPhone đến server đi như thế nào?](01-mot-request-tu-iphone-en-server-i-nhu-the-nao.md)
