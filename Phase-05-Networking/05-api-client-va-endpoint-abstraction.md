---
title: "API client và Endpoint abstraction"
phase: "Networking"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "api-client-va-endpoint-abstraction"
  - "global-commerce"
---
...
Trong Global Commerce, request Commerce thành công ở happy path nhưng production gặp mạng yếu, token hết hạn và response đến sai thứ tự. Chapter này tập trung vào **API client và Endpoint abstraction**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Codable, CodingKeys và resilient decoding](04-codable-codingkeys-va-resilient-decoding.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Error taxonomy: transport/HTTP/decode/business](06-error-taxonomy-transport-http-decode-business.md) dùng contract của chapter này làm building block.
...
View → ViewModel → Repository → API Client → URLSession → HTTP server → mapped result
                 ↓
Focus: API client và Endpoint abstraction
                 ↓
...
Trọng tâm của **API client và Endpoint abstraction** là: Endpoint làm request construction typed; API client thực thi transport và map lỗi. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa API client và Endpoint abstraction?
...
URLSession quản lý task/connection theo configuration; HTTP response và transport completion là hai lớp evidence khác nhau. Với **API client và Endpoint abstraction**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **API client và Endpoint abstraction** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **API client và Endpoint abstraction** là code an toàn.
>
...
**Hỏi:** API client và Endpoint abstraction giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **API client và Endpoint abstraction** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **API client và Endpoint abstraction**. Thu evidence bằng request snapshot test và integration fixture, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Error taxonomy: transport/HTTP/decode/business](06-error-taxonomy-transport-http-decode-business.md)
