---
title: "Kết hợp remote cache và offline data"
phase: "Networking"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "ket-hop-remote-cache-va-offline-data"
  - "global-commerce"
---
...
Trong Global Commerce, request Commerce thành công ở happy path nhưng production gặp mạng yếu, token hết hạn và response đến sai thứ tự. Chapter này tập trung vào **Kết hợp remote cache và offline data**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [HTTP caching, ETag và cache policy](12-http-caching-etag-va-cache-policy.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [ATS, TLS và certificate pinning trade-offs](14-ats-tls-va-certificate-pinning-trade-offs.md) dùng contract của chapter này làm building block.
...
View → ViewModel → Repository → API Client → URLSession → HTTP server → mapped result
                 ↓
Focus: Kết hợp remote cache và offline data
                 ↓
...
Trọng tâm của **Kết hợp remote cache và offline data** là: cache cần key, freshness, validation, invalidation và source-of-truth policy. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Kết hợp remote cache và offline data?
...
URLSession quản lý task/connection theo configuration; HTTP response và transport completion là hai lớp evidence khác nhau. Với **Kết hợp remote cache và offline data**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Kết hợp remote cache và offline data** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Kết hợp remote cache và offline data** là code an toàn.
>
...
**Hỏi:** Kết hợp remote cache và offline data giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Kết hợp remote cache và offline data** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Kết hợp remote cache và offline data**. Thu evidence bằng hit rate, age, revalidation và generation, vẽ owner/state graph, xác định root cause và thêm regression test.
