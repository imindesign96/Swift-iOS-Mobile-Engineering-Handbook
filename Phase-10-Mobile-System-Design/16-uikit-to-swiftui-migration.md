...
levels:
  - L6
prerequisites:
  - "Modular architecture cho nhiều team"
used_later:
...
Trong Global Commerce, thiết kế backend-correct nhưng mobile-failure vì bỏ qua offline, background interruption và resource budget. Chapter này tập trung vào **UIKit → SwiftUI migration**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Modular architecture cho nhiều team](15-modular-architecture-cho-nhieu-team.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Commerce Checkout state machine](17-commerce-checkout-state-machine.md) dùng contract của chapter này làm building block.
...
Trọng tâm của **UIKit → SwiftUI migration** là: SwiftUI state ownership và stable identity quyết định lifetime/update. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa UIKit → SwiftUI migration?
...
Thiết kế phải chịu foreground/background/suspend/terminate và network thay đổi giữa chừng. Với **UIKit → SwiftUI migration**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **UIKit → SwiftUI migration** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **UIKit → SwiftUI migration** là code an toàn.
>
...
**Hỏi:** UIKit → SwiftUI migration giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **UIKit → SwiftUI migration** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **UIKit → SwiftUI migration**. Thu evidence bằng SwiftUI Instruments và state-transition log, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Modular architecture cho nhiều team](15-modular-architecture-cho-nhieu-team.md)
