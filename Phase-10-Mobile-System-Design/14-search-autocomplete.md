...
Trong Global Commerce, thiết kế backend-correct nhưng mobile-failure vì bỏ qua offline, background interruption và resource budget. Chapter này tập trung vào **Search autocomplete**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Caching layer](13-caching-layer.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Modular architecture cho nhiều team](15-modular-architecture-cho-nhieu-team.md) dùng contract của chapter này làm building block.
...
Trọng tâm của **Search autocomplete** là: ARC quản lý lifetime qua strong references trong ownership graph. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Search autocomplete?
...
Thiết kế phải chịu foreground/background/suspend/terminate và network thay đổi giữa chừng. Với **Search autocomplete**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Search autocomplete** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Search autocomplete** là code an toàn.
>
...
**Hỏi:** Search autocomplete giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Search autocomplete** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Search autocomplete**. Thu evidence bằng Memory Graph + deinit expectation, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Caching layer](13-caching-layer.md)
