---
title: "Offline-first app và conflict resolution"
...
competencies:
  ...
  - "Interview"
tags:
  - "offline-first-app-va-conflict-resolution"
  - "global-commerce"
---
...
Trong Global Commerce, thiết kế backend-correct nhưng mobile-failure vì bỏ qua offline, background interruption và resource budget. Chapter này tập trung vào **Offline-first app và conflict resolution**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Download Manager hỗ trợ resume/background](05-download-manager-ho-tro-resume-background.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Video Feed và resource budgeting](07-video-feed-va-resource-budgeting.md) dùng contract của chapter này làm building block.
...
Requirements → constraints → data/state → architecture → failure/security/performance → observability/testing
                 ↓
Focus: Offline-first app và conflict resolution
                 ↓
...
Trọng tâm của **Offline-first app và conflict resolution** là: cache cần key, freshness, validation, invalidation và source-of-truth policy. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Offline-first app và conflict resolution?
...
Thiết kế phải chịu foreground/background/suspend/terminate và network thay đổi giữa chừng. Với **Offline-first app và conflict resolution**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Offline-first app và conflict resolution** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Offline-first app và conflict resolution** là code an toàn.
>
...
**Hỏi:** Offline-first app và conflict resolution giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Offline-first app và conflict resolution** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Offline-first app và conflict resolution**. Thu evidence bằng hit rate, age, revalidation và generation, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Download Manager hỗ trợ resume/background](05-download-manager-ho-tro-resume-background.md)
- [Video Feed và resource budgeting](07-video-feed-va-resource-budgeting.md)
