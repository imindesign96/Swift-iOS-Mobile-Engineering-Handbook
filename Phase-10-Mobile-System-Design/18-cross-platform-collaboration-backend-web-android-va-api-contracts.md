---
title: "Cross-platform collaboration: backend, web, Android và API contracts"
phase: "Mobile System Design"
difficulty: 5
importance: 5
interview_frequency: 4
status: complete
last_verified: 2026-08-09
swift_baseline: "Swift 6.3 toolchain; Swift 6 language mode"
levels:
  - L6
prerequisites:
  - "Commerce Checkout state machine"
used_later:
  - "Phase Review: Mobile System Design"
competencies:
  - "Mobile System Design"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "cross-platform-collaboration-backend-web-android-va-api-contracts"
  - "global-commerce"
---

# Cross-platform collaboration: backend, web, Android và API contracts

> **Version scope**
>
> Baseline Swift 6.3 toolchain, Swift 6 language mode; API iOS-specific phải kiểm tra availability tại call site. Xác minh 2026-08-09.

## Story / Problem

Trong Global Commerce, thiết kế backend-correct nhưng mobile-failure vì bỏ qua offline, background interruption và resource budget. Chapter này tập trung vào **Cross-platform collaboration: backend, web, Android và API contracts**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.

Nếu team chỉ nhớ tên concept, fix thường dịch symptom sang chỗ khác. Câu hỏi mở đầu là:

```text
Input/event → state hoặc resource nào thay đổi?
            → ai sở hữu thay đổi đó?
            → contract nào có thể bị vi phạm?
            → evidence nào chứng minh kết luận?
```

## Objectives

Sau chapter này, bạn có thể:

- giải thích chính xác rằng cross-platform delivery cần contract chung về schema, error, auth, time, money, pagination, idempotency và rollout thay vì mỗi client tự suy luận;
- nhận diện failure mode chính: backend thay enum bắt buộc, web/iOS khác cookie session, Android/iOS khác lifecycle hoặc rollout phá backward compatibility;
- chọn giải pháp bằng rule: đồng sở hữu API contract, dùng tolerant reader có chủ đích, version breaking change và ghi rõ platform-specific capability;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng OpenAPI/fixture contract tests, compatibility matrix, correlation ID và staged-rollout metrics;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Commerce Checkout state machine](17-commerce-checkout-state-machine.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Phase Review: Mobile System Design](99-phase-review.md) dùng contract của chapter này làm building block.
- [Production Playbook](../PRODUCTION_PLAYBOOK.md) dùng cùng flow evidence-first.
- [Interview Playbook](../INTERVIEW_PLAYBOOK.md) dùng mental model để xử lý follow-up.

## Mental Model

```text
Requirements → constraints → data/state → architecture → failure/security/performance → observability/testing
                 ↓
Focus: Cross-platform collaboration: backend, web, Android và API contracts
                 ↓
Evidence: OpenAPI/fixture contract tests, compatibility matrix, correlation ID và staged-rollout metrics
```

Mental model hữu ích để đặt câu hỏi đúng, nhưng không thay thế API contract hoặc measurement. Đặc biệt, không suy luận implementation private của compiler/framework thành behavior được đảm bảo.

## What?

Trọng tâm của **Cross-platform collaboration: backend, web, Android và API contracts** là: cross-platform delivery cần contract chung về schema, error, auth, time, money, pagination, idempotency và rollout thay vì mỗi client tự suy luận. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.

Một contract tốt nói rõ input hợp lệ, output/failure, side effect, owner và thời điểm kết thúc. Nếu concept chạm external data hoặc asynchronous work, contract cũng phải nói cancellation, ordering và recovery.

## Why?

Concept tồn tại để giúp thiết kế feature trong constraint memory, battery, network, storage và app lifecycle. Không có boundary này, rule domain bị nhân bản, behavior phụ thuộc call order và incident khó điều tra vì không biết layer nào chịu trách nhiệm.

Giá trị lớn nhất không phải ít dòng code hơn; đó là giảm số state hợp lệ cần giữ trong đầu và làm violation xuất hiện sớm qua compiler, test hoặc telemetry.

## How?

1. Xác định source of truth và invariant liên quan.
2. Xác định creator/owner/mutator/observer.
3. Chọn API hoặc abstraction thể hiện đúng semantics.
4. Mô hình hóa failure/cancellation thay vì che bằng fallback.
5. Đo observable behavior bằng OpenAPI/fixture contract tests, compatibility matrix, correlation ID và staged-rollout metrics.

## Technical Deep Dive

### Shared contract

Backend, web, Android và iOS thống nhất schema qua OpenAPI/JSON examples và contract fixtures. Mỗi field ghi required/nullable/default, unit và timezone; tiền dùng minor unit + ISO currency; timestamp có timezone; pagination định nghĩa cursor stability; mutation có idempotency; error có machine code, retryability và correlation ID.

Client release chậm hơn server, vì vậy server phải backward compatible qua nhiều app versions. Additive field thường an toàn khi decoder tolerant; đổi enum, meaning, nullability hoặc authentication flow có thể breaking. Deprecation cần usage telemetry, minimum supported version và staged removal. Feature capability negotiation tốt hơn đoán theo user-agent.

### Platform differences

Web có cookie/SameSite/CORS và tab lifecycle; mobile có secure storage, offline/background, app update lag và deep links. Android/iOS khác permission, process death và UI conventions. Shared business rule nên có contract; platform adapter vẫn được quyền khác implementation để tôn trọng OS.

Một design review hiệu quả có sequence diagram, owner cho từng failure, sample payload, rollout/rollback và observability. Consumer-driven contract tests chạy trong CI của producer lẫn clients. Incident log dùng correlation ID end-to-end, không buộc mobile log PII để backend tìm request.

### Documented behavior vs inference

- **Documented:** dùng contract trong Swift/Apple documentation và availability của SDK.
- **Inference:** storage placement, executor scheduling chi tiết hoặc reconciliation internals chỉ được dùng như giả thuyết đo lường, không phải guarantee.

## When?

Áp dụng khi đồng sở hữu API contract, dùng tolerant reader có chủ đích, version breaking change và ghi rõ platform-specific capability. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.

Tránh dùng concept như cargo cult. Một type, layer hay primitive không có owner/contract/test rõ chỉ chuyển complexity chứ không loại bỏ nó.

## What if?

Failure mode quan trọng là backend thay enum bắt buộc, web/iOS khác cookie session, Android/iOS khác lifecycle hoặc rollout phá backward compatibility. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.

Khi assumption có thể thay đổi qua `await`, lifecycle callback, network retry hoặc migration, hãy revalidate state trước khi commit kết quả.

### Review questions

1. Observable behavior nào định nghĩa Cross-platform collaboration: backend, web, Android và API contracts?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Backend thay enum bắt buộc, web/ios khác cookie session, android/ios khác lifecycle hoặc rollout phá backward compatibility tạo evidence gì?
4. Rule chọn giải pháp là gì, và constraint nào khiến rule đổi?

## Implementation Example

```swift
struct MoneyDTO: Codable, Sendable {
    let amountMinor: Int64
    let currency: String
}

struct APIErrorDTO: Codable, Sendable {
    let code: String          // machine-readable, documented and versioned
    let message: String       // user-safe fallback
    let retryable: Bool
    let correlationID: String
}

// Backend, web, Android và iOS dùng cùng contract fixtures/OpenAPI examples.
```

Ví dụ Swift chạy trong executable/test target hoặc iOS target tương ứng. Ví dụ Ruby mô tả `Fastfile` versioned. Mục tiêu là minh họa contract thực tế, không giả lập framework bằng toy code.

## iOS Runtime Behavior { data-search-exclude }

Thiết kế phải chịu foreground/background/suspend/terminate và network thay đổi giữa chừng. Với **Cross-platform collaboration: backend, web, Android và API contracts**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.

Một callback xuất hiện không chứng minh object còn owner đúng; một UI update đúng không chứng minh request cũ đã bị cancel; một compile success cũng không chứng minh logical ordering đúng.

## Memory Implications { data-search-exclude }

Mỗi cache/buffer/media pipeline cần capacity, eviction và decoded-size budget.

```text
Who creates? → Who owns? → Who releases? → Expected deinit/eviction?
```

Nếu không có reference object trong chapter, câu hỏi vẫn hữu ích cho buffer, cache, task capture và framework object được ví dụ tạo ra.

## Concurrency Implications { data-search-exclude }

Xác định task owner, coalescing, cancellation, ordering, retry và persistence của pending work.

```text
Task owner? → isolation? → cancellation? → lifetime? → ordering?
```

Data-race freedom không tự động bảo đảm business invariant nhiều bước. Sau suspension, response hoặc lifecycle change, state có thể đã thuộc generation khác.

## Architecture Notes { data-search-exclude }

Nêu source of truth, dependency direction, team ownership và migration path; luôn ghi trade-off. Dependency hướng vào contract ổn định; implementation details nằm phía ngoài. Composition root tạo concrete dependencies và quyết định lifetime.

Không thêm layer chỉ vì chapter nhắc đến pattern. Hãy yêu cầu layer mới có ít nhất một giá trị: hấp thụ volatility, bảo vệ invariant, tạo test seam hoặc quản lý lifetime.

## Production Case { data-search-exclude }

### Context

Feature Commerce áp dụng **Cross-platform collaboration: backend, web, Android và API contracts** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy backend thay enum bắt buộc, web/iOS khác cookie session, Android/iOS khác lifecycle hoặc rollout phá backward compatibility.

### Hypotheses

1. Contract bị hiểu sai tại call site.
2. Owner/lifetime không khớp feature scope.
3. Ordering hoặc external input phá invariant.
4. Resource budget/policy thiếu bound.

### Investigation

Dùng OpenAPI/fixture contract tests, compatibility matrix, correlation ID và staged-rollout metrics; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.

### Root Cause

Root cause chỉ được kết luận khi evidence chỉ ra nơi invariant bị phá, không phải nơi symptom cuối cùng xuất hiện.

### Fix

Áp dụng rule **đồng sở hữu API contract, dùng tolerant reader có chủ đích, version breaking change và ghi rõ platform-specific capability**, thu hẹp mutation/ownership và làm failure/cancellation explicit.

### Prevention

Thêm regression test tại boundary, metric cho failure mode, review checklist về owner/state và documentation cho constraint quyết định.

## Debug / Instruments { data-search-exclude }

Primary evidence: **OpenAPI/fixture contract tests, compatibility matrix, correlation ID và staged-rollout metrics**.

1. Tái hiện trên build/device gần production.
2. Đánh dấu user flow bằng signpost hoặc correlation ID không chứa PII.
3. Thu trace/log trước khi sửa.
4. Đặt hypothesis có thể bị bác bỏ.
5. Đo lại cùng workload và thêm regression protection.

## Myth vs Reality { data-search-exclude }

> **Myth:** Chỉ cần dùng đúng API/pattern tên **Cross-platform collaboration: backend, web, Android và API contracts** là code an toàn.
>
> **Reality:** Safety đến từ semantics, owner, invariant, lifecycle và evidence; API chỉ là một phần của contract.

## Common Mistakes { data-search-exclude }

- Backend thay enum bắt buộc, web/ios khác cookie session, android/ios khác lifecycle hoặc rollout phá backward compatibility → behavior chỉ sai ở scale/lifecycle edge.
- Che failure bằng default hoặc retry → mất root-cause signal.
- Nhiều layer cùng mutate state → source of truth không còn rõ.
- Tối ưu trước khi đo → tăng complexity nhưng không cải thiện bottleneck.

## Best Practices { data-search-exclude }

- Đồng sở hữu api contract, dùng tolerant reader có chủ đích, version breaking change và ghi rõ platform-specific capability.
- Ghi assumption và availability cạnh boundary version-sensitive.
- Dùng immutable value và explicit state transition khi phù hợp.
- Log category/correlation an toàn; không log token, PII hoặc payment payload.
- Đo trước/sau trên cùng workload và giữ regression test.

## Interview Questions { data-search-exclude }

### Foundation

**Hỏi:** Cross-platform collaboration: backend, web, Android và API contracts giải quyết vấn đề gì?

**30-second:** Cross-platform delivery cần contract chung về schema, error, auth, time, money, pagination, idempotency và rollout thay vì mỗi client tự suy luận. Chọn nó khi đồng sở hữu API contract, dùng tolerant reader có chủ đích, version breaking change và ghi rõ platform-specific capability; rủi ro chính là backend thay enum bắt buộc, web/iOS khác cookie session, Android/iOS khác lifecycle hoặc rollout phá backward compatibility.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao backend thay enum bắt buộc, web/iOS khác cookie session, Android/iOS khác lifecycle hoặc rollout phá backward compatibility phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **OpenAPI/fixture contract tests, compatibility matrix, correlation ID và staged-rollout metrics**.

### Senior

**Hỏi:** Constraint nào khiến bạn chọn giải pháp khác?

Deep Dive phải so sánh complexity, lifecycle, scale, migration, operability và rollback; không biến best practice thành luật tuyệt đối.

### Production

**Hỏi:** Symptom chỉ xảy ra trên 0,1% session. Bạn điều tra thế nào?

Clarify impact → thu evidence → hypothesis → controlled measurement → root cause → mitigation/fix → regression metric.

## Exercises { data-search-exclude }

### Easy

Viết một ví dụ nhỏ minh họa **Cross-platform collaboration: backend, web, Android và API contracts** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **backend thay enum bắt buộc, web/iOS khác cookie session, Android/iOS khác lifecycle hoặc rollout phá backward compatibility** để contract/owner rõ hơn.

### Hard

Thiết kế state transition có cancellation/lifecycle interruption và chứng minh không publish stale result.

### Debugging Lab

Bug report: repeated Commerce flow tạo symptom liên quan **Cross-platform collaboration: backend, web, Android và API contracts**. Thu evidence bằng OpenAPI/fixture contract tests, compatibility matrix, correlation ID và staged-rollout metrics, vẽ owner/state graph, xác định root cause và thêm regression test.

### Engineering / Design Exercise

Viết ADR một trang: context, options, decision, consequences và revisit conditions cho lựa chọn trong chapter.

## Cheat Sheet { data-search-exclude }

```text
Concept   → cross-platform delivery cần contract chung về schema, error, auth, time, money, pagination, idempotency và rollout thay vì mỗi client tự suy luận
Use when  → đồng sở hữu API contract, dùng tolerant reader có chủ đích, version breaking change và ghi rõ platform-specific capability
Risk      → backend thay enum bắt buộc, web/iOS khác cookie session, Android/iOS khác lifecycle hoặc rollout phá backward compatibility
Evidence  → OpenAPI/fixture contract tests, compatibility matrix, correlation ID và staged-rollout metrics
Remember  → owner + state + failure + lifecycle + measurement
```

## Chapter Summary { data-search-exclude }

1. Problem: API/pattern không đủ nếu semantics và owner mơ hồ.
2. Mental model: Requirements → constraints → data/state → architecture → failure/security/performance → observability/testing.
3. Usage rule: đồng sở hữu API contract, dùng tolerant reader có chủ đích, version breaking change và ghi rõ platform-specific capability.
4. Mistake nguy hiểm: backend thay enum bắt buộc, web/iOS khác cookie session, Android/iOS khác lifecycle hoặc rollout phá backward compatibility.
5. Production lesson: kết luận bằng OpenAPI/fixture contract tests, compatibility matrix, correlation ID và staged-rollout metrics, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Commerce Checkout state machine](17-commerce-checkout-state-machine.md)
- [Phase Review: Mobile System Design](99-phase-review.md)
- [Cross-reference Index](../CROSS_REFERENCE_INDEX.md)

## References { data-search-exclude }

- [App architecture](https://developer.apple.com/documentation/technologyoverviews/app-architecture) — truy cập 2026-08-09.
- [Background tasks](https://developer.apple.com/documentation/backgroundtasks) — truy cập 2026-08-09.
- [Performance and metrics](https://developer.apple.com/documentation/xcode/performance-and-metrics) — truy cập 2026-08-09.
- [OpenAPI Specification](https://spec.openapis.org/oas/latest.html) — truy cập 2026-08-09.
- [HTTP Semantics — RFC 9110](https://www.rfc-editor.org/rfc/rfc9110) — truy cập 2026-08-09.

## Completion Checklist { data-search-exclude }

- [x] Objectives có thể kiểm chứng
- [x] Mental model và giới hạn được nói rõ
- [x] What/Why/How/When/What-if đầy đủ
- [x] Code hoặc availability rõ
- [x] Runtime/memory/concurrency implications đúng phạm vi
- [x] Production case dựa trên evidence
- [x] Review + interview + exercises + cheat sheet
- [x] Internal links chỉ tới file tồn tại trong catalog
- [x] Claim version-sensitive có primary source và ngày verify
- [x] Không còn placeholder; status là complete
