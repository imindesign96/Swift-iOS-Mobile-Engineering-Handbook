---
title: "GCD: serial/concurrent, sync/async và QoS"
phase: "Concurrency"
difficulty: 2
importance: 5
...
used_later:
  - "Deadlock, barrier, group, semaphore và OperationQueue"
competencies:
  - "Concurrency"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "gcd-serial-concurrent-sync-async-va-qos"
  - "global-commerce"
---

# GCD: serial/concurrent, sync/async và QoS
...
Trong Global Commerce, nhiều request Commerce hoàn tất khác thứ tự, tạo duplicate work hoặc ghi đè state mới bằng response cũ. Chapter này tập trung vào **GCD: serial/concurrent, sync/async và QoS**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng queue quyết định scheduling contract; sync/async nói caller có chờ hay không;
- nhận diện failure mode chính: sync vào cùng serial queue hoặc dùng concurrent queue mà không bảo vệ state;
- chọn giải pháp bằng rule: ưu tiên structured concurrency cho flow mới; giữ GCD ở API/system boundary phù hợp;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng stack sample, queue label và QoS evidence;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Thread, shared mutable state và data race](01-thread-shared-mutable-state-va-data-race.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Deadlock, barrier, group, semaphore và OperationQueue](03-deadlock-barrier-group-semaphore-va-operationqueue.md) dùng contract của chapter này làm building block.
...
Task owner → suspension points → isolation domain → cancellation/order → observable state
                 ↓
Focus: GCD: serial/concurrent, sync/async và QoS
                 ↓
Evidence: stack sample, queue label và QoS evidence
```
...
Trọng tâm của **GCD: serial/concurrent, sync/async và QoS** là: queue quyết định scheduling contract; sync/async nói caller có chờ hay không. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng stack sample, queue label và QoS evidence.
...
Áp dụng khi ưu tiên structured concurrency cho flow mới; giữ GCD ở API/system boundary phù hợp. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là sync vào cùng serial queue hoặc dùng concurrent queue mà không bảo vệ state. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa GCD: serial/concurrent, sync/async và QoS?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Sync vào cùng serial queue hoặc dùng concurrent queue mà không bảo vệ state tạo evidence gì?
4. Rule chọn giải pháp là gì, và constraint nào khiến rule đổi?

## Runnable Swift Example

```swift
let task = Task { try await repository.products() }
...
Một async function có thể suspend và resume; await không phải lệnh chuyển sang background thread. Với **GCD: serial/concurrent, sync/async và QoS**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **GCD: serial/concurrent, sync/async và QoS** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy sync vào cùng serial queue hoặc dùng concurrent queue mà không bảo vệ state.
...
Dùng stack sample, queue label và QoS evidence; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **ưu tiên structured concurrency cho flow mới; giữ GCD ở API/system boundary phù hợp**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **stack sample, queue label và QoS evidence**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **GCD: serial/concurrent, sync/async và QoS** là code an toàn.
>
...
- Sync vào cùng serial queue hoặc dùng concurrent queue mà không bảo vệ state → behavior chỉ sai ở scale/lifecycle edge.
...
- Ưu tiên structured concurrency cho flow mới; giữ gcd ở api/system boundary phù hợp.
...
**Hỏi:** GCD: serial/concurrent, sync/async và QoS giải quyết vấn đề gì?

**30-second:** Queue quyết định scheduling contract; sync/async nói caller có chờ hay không. Chọn nó khi ưu tiên structured concurrency cho flow mới; giữ GCD ở API/system boundary phù hợp; rủi ro chính là sync vào cùng serial queue hoặc dùng concurrent queue mà không bảo vệ state.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao sync vào cùng serial queue hoặc dùng concurrent queue mà không bảo vệ state phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **stack sample, queue label và QoS evidence**.
...
Viết một ví dụ nhỏ minh họa **GCD: serial/concurrent, sync/async và QoS** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **sync vào cùng serial queue hoặc dùng concurrent queue mà không bảo vệ state** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **GCD: serial/concurrent, sync/async và QoS**. Thu evidence bằng stack sample, queue label và QoS evidence, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → queue quyết định scheduling contract; sync/async nói caller có chờ hay không
Use when  → ưu tiên structured concurrency cho flow mới; giữ GCD ở API/system boundary phù hợp
Risk      → sync vào cùng serial queue hoặc dùng concurrent queue mà không bảo vệ state
Evidence  → stack sample, queue label và QoS evidence
...
3. Usage rule: ưu tiên structured concurrency cho flow mới; giữ GCD ở API/system boundary phù hợp.
4. Mistake nguy hiểm: sync vào cùng serial queue hoặc dùng concurrent queue mà không bảo vệ state.
5. Production lesson: kết luận bằng stack sample, queue label và QoS evidence, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Thread, shared mutable state và data race](01-thread-shared-mutable-state-va-data-race.md)
- [Deadlock, barrier, group, semaphore và OperationQueue](03-deadlock-barrier-group-semaphore-va-operationqueue.md)
