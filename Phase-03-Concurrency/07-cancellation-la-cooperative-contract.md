---
title: "Cancellation là cooperative contract"
phase: "Concurrency"
difficulty: 3
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "cancellation-la-cooperative-contract"
  - "global-commerce"
---
...
Trong Global Commerce, nhiều request Commerce hoàn tất khác thứ tự, tạo duplicate work hoặc ghi đè state mới bằng response cũ. Chapter này tập trung vào **Cancellation là cooperative contract**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [`async let` và TaskGroup](06-async-let-va-taskgroup.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Actor và actor isolation](08-actor-va-actor-isolation.md) dùng contract của chapter này làm building block.
...
Task owner → suspension points → isolation domain → cancellation/order → observable state
                 ↓
Focus: Cancellation là cooperative contract
                 ↓
...
Trọng tâm của **Cancellation là cooperative contract** là: cancellation là cooperative signal, không cưỡng bức dừng mọi code. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Cancellation là cooperative contract?
...
Một async function có thể suspend và resume; await không phải lệnh chuyển sang background thread. Với **Cancellation là cooperative contract**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Cancellation là cooperative contract** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Cancellation là cooperative contract** là code an toàn.
>
...
**Hỏi:** Cancellation là cooperative contract giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Cancellation là cooperative contract** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Cancellation là cooperative contract**. Thu evidence bằng cancel-to-stop latency và state after cancel, vẽ owner/state graph, xác định root cause và thêm regression test.
