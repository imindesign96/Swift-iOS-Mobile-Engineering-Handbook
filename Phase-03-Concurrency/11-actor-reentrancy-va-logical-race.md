---
title: "Actor reentrancy và logical race"
phase: "Concurrency"
difficulty: 3
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "actor-reentrancy-va-logical-race"
  - "global-commerce"
---
...
Trong Global Commerce, nhiều request Commerce hoàn tất khác thứ tự, tạo duplicate work hoặc ghi đè state mới bằng response cũ. Chapter này tập trung vào **Actor reentrancy và logical race**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng actor loại data race trên isolated state nhưng mỗi await mở cửa cho work khác;
- nhận diện failure mode chính: đọc state, await, rồi dùng assumption cũ;
- chọn giải pháp bằng rule: revalidate hoặc lưu in-flight task/single-flight state trong actor;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng operation ID/state machine và duplicate-work metric;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Sendable, `@Sendable` và strict concurrency](10-sendable-at-sendable-va-strict-concurrency.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Structured vs unstructured vs detached task](12-structured-vs-unstructured-vs-detached-task.md) dùng contract của chapter này làm building block.
...
Task owner → suspension points → isolation domain → cancellation/order → observable state
                 ↓
Focus: Actor reentrancy và logical race
                 ↓
Evidence: operation ID/state machine và duplicate-work metric
```
...
Trọng tâm của **Actor reentrancy và logical race** là: actor loại data race trên isolated state nhưng mỗi await mở cửa cho work khác. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng operation ID/state machine và duplicate-work metric.
...
Áp dụng khi revalidate hoặc lưu in-flight task/single-flight state trong actor. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là đọc state, await, rồi dùng assumption cũ. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Actor reentrancy và logical race?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Đọc state, await, rồi dùng assumption cũ tạo evidence gì?
...
Một async function có thể suspend và resume; await không phải lệnh chuyển sang background thread. Với **Actor reentrancy và logical race**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Actor reentrancy và logical race** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy đọc state, await, rồi dùng assumption cũ.
...
Dùng operation ID/state machine và duplicate-work metric; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **revalidate hoặc lưu in-flight task/single-flight state trong actor**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **operation ID/state machine và duplicate-work metric**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Actor reentrancy và logical race** là code an toàn.
>
...
- Đọc state, await, rồi dùng assumption cũ → behavior chỉ sai ở scale/lifecycle edge.
...
- Revalidate hoặc lưu in-flight task/single-flight state trong actor.
...
**Hỏi:** Actor reentrancy và logical race giải quyết vấn đề gì?

**30-second:** Actor loại data race trên isolated state nhưng mỗi await mở cửa cho work khác. Chọn nó khi revalidate hoặc lưu in-flight task/single-flight state trong actor; rủi ro chính là đọc state, await, rồi dùng assumption cũ.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao đọc state, await, rồi dùng assumption cũ phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **operation ID/state machine và duplicate-work metric**.
...
Viết một ví dụ nhỏ minh họa **Actor reentrancy và logical race** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **đọc state, await, rồi dùng assumption cũ** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Actor reentrancy và logical race**. Thu evidence bằng operation ID/state machine và duplicate-work metric, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → actor loại data race trên isolated state nhưng mỗi await mở cửa cho work khác
Use when  → revalidate hoặc lưu in-flight task/single-flight state trong actor
Risk      → đọc state, await, rồi dùng assumption cũ
Evidence  → operation ID/state machine và duplicate-work metric
...
3. Usage rule: revalidate hoặc lưu in-flight task/single-flight state trong actor.
4. Mistake nguy hiểm: đọc state, await, rồi dùng assumption cũ.
5. Production lesson: kết luận bằng operation ID/state machine và duplicate-work metric, rồi bảo vệ bằng test và metric.
...
- [Structured vs unstructured vs detached task](12-structured-vs-unstructured-vs-detached-task.md)
