---
title: "Task và structured concurrency"
phase: "Concurrency"
difficulty: 3
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "task-va-structured-concurrency"
  - "global-commerce"
---
...
Trong Global Commerce, nhiều request Commerce hoàn tất khác thứ tự, tạo duplicate work hoặc ghi đè state mới bằng response cũ. Chapter này tập trung vào **Task và structured concurrency**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng Task và structured concurrency phải được hiểu bằng behavior, constraint và trade-off quan sát được;
- nhận diện failure mode chính: dùng API/pattern theo tên gọi mà không xác định owner, state và failure mode;
- chọn giải pháp bằng rule: chọn giải pháp phục vụ mục tiêu: quản lý isolation, task lifetime, cancellation và ordering thay vì suy nghĩ bằng thread thuần túy;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng Strict Concurrency diagnostics, Swift Concurrency instrument, Thread Sanitizer khi phù hợp và structured logs theo task/request.;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Điều gì thực sự xảy ra tại `await`?](04-ieu-gi-thuc-su-xay-ra-tai-await.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [`async let` và TaskGroup](06-async-let-va-taskgroup.md) dùng contract của chapter này làm building block.
...
Task owner → suspension points → isolation domain → cancellation/order → observable state
                 ↓
Focus: Task và structured concurrency
                 ↓
...
Trọng tâm của **Task và structured concurrency** là: Task và structured concurrency phải được hiểu bằng behavior, constraint và trade-off quan sát được. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng Strict Concurrency diagnostics, Swift Concurrency instrument, Thread Sanitizer khi phù hợp và structured logs theo task/request..
...
Áp dụng khi chọn giải pháp phục vụ mục tiêu: quản lý isolation, task lifetime, cancellation và ordering thay vì suy nghĩ bằng thread thuần túy. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
1. Observable behavior nào định nghĩa Task và structured concurrency?
...
Một async function có thể suspend và resume; await không phải lệnh chuyển sang background thread. Với **Task và structured concurrency**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Task và structured concurrency** trong flow có network, UI lifecycle và cache.
...
Dùng Strict Concurrency diagnostics, Swift Concurrency instrument, Thread Sanitizer khi phù hợp và structured logs theo task/request.; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **chọn giải pháp phục vụ mục tiêu: quản lý isolation, task lifetime, cancellation và ordering thay vì suy nghĩ bằng thread thuần túy**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **Strict Concurrency diagnostics, Swift Concurrency instrument, Thread Sanitizer khi phù hợp và structured logs theo task/request.**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Task và structured concurrency** là code an toàn.
>
...
- Chọn giải pháp phục vụ mục tiêu: quản lý isolation, task lifetime, cancellation và ordering thay vì suy nghĩ bằng thread thuần túy.
...
**Hỏi:** Task và structured concurrency giải quyết vấn đề gì?

**30-second:** Task và structured concurrency phải được hiểu bằng behavior, constraint và trade-off quan sát được. Chọn nó khi chọn giải pháp phục vụ mục tiêu: quản lý isolation, task lifetime, cancellation và ordering thay vì suy nghĩ bằng thread thuần túy; rủi ro chính là dùng API/pattern theo tên gọi mà không xác định owner, state và failure mode.
...
Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **Strict Concurrency diagnostics, Swift Concurrency instrument, Thread Sanitizer khi phù hợp và structured logs theo task/request.**.
...
Viết một ví dụ nhỏ minh họa **Task và structured concurrency** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Task và structured concurrency**. Thu evidence bằng Strict Concurrency diagnostics, Swift Concurrency instrument, Thread Sanitizer khi phù hợp và structured logs theo task/request., vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → Task và structured concurrency phải được hiểu bằng behavior, constraint và trade-off quan sát được
Use when  → chọn giải pháp phục vụ mục tiêu: quản lý isolation, task lifetime, cancellation và ordering thay vì suy nghĩ bằng thread thuần túy
...
3. Usage rule: chọn giải pháp phục vụ mục tiêu: quản lý isolation, task lifetime, cancellation và ordering thay vì suy nghĩ bằng thread thuần túy.
4. Mistake nguy hiểm: dùng API/pattern theo tên gọi mà không xác định owner, state và failure mode.
5. Production lesson: kết luận bằng Strict Concurrency diagnostics, Swift Concurrency instrument, Thread Sanitizer khi phù hợp và structured logs theo task/request., rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Điều gì thực sự xảy ra tại `await`?](04-ieu-gi-thuc-su-xay-ra-tai-await.md)
- [`async let` và TaskGroup](06-async-let-va-taskgroup.md)
