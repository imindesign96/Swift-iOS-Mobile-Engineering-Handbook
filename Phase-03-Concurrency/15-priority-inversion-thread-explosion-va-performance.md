---
title: "Priority inversion, thread explosion và performance"
phase: "Concurrency"
difficulty: 3
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "priority-inversion-thread-explosion-va-performance"
  - "global-commerce"
---
...
Trong Global Commerce, nhiều request Commerce hoàn tất khác thứ tự, tạo duplicate work hoặc ghi đè state mới bằng response cũ. Chapter này tập trung vào **Priority inversion, thread explosion và performance**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Migration từ callback/GCD sang async/await](14-migration-tu-callback-gcd-sang-async-await.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
...
Task owner → suspension points → isolation domain → cancellation/order → observable state
                 ↓
Focus: Priority inversion, thread explosion và performance
                 ↓
...
Trọng tâm của **Priority inversion, thread explosion và performance** là: data race là concurrent unsynchronized conflicting access vào cùng memory. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Priority inversion, thread explosion và performance?
...
Một async function có thể suspend và resume; await không phải lệnh chuyển sang background thread. Với **Priority inversion, thread explosion và performance**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Priority inversion, thread explosion và performance** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Priority inversion, thread explosion và performance** là code an toàn.
>
...
**Hỏi:** Priority inversion, thread explosion và performance giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Priority inversion, thread explosion và performance** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Priority inversion, thread explosion và performance**. Thu evidence bằng Thread Sanitizer/strict concurrency và ordering logs, vẽ owner/state graph, xác định root cause và thêm regression test.
