...
Trong Global Commerce, nhiều request Commerce hoàn tất khác thứ tự, tạo duplicate work hoặc ghi đè state mới bằng response cũ. Chapter này tập trung vào **`async let` và TaskGroup**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Task và structured concurrency](05-task-va-structured-concurrency.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Cancellation là cooperative contract](07-cancellation-la-cooperative-contract.md) dùng contract của chapter này làm building block.
...
Trọng tâm của **`async let` và TaskGroup** là: coordination primitive giải một constraint cụ thể và có cost/liveness risk. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa `async let` và TaskGroup?
...
Một async function có thể suspend và resume; await không phải lệnh chuyển sang background thread. Với **`async let` và TaskGroup**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **`async let` và TaskGroup** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **`async let` và TaskGroup** là code an toàn.
>
...
**Hỏi:** `async let` và TaskGroup giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **`async let` và TaskGroup** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **`async let` và TaskGroup**. Thu evidence bằng hang sample và wait-for graph, vẽ owner/state graph, xác định root cause và thêm regression test.
