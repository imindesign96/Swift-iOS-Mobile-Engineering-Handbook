---
title: "Deadlock, barrier, group, semaphore và OperationQueue"
phase: "Concurrency"
difficulty: 2
importance: 5
...
used_later:
  - "Điều gì thực sự xảy ra tại `await`?"
competencies:
  - "Concurrency"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "deadlock-barrier-group-semaphore-va-operationqueue"
  - "global-commerce"
---
...
Trong Global Commerce, nhiều request Commerce hoàn tất khác thứ tự, tạo duplicate work hoặc ghi đè state mới bằng response cũ. Chapter này tập trung vào **Deadlock, barrier, group, semaphore và OperationQueue**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng coordination primitive giải một constraint cụ thể và có cost/liveness risk;
- nhận diện failure mode chính: block thread bằng semaphore để giả async hoặc lock ordering mơ hồ;
- chọn giải pháp bằng rule: chọn primitive nhỏ nhất; ưu tiên async coordination không block thread;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng hang sample và wait-for graph;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [GCD: serial/concurrent, sync/async và QoS](02-gcd-serial-concurrent-sync-async-va-qos.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Điều gì thực sự xảy ra tại `await`?](04-ieu-gi-thuc-su-xay-ra-tai-await.md) dùng contract của chapter này làm building block.
...
Task owner → suspension points → isolation domain → cancellation/order → observable state
                 ↓
Focus: Deadlock, barrier, group, semaphore và OperationQueue
                 ↓
Evidence: hang sample và wait-for graph
```
...
Trọng tâm của **Deadlock, barrier, group, semaphore và OperationQueue** là: coordination primitive giải một constraint cụ thể và có cost/liveness risk. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng hang sample và wait-for graph.
...
Áp dụng khi chọn primitive nhỏ nhất; ưu tiên async coordination không block thread. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là block thread bằng semaphore để giả async hoặc lock ordering mơ hồ. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Deadlock, barrier, group, semaphore và OperationQueue?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Block thread bằng semaphore để giả async hoặc lock ordering mơ hồ tạo evidence gì?
...
Một async function có thể suspend và resume; await không phải lệnh chuyển sang background thread. Với **Deadlock, barrier, group, semaphore và OperationQueue**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Deadlock, barrier, group, semaphore và OperationQueue** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy block thread bằng semaphore để giả async hoặc lock ordering mơ hồ.
...
Dùng hang sample và wait-for graph; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **chọn primitive nhỏ nhất; ưu tiên async coordination không block thread**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **hang sample và wait-for graph**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Deadlock, barrier, group, semaphore và OperationQueue** là code an toàn.
>
...
- Block thread bằng semaphore để giả async hoặc lock ordering mơ hồ → behavior chỉ sai ở scale/lifecycle edge.
...
- Chọn primitive nhỏ nhất; ưu tiên async coordination không block thread.
...
**Hỏi:** Deadlock, barrier, group, semaphore và OperationQueue giải quyết vấn đề gì?

**30-second:** Coordination primitive giải một constraint cụ thể và có cost/liveness risk. Chọn nó khi chọn primitive nhỏ nhất; ưu tiên async coordination không block thread; rủi ro chính là block thread bằng semaphore để giả async hoặc lock ordering mơ hồ.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao block thread bằng semaphore để giả async hoặc lock ordering mơ hồ phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **hang sample và wait-for graph**.
...
Viết một ví dụ nhỏ minh họa **Deadlock, barrier, group, semaphore và OperationQueue** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **block thread bằng semaphore để giả async hoặc lock ordering mơ hồ** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Deadlock, barrier, group, semaphore và OperationQueue**. Thu evidence bằng hang sample và wait-for graph, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → coordination primitive giải một constraint cụ thể và có cost/liveness risk
Use when  → chọn primitive nhỏ nhất; ưu tiên async coordination không block thread
Risk      → block thread bằng semaphore để giả async hoặc lock ordering mơ hồ
Evidence  → hang sample và wait-for graph
...
3. Usage rule: chọn primitive nhỏ nhất; ưu tiên async coordination không block thread.
4. Mistake nguy hiểm: block thread bằng semaphore để giả async hoặc lock ordering mơ hồ.
5. Production lesson: kết luận bằng hang sample và wait-for graph, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [GCD: serial/concurrent, sync/async và QoS](02-gcd-serial-concurrent-sync-async-va-qos.md)
- [Điều gì thực sự xảy ra tại `await`?](04-ieu-gi-thuc-su-xay-ra-tai-await.md)
