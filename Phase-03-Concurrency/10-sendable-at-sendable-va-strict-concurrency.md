---
title: "Sendable, `@Sendable` và strict concurrency"
phase: "Concurrency"
difficulty: 3
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "sendable-at-sendable-va-strict-concurrency"
  - "global-commerce"
---
...
Trong Global Commerce, nhiều request Commerce hoàn tất khác thứ tự, tạo duplicate work hoặc ghi đè state mới bằng response cũ. Chapter này tập trung vào **Sendable, `@Sendable` và strict concurrency**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng Sendable mô tả value an toàn khi đi qua isolation boundary;
- nhận diện failure mode chính: unchecked Sendable để tắt warning mà không có synchronization proof;
- chọn giải pháp bằng rule: ưu tiên immutable value; bao shared state bằng actor/lock có contract;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng compiler diagnostics và focused race tests;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [MainActor, global actor và UI isolation](09-mainactor-global-actor-va-ui-isolation.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Actor reentrancy và logical race](11-actor-reentrancy-va-logical-race.md) dùng contract của chapter này làm building block.
...
Task owner → suspension points → isolation domain → cancellation/order → observable state
                 ↓
Focus: Sendable, `@Sendable` và strict concurrency
                 ↓
Evidence: compiler diagnostics và focused race tests
```
...
Trọng tâm của **Sendable, `@Sendable` và strict concurrency** là: Sendable mô tả value an toàn khi đi qua isolation boundary. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng compiler diagnostics và focused race tests.
...
Áp dụng khi ưu tiên immutable value; bao shared state bằng actor/lock có contract. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là unchecked Sendable để tắt warning mà không có synchronization proof. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Sendable, `@Sendable` và strict concurrency?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Unchecked sendable để tắt warning mà không có synchronization proof tạo evidence gì?
...
Một async function có thể suspend và resume; await không phải lệnh chuyển sang background thread. Với **Sendable, `@Sendable` và strict concurrency**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Sendable, `@Sendable` và strict concurrency** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy unchecked Sendable để tắt warning mà không có synchronization proof.
...
Dùng compiler diagnostics và focused race tests; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **ưu tiên immutable value; bao shared state bằng actor/lock có contract**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **compiler diagnostics và focused race tests**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Sendable, `@Sendable` và strict concurrency** là code an toàn.
>
...
- Unchecked sendable để tắt warning mà không có synchronization proof → behavior chỉ sai ở scale/lifecycle edge.
...
- Ưu tiên immutable value; bao shared state bằng actor/lock có contract.
...
**Hỏi:** Sendable, `@Sendable` và strict concurrency giải quyết vấn đề gì?

**30-second:** Sendable mô tả value an toàn khi đi qua isolation boundary. Chọn nó khi ưu tiên immutable value; bao shared state bằng actor/lock có contract; rủi ro chính là unchecked Sendable để tắt warning mà không có synchronization proof.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao unchecked Sendable để tắt warning mà không có synchronization proof phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **compiler diagnostics và focused race tests**.
...
Viết một ví dụ nhỏ minh họa **Sendable, `@Sendable` và strict concurrency** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **unchecked Sendable để tắt warning mà không có synchronization proof** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Sendable, `@Sendable` và strict concurrency**. Thu evidence bằng compiler diagnostics và focused race tests, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → Sendable mô tả value an toàn khi đi qua isolation boundary
Use when  → ưu tiên immutable value; bao shared state bằng actor/lock có contract
Risk      → unchecked Sendable để tắt warning mà không có synchronization proof
Evidence  → compiler diagnostics và focused race tests
...
3. Usage rule: ưu tiên immutable value; bao shared state bằng actor/lock có contract.
4. Mistake nguy hiểm: unchecked Sendable để tắt warning mà không có synchronization proof.
5. Production lesson: kết luận bằng compiler diagnostics và focused race tests, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [MainActor, global actor và UI isolation](09-mainactor-global-actor-va-ui-isolation.md)
