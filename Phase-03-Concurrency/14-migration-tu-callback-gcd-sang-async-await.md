---
title: "Migration từ callback/GCD sang async/await"
phase: "Concurrency"
difficulty: 3
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "migration-tu-callback-gcd-sang-async-await"
  - "global-commerce"
---
...
Trong Global Commerce, nhiều request Commerce hoàn tất khác thứ tự, tạo duplicate work hoặc ghi đè state mới bằng response cũ. Chapter này tập trung vào **Migration từ callback/GCD sang async/await**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Task lifetime qua screen lifecycle](13-task-lifetime-qua-screen-lifecycle.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Priority inversion, thread explosion và performance](15-priority-inversion-thread-explosion-va-performance.md) dùng contract của chapter này làm building block.
...
Task owner → suspension points → isolation domain → cancellation/order → observable state
                 ↓
Focus: Migration từ callback/GCD sang async/await
                 ↓
...
Trọng tâm của **Migration từ callback/GCD sang async/await** là: queue quyết định scheduling contract; sync/async nói caller có chờ hay không. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Migration từ callback/GCD sang async/await?
...
Một async function có thể suspend và resume; await không phải lệnh chuyển sang background thread. Với **Migration từ callback/GCD sang async/await**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Migration từ callback/GCD sang async/await** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Migration từ callback/GCD sang async/await** là code an toàn.
>
...
**Hỏi:** Migration từ callback/GCD sang async/await giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Migration từ callback/GCD sang async/await** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Migration từ callback/GCD sang async/await**. Thu evidence bằng stack sample, queue label và QoS evidence, vẽ owner/state graph, xác định root cause và thêm regression test.
