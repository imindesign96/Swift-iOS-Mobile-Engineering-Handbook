---
title: "Actor và actor isolation"
phase: "Concurrency"
difficulty: 3
importance: 5
...
levels:
  - L2
prerequisites:
  - "Cancellation là cooperative contract"
used_later:
...
Trong Global Commerce, nhiều request Commerce hoàn tất khác thứ tự, tạo duplicate work hoặc ghi đè state mới bằng response cũ. Chapter này tập trung vào **Actor và actor isolation**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Cancellation là cooperative contract](07-cancellation-la-cooperative-contract.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [MainActor, global actor và UI isolation](09-mainactor-global-actor-va-ui-isolation.md) dùng contract của chapter này làm building block.
...
Task owner → suspension points → isolation domain → cancellation/order → observable state
                 ↓
Focus: Actor và actor isolation
                 ↓
...
Trọng tâm của **Actor và actor isolation** là: actor serialize access vào isolated state, không làm toàn workflow atomic. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Actor và actor isolation?
...
Một async function có thể suspend và resume; await không phải lệnh chuyển sang background thread. Với **Actor và actor isolation**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Actor và actor isolation** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Actor và actor isolation** là code an toàn.
>
...
**Hỏi:** Actor và actor isolation giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Actor và actor isolation** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Actor và actor isolation**. Thu evidence bằng strict concurrency diagnostics và transition logs, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Cancellation là cooperative contract](07-cancellation-la-cooperative-contract.md)
