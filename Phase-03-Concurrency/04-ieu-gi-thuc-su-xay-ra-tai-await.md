---
title: "Điều gì thực sự xảy ra tại `await`?"
phase: "Concurrency"
difficulty: 3
importance: 5
...
used_later:
  - "Task và structured concurrency"
competencies:
  - "Concurrency"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "ieu-gi-thuc-su-xay-ra-tai-await"
  - "global-commerce"
---
...
Trong Global Commerce, nhiều request Commerce hoàn tất khác thứ tự, tạo duplicate work hoặc ghi đè state mới bằng response cũ. Chapter này tập trung vào **Điều gì thực sự xảy ra tại `await`?**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng await đánh dấu nơi function có thể suspend và actor state có thể thay đổi trước khi resume;
- nhận diện failure mode chính: coi await là background hop hoặc atomic transaction;
- chọn giải pháp bằng rule: giữ critical state transition ngắn và revalidate sau await;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng log trước/sau suspension và kiểm tra invariant lại;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Deadlock, barrier, group, semaphore và OperationQueue](03-deadlock-barrier-group-semaphore-va-operationqueue.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Task và structured concurrency](05-task-va-structured-concurrency.md) dùng contract của chapter này làm building block.
...
Task owner → suspension points → isolation domain → cancellation/order → observable state
                 ↓
Focus: Điều gì thực sự xảy ra tại `await`?
                 ↓
Evidence: log trước/sau suspension và kiểm tra invariant lại
```
...
Trọng tâm của **Điều gì thực sự xảy ra tại `await`?** là: await đánh dấu nơi function có thể suspend và actor state có thể thay đổi trước khi resume. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng log trước/sau suspension và kiểm tra invariant lại.
...
Áp dụng khi giữ critical state transition ngắn và revalidate sau await. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là coi await là background hop hoặc atomic transaction. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Điều gì thực sự xảy ra tại `await`??
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Coi await là background hop hoặc atomic transaction tạo evidence gì?
...
Một async function có thể suspend và resume; await không phải lệnh chuyển sang background thread. Với **Điều gì thực sự xảy ra tại `await`?**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Điều gì thực sự xảy ra tại `await`?** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy coi await là background hop hoặc atomic transaction.
...
Dùng log trước/sau suspension và kiểm tra invariant lại; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **giữ critical state transition ngắn và revalidate sau await**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **log trước/sau suspension và kiểm tra invariant lại**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Điều gì thực sự xảy ra tại `await`?** là code an toàn.
>
...
- Coi await là background hop hoặc atomic transaction → behavior chỉ sai ở scale/lifecycle edge.
...
- Giữ critical state transition ngắn và revalidate sau await.
...
**Hỏi:** Điều gì thực sự xảy ra tại `await`? giải quyết vấn đề gì?

**30-second:** Await đánh dấu nơi function có thể suspend và actor state có thể thay đổi trước khi resume. Chọn nó khi giữ critical state transition ngắn và revalidate sau await; rủi ro chính là coi await là background hop hoặc atomic transaction.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao coi await là background hop hoặc atomic transaction phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **log trước/sau suspension và kiểm tra invariant lại**.
...
Viết một ví dụ nhỏ minh họa **Điều gì thực sự xảy ra tại `await`?** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **coi await là background hop hoặc atomic transaction** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Điều gì thực sự xảy ra tại `await`?**. Thu evidence bằng log trước/sau suspension và kiểm tra invariant lại, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → await đánh dấu nơi function có thể suspend và actor state có thể thay đổi trước khi resume
Use when  → giữ critical state transition ngắn và revalidate sau await
Risk      → coi await là background hop hoặc atomic transaction
Evidence  → log trước/sau suspension và kiểm tra invariant lại
...
3. Usage rule: giữ critical state transition ngắn và revalidate sau await.
4. Mistake nguy hiểm: coi await là background hop hoặc atomic transaction.
5. Production lesson: kết luận bằng log trước/sau suspension và kiểm tra invariant lại, rồi bảo vệ bằng test và metric.
...
- [Task và structured concurrency](05-task-va-structured-concurrency.md)
