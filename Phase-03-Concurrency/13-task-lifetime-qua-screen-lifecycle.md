---
title: "Task lifetime qua screen lifecycle"
phase: "Concurrency"
difficulty: 3
importance: 5
...
Trong Global Commerce, nhiều request Commerce hoàn tất khác thứ tự, tạo duplicate work hoặc ghi đè state mới bằng response cũ. Chapter này tập trung vào **Task lifetime qua screen lifecycle**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Structured vs unstructured vs detached task](12-structured-vs-unstructured-vs-detached-task.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Migration từ callback/GCD sang async/await](14-migration-tu-callback-gcd-sang-async-await.md) dùng contract của chapter này làm building block.
...
Task owner → suspension points → isolation domain → cancellation/order → observable state
                 ↓
Focus: Task lifetime qua screen lifecycle
                 ↓
...
Trọng tâm của **Task lifetime qua screen lifecycle** là: registration/work tạo lifetime edge cần cleanup contract. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Task lifetime qua screen lifecycle?
...
Một async function có thể suspend và resume; await không phải lệnh chuyển sang background thread. Với **Task lifetime qua screen lifecycle**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Task lifetime qua screen lifecycle** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Task lifetime qua screen lifecycle** là code an toàn.
>
...
**Hỏi:** Task lifetime qua screen lifecycle giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Task lifetime qua screen lifecycle** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Task lifetime qua screen lifecycle**. Thu evidence bằng deinit probe, cancellation log và path to root, vẽ owner/state graph, xác định root cause và thêm regression test.
