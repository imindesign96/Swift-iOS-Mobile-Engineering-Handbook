---
title: "Thread, shared mutable state và data race"
phase: "Concurrency"
difficulty: 2
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "thread-shared-mutable-state-va-data-race"
  - "global-commerce"
---
...
Trong Global Commerce, nhiều request Commerce hoàn tất khác thứ tự, tạo duplicate work hoặc ghi đè state mới bằng response cũ. Chapter này tập trung vào **Thread, shared mutable state và data race**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng data race là concurrent unsynchronized conflicting access vào cùng memory;
- nhận diện failure mode chính: đồng nhất race condition với data race;
- chọn giải pháp bằng rule: loại shared mutation hoặc đặt một isolation/synchronization owner;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng Thread Sanitizer/strict concurrency và ordering logs;
...
- [GCD: serial/concurrent, sync/async và QoS](02-gcd-serial-concurrent-sync-async-va-qos.md) dùng contract của chapter này làm building block.
...
Task owner → suspension points → isolation domain → cancellation/order → observable state
                 ↓
Focus: Thread, shared mutable state và data race
                 ↓
Evidence: Thread Sanitizer/strict concurrency và ordering logs
```
...
Trọng tâm của **Thread, shared mutable state và data race** là: data race là concurrent unsynchronized conflicting access vào cùng memory. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng Thread Sanitizer/strict concurrency và ordering logs.
...
Áp dụng khi loại shared mutation hoặc đặt một isolation/synchronization owner. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là đồng nhất race condition với data race. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Thread, shared mutable state và data race?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Đồng nhất race condition với data race tạo evidence gì?
...
Một async function có thể suspend và resume; await không phải lệnh chuyển sang background thread. Với **Thread, shared mutable state và data race**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Thread, shared mutable state và data race** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy đồng nhất race condition với data race.
...
Dùng Thread Sanitizer/strict concurrency và ordering logs; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **loại shared mutation hoặc đặt một isolation/synchronization owner**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **Thread Sanitizer/strict concurrency và ordering logs**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Thread, shared mutable state và data race** là code an toàn.
>
...
- Đồng nhất race condition với data race → behavior chỉ sai ở scale/lifecycle edge.
...
- Loại shared mutation hoặc đặt một isolation/synchronization owner.
...
**Hỏi:** Thread, shared mutable state và data race giải quyết vấn đề gì?

**30-second:** Data race là concurrent unsynchronized conflicting access vào cùng memory. Chọn nó khi loại shared mutation hoặc đặt một isolation/synchronization owner; rủi ro chính là đồng nhất race condition với data race.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao đồng nhất race condition với data race phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **Thread Sanitizer/strict concurrency và ordering logs**.
...
Viết một ví dụ nhỏ minh họa **Thread, shared mutable state và data race** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **đồng nhất race condition với data race** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Thread, shared mutable state và data race**. Thu evidence bằng Thread Sanitizer/strict concurrency và ordering logs, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → data race là concurrent unsynchronized conflicting access vào cùng memory
Use when  → loại shared mutation hoặc đặt một isolation/synchronization owner
Risk      → đồng nhất race condition với data race
Evidence  → Thread Sanitizer/strict concurrency và ordering logs
...
3. Usage rule: loại shared mutation hoặc đặt một isolation/synchronization owner.
4. Mistake nguy hiểm: đồng nhất race condition với data race.
5. Production lesson: kết luận bằng Thread Sanitizer/strict concurrency và ordering logs, rồi bảo vệ bằng test và metric.
