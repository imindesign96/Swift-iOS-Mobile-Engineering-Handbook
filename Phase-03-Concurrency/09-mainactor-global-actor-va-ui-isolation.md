---
title: "MainActor, global actor và UI isolation"
phase: "Concurrency"
difficulty: 3
importance: 5
...
used_later:
  - "Sendable, `@Sendable` và strict concurrency"
competencies:
  - "Concurrency"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "mainactor-global-actor-va-ui-isolation"
  - "global-commerce"
---

# MainActor, global actor và UI isolation
...
Trong Global Commerce, nhiều request Commerce hoàn tất khác thứ tự, tạo duplicate work hoặc ghi đè state mới bằng response cũ. Chapter này tập trung vào **MainActor, global actor và UI isolation**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Actor và actor isolation](08-actor-va-actor-isolation.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Sendable, `@Sendable` và strict concurrency](10-sendable-at-sendable-va-strict-concurrency.md) dùng contract của chapter này làm building block.
...
Task owner → suspension points → isolation domain → cancellation/order → observable state
                 ↓
Focus: MainActor, global actor và UI isolation
                 ↓
...
Trọng tâm của **MainActor, global actor và UI isolation** là: actor serialize access vào isolated state, không làm toàn workflow atomic. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa MainActor, global actor và UI isolation?
...
Một async function có thể suspend và resume; await không phải lệnh chuyển sang background thread. Với **MainActor, global actor và UI isolation**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **MainActor, global actor và UI isolation** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **MainActor, global actor và UI isolation** là code an toàn.
>
...
**Hỏi:** MainActor, global actor và UI isolation giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **MainActor, global actor và UI isolation** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **MainActor, global actor và UI isolation**. Thu evidence bằng strict concurrency diagnostics và transition logs, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Actor và actor isolation](08-actor-va-actor-isolation.md)
- [Sendable, `@Sendable` và strict concurrency](10-sendable-at-sendable-va-strict-concurrency.md)
