...
Trong Global Commerce, ứng viên nhớ định nghĩa nhưng không giải thích trade-off, evidence hoặc follow-up production. Chapter này tập trung vào **Architecture review**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Persistence review](07-persistence-review.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Testing review](09-testing-review.md) dùng contract của chapter này làm building block.
...
Trọng tâm của **Architecture review** là: ARC quản lý lifetime qua strong references trong ownership graph. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Architecture review?
...
Câu trả lời framework/runtime phải tách documented behavior khỏi inference và tránh slogan. Với **Architecture review**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Architecture review** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Architecture review** là code an toàn.
>
...
**Hỏi:** Architecture review giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Architecture review** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Architecture review**. Thu evidence bằng Memory Graph + deinit expectation, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Persistence review](07-persistence-review.md)
