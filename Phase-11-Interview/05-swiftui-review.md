...
Trong Global Commerce, ứng viên nhớ định nghĩa nhưng không giải thích trade-off, evidence hoặc follow-up production. Chapter này tập trung vào **SwiftUI review**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng SwiftUI state ownership và stable identity quyết định lifetime/update;
- nhận diện failure mode chính: nhiều source of truth, unstable ID hoặc heavy body;
- chọn giải pháp bằng rule: owner tạo state; Binding chỉ mượn mutation capability; dependency đi qua boundary rõ;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng SwiftUI Instruments và state-transition log;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [UIKit review](04-uikit-review.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Networking review](06-networking-review.md) dùng contract của chapter này làm building block.
...
Clarify → 30-second thesis → mechanism/trade-off → example → production evidence → senior extension
                 ↓
Focus: SwiftUI review
                 ↓
Evidence: SwiftUI Instruments và state-transition log
```
...
Trọng tâm của **SwiftUI review** là: SwiftUI state ownership và stable identity quyết định lifetime/update. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng SwiftUI Instruments và state-transition log.
...
Áp dụng khi owner tạo state; Binding chỉ mượn mutation capability; dependency đi qua boundary rõ. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là nhiều source of truth, unstable ID hoặc heavy body. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa SwiftUI review?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Nhiều source of truth, unstable id hoặc heavy body tạo evidence gì?
...
Câu trả lời framework/runtime phải tách documented behavior khỏi inference và tránh slogan. Với **SwiftUI review**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **SwiftUI review** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy nhiều source of truth, unstable ID hoặc heavy body.
...
Dùng SwiftUI Instruments và state-transition log; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **owner tạo state; Binding chỉ mượn mutation capability; dependency đi qua boundary rõ**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **SwiftUI Instruments và state-transition log**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **SwiftUI review** là code an toàn.
>
...
- Nhiều source of truth, unstable id hoặc heavy body → behavior chỉ sai ở scale/lifecycle edge.
...
- Owner tạo state; binding chỉ mượn mutation capability; dependency đi qua boundary rõ.
...
**Hỏi:** SwiftUI review giải quyết vấn đề gì?

**30-second:** Swiftui state ownership và stable identity quyết định lifetime/update. Chọn nó khi owner tạo state; Binding chỉ mượn mutation capability; dependency đi qua boundary rõ; rủi ro chính là nhiều source of truth, unstable ID hoặc heavy body.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao nhiều source of truth, unstable ID hoặc heavy body phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **SwiftUI Instruments và state-transition log**.
...
Viết một ví dụ nhỏ minh họa **SwiftUI review** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **nhiều source of truth, unstable ID hoặc heavy body** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **SwiftUI review**. Thu evidence bằng SwiftUI Instruments và state-transition log, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → SwiftUI state ownership và stable identity quyết định lifetime/update
Use when  → owner tạo state; Binding chỉ mượn mutation capability; dependency đi qua boundary rõ
Risk      → nhiều source of truth, unstable ID hoặc heavy body
Evidence  → SwiftUI Instruments và state-transition log
...
3. Usage rule: owner tạo state; Binding chỉ mượn mutation capability; dependency đi qua boundary rõ.
4. Mistake nguy hiểm: nhiều source of truth, unstable ID hoặc heavy body.
5. Production lesson: kết luận bằng SwiftUI Instruments và state-transition log, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [UIKit review](04-uikit-review.md)
- [Networking review](06-networking-review.md)
