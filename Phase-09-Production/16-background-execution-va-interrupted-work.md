---
title: "Background execution và interrupted work"
phase: "Production"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "background-execution-va-interrupted-work"
  - "global-commerce"
---
...
Trong Global Commerce, một lỗi hiếm chỉ xuất hiện trên thiết bị thật và hàng triệu session, nơi đoán mò không còn hiệu quả. Chapter này tập trung vào **Background execution và interrupted work**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Release-only và device-specific bugs](15-release-only-va-device-specific-bugs.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Concurrency incidents ở quy mô session lớn](17-concurrency-incidents-o-quy-mo-session-lon.md) dùng contract của chapter này làm building block.
...
Symptom → evidence → hypotheses → measurement → root cause → fix → prevention
                 ↓
Focus: Background execution và interrupted work
                 ↓
...
Trọng tâm của **Background execution và interrupted work** là: app/scene state là external lifecycle signal, không phải bảo đảm thời lượng. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Background execution và interrupted work?
...
Crash/hang/termination/performance regression tạo artifact khác nhau; trước hết phải phân loại đúng tín hiệu. Với **Background execution và interrupted work**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Background execution và interrupted work** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Background execution và interrupted work** là code an toàn.
>
...
**Hỏi:** Background execution và interrupted work giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Background execution và interrupted work** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Background execution và interrupted work**. Thu evidence bằng lifecycle logs và background task expiration, vẽ owner/state graph, xác định root cause và thêm regression test.
