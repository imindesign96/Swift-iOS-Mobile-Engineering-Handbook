---
title: "Crash report và symbolication"
phase: "Production"
difficulty: 5
importance: 5
...
Trong Global Commerce, một lỗi hiếm chỉ xuất hiện trên thiết bị thật và hàng triệu session, nơi đoán mò không còn hiệu quả. Chapter này tập trung vào **Crash report và symbolication**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Structured, privacy-aware logging](01-structured-privacy-aware-logging.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [EXC_BAD_ACCESS, fatal error và index out of range](03-exc-bad-access-fatal-error-va-index-out-of-range.md) dùng contract của chapter này làm building block.
...
Symptom → evidence → hypotheses → measurement → root cause → fix → prevention
                 ↓
Focus: Crash report và symbolication
                 ↓
...
Trọng tâm của **Crash report và symbolication** là: crash analysis bắt đầu từ symbolicated faulting thread và app frames. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Crash report và symbolication?
...
Crash/hang/termination/performance regression tạo artifact khác nhau; trước hết phải phân loại đúng tín hiệu. Với **Crash report và symbolication**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Crash report và symbolication** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Crash report và symbolication** là code an toàn.
>
...
**Hỏi:** Crash report và symbolication giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Crash report và symbolication** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Crash report và symbolication**. Thu evidence bằng crash report, exception/termination reason và breadcrumbs, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Structured, privacy-aware logging](01-structured-privacy-aware-logging.md)
- [EXC_BAD_ACCESS, fatal error và index out of range](03-exc-bad-access-fatal-error-va-index-out-of-range.md)
