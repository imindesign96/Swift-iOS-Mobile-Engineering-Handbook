...
Trong Global Commerce, một lỗi hiếm chỉ xuất hiện trên thiết bị thật và hàng triệu session, nơi đoán mò không còn hiệu quả. Chapter này tập trung vào **Memory leak investigation**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng Memory leak investigation phải được hiểu bằng behavior, constraint và trade-off quan sát được;
...
- [Hang, watchdog và stack evidence](04-hang-watchdog-va-stack-evidence.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Memory pressure và OS termination](06-memory-pressure-va-os-termination.md) dùng contract của chapter này làm building block.
...
Symptom → evidence → hypotheses → measurement → root cause → fix → prevention
                 ↓
Focus: Memory leak investigation
                 ↓
...
Trọng tâm của **Memory leak investigation** là: Memory leak investigation phải được hiểu bằng behavior, constraint và trade-off quan sát được. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Memory leak investigation?
...
Crash/hang/termination/performance regression tạo artifact khác nhau; trước hết phải phân loại đúng tín hiệu. Với **Memory leak investigation**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Memory leak investigation** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Memory leak investigation** là code an toàn.
>
...
Viết một ví dụ nhỏ minh họa **Memory leak investigation** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Memory leak investigation**. Thu evidence bằng Xcode Organizer, crash report/symbolication, Instruments, MetricKit và structured production telemetry., vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → Memory leak investigation phải được hiểu bằng behavior, constraint và trade-off quan sát được
