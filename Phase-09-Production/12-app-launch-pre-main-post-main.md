---
title: "App launch: pre-main/post-main"
phase: "Production"
difficulty: 5
importance: 5
...
Trong Global Commerce, một lỗi hiếm chỉ xuất hiện trên thiết bị thật và hàng triệu session, nơi đoán mò không còn hiệu quả. Chapter này tập trung vào **App launch: pre-main/post-main**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Image decoding, downsampling và cache budget](11-image-decoding-downsampling-va-cache-budget.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Battery và energy diagnostics](13-battery-va-energy-diagnostics.md) dùng contract của chapter này làm building block.
...
Symptom → evidence → hypotheses → measurement → root cause → fix → prevention
                 ↓
Focus: App launch: pre-main/post-main
                 ↓
...
Trọng tâm của **App launch: pre-main/post-main** là: performance work cần baseline, trace và budget trên device đại diện. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa App launch: pre-main/post-main?
...
Crash/hang/termination/performance regression tạo artifact khác nhau; trước hết phải phân loại đúng tín hiệu. Với **App launch: pre-main/post-main**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **App launch: pre-main/post-main** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **App launch: pre-main/post-main** là code an toàn.
>
...
**Hỏi:** App launch: pre-main/post-main giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **App launch: pre-main/post-main** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **App launch: pre-main/post-main**. Thu evidence bằng call tree, signpost, launch/hang/energy metrics, vẽ owner/state graph, xác định root cause và thêm regression test.
