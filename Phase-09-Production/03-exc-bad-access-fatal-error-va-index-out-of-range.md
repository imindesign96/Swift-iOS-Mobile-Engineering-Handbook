---
title: "EXC_BAD_ACCESS, fatal error và index out of range"
phase: "Production"
difficulty: 5
importance: 5
...
levels:
  - L5
prerequisites:
  - "Crash report và symbolication"
used_later:
  - "Hang, watchdog và stack evidence"
competencies:
  - "Production"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "exc-bad-access-fatal-error-va-index-out-of-range"
  - "global-commerce"
---

# EXC_BAD_ACCESS, fatal error và index out of range
...
Trong Global Commerce, một lỗi hiếm chỉ xuất hiện trên thiết bị thật và hàng triệu session, nơi đoán mò không còn hiệu quả. Chapter này tập trung vào **EXC_BAD_ACCESS, fatal error và index out of range**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng crash analysis bắt đầu từ symbolicated faulting thread và app frames;
- nhận diện failure mode chính: fix dòng top stack mà không xét context/volume;
- chọn giải pháp bằng rule: reproduce invariant, fix root cause và thêm regression test;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng crash report, exception/termination reason và breadcrumbs;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Crash report và symbolication](02-crash-report-va-symbolication.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Hang, watchdog và stack evidence](04-hang-watchdog-va-stack-evidence.md) dùng contract của chapter này làm building block.
...
Symptom → evidence → hypotheses → measurement → root cause → fix → prevention
                 ↓
Focus: EXC_BAD_ACCESS, fatal error và index out of range
                 ↓
Evidence: crash report, exception/termination reason và breadcrumbs
```
...
Trọng tâm của **EXC_BAD_ACCESS, fatal error và index out of range** là: crash analysis bắt đầu từ symbolicated faulting thread và app frames. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng crash report, exception/termination reason và breadcrumbs.
...
Áp dụng khi reproduce invariant, fix root cause và thêm regression test. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là fix dòng top stack mà không xét context/volume. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa EXC_BAD_ACCESS, fatal error và index out of range?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Fix dòng top stack mà không xét context/volume tạo evidence gì?
...
Crash/hang/termination/performance regression tạo artifact khác nhau; trước hết phải phân loại đúng tín hiệu. Với **EXC_BAD_ACCESS, fatal error và index out of range**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **EXC_BAD_ACCESS, fatal error và index out of range** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy fix dòng top stack mà không xét context/volume.
...
Dùng crash report, exception/termination reason và breadcrumbs; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **reproduce invariant, fix root cause và thêm regression test**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **crash report, exception/termination reason và breadcrumbs**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **EXC_BAD_ACCESS, fatal error và index out of range** là code an toàn.
>
...
- Fix dòng top stack mà không xét context/volume → behavior chỉ sai ở scale/lifecycle edge.
...
- Reproduce invariant, fix root cause và thêm regression test.
...
**Hỏi:** EXC_BAD_ACCESS, fatal error và index out of range giải quyết vấn đề gì?

**30-second:** Crash analysis bắt đầu từ symbolicated faulting thread và app frames. Chọn nó khi reproduce invariant, fix root cause và thêm regression test; rủi ro chính là fix dòng top stack mà không xét context/volume.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao fix dòng top stack mà không xét context/volume phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **crash report, exception/termination reason và breadcrumbs**.
...
Viết một ví dụ nhỏ minh họa **EXC_BAD_ACCESS, fatal error và index out of range** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **fix dòng top stack mà không xét context/volume** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **EXC_BAD_ACCESS, fatal error và index out of range**. Thu evidence bằng crash report, exception/termination reason và breadcrumbs, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → crash analysis bắt đầu từ symbolicated faulting thread và app frames
Use when  → reproduce invariant, fix root cause và thêm regression test
Risk      → fix dòng top stack mà không xét context/volume
Evidence  → crash report, exception/termination reason và breadcrumbs
...
3. Usage rule: reproduce invariant, fix root cause và thêm regression test.
4. Mistake nguy hiểm: fix dòng top stack mà không xét context/volume.
5. Production lesson: kết luận bằng crash report, exception/termination reason và breadcrumbs, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Crash report và symbolication](02-crash-report-va-symbolication.md)
- [Hang, watchdog và stack evidence](04-hang-watchdog-va-stack-evidence.md)
