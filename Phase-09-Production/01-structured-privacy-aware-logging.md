---
title: "Structured, privacy-aware logging"
phase: "Production"
difficulty: 5
importance: 5
...
Trong Global Commerce, một lỗi hiếm chỉ xuất hiện trên thiết bị thật và hàng triệu session, nơi đoán mò không còn hiệu quả. Chapter này tập trung vào **Structured, privacy-aware logging**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Crash report và symbolication](02-crash-report-va-symbolication.md) dùng contract của chapter này làm building block.
...
Symptom → evidence → hypotheses → measurement → root cause → fix → prevention
                 ↓
Focus: Structured, privacy-aware logging
                 ↓
...
Trọng tâm của **Structured, privacy-aware logging** là: telemetry phải trả lời user impact và hỗ trợ correlation mà không lộ dữ liệu. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Structured, privacy-aware logging?
...
Crash/hang/termination/performance regression tạo artifact khác nhau; trước hết phải phân loại đúng tín hiệu. Với **Structured, privacy-aware logging**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Structured, privacy-aware logging** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Structured, privacy-aware logging** là code an toàn.
>
...
**Hỏi:** Structured, privacy-aware logging giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Structured, privacy-aware logging** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Structured, privacy-aware logging**. Thu evidence bằng dashboard theo SLO và trace/request ID, vẽ owner/state graph, xác định root cause và thêm regression test.
