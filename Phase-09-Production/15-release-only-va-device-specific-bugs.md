---
title: "Release-only và device-specific bugs"
phase: "Production"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "release-only-va-device-specific-bugs"
  - "global-commerce"
---
...
Trong Global Commerce, một lỗi hiếm chỉ xuất hiện trên thiết bị thật và hàng triệu session, nơi đoán mò không còn hiệu quả. Chapter này tập trung vào **Release-only và device-specific bugs**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng Release-only và device-specific bugs phải được hiểu bằng behavior, constraint và trade-off quan sát được;
...
- [Network-only production failure](14-network-only-production-failure.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Background execution và interrupted work](16-background-execution-va-interrupted-work.md) dùng contract của chapter này làm building block.
...
Symptom → evidence → hypotheses → measurement → root cause → fix → prevention
                 ↓
Focus: Release-only và device-specific bugs
                 ↓
...
Trọng tâm của **Release-only và device-specific bugs** là: Release-only và device-specific bugs phải được hiểu bằng behavior, constraint và trade-off quan sát được. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Release-only và device-specific bugs?
...
Crash/hang/termination/performance regression tạo artifact khác nhau; trước hết phải phân loại đúng tín hiệu. Với **Release-only và device-specific bugs**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Release-only và device-specific bugs** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Release-only và device-specific bugs** là code an toàn.
>
...
Viết một ví dụ nhỏ minh họa **Release-only và device-specific bugs** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Release-only và device-specific bugs**. Thu evidence bằng Xcode Organizer, crash report/symbolication, Instruments, MetricKit và structured production telemetry., vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → Release-only và device-specific bugs phải được hiểu bằng behavior, constraint và trade-off quan sát được
