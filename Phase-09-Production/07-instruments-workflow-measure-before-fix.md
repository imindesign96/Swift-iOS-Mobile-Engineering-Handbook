---
title: "Instruments workflow: measure before fix"
phase: "Production"
difficulty: 5
importance: 5
...
Trong Global Commerce, một lỗi hiếm chỉ xuất hiện trên thiết bị thật và hàng triệu session, nơi đoán mò không còn hiệu quả. Chapter này tập trung vào **Instruments workflow: measure before fix**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng Instruments workflow: measure before fix phải được hiểu bằng behavior, constraint và trade-off quan sát được;
...
- [Memory pressure và OS termination](06-memory-pressure-va-os-termination.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Time Profiler và hot call tree](08-time-profiler-va-hot-call-tree.md) dùng contract của chapter này làm building block.
...
Symptom → evidence → hypotheses → measurement → root cause → fix → prevention
                 ↓
Focus: Instruments workflow: measure before fix
                 ↓
...
Trọng tâm của **Instruments workflow: measure before fix** là: Instruments workflow: measure before fix phải được hiểu bằng behavior, constraint và trade-off quan sát được. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Instruments workflow: measure before fix?
...
Crash/hang/termination/performance regression tạo artifact khác nhau; trước hết phải phân loại đúng tín hiệu. Với **Instruments workflow: measure before fix**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Instruments workflow: measure before fix** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Instruments workflow: measure before fix** là code an toàn.
>
...
Viết một ví dụ nhỏ minh họa **Instruments workflow: measure before fix** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Instruments workflow: measure before fix**. Thu evidence bằng Xcode Organizer, crash report/symbolication, Instruments, MetricKit và structured production telemetry., vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → Instruments workflow: measure before fix phải được hiểu bằng behavior, constraint và trade-off quan sát được
...
- [Memory pressure và OS termination](06-memory-pressure-va-os-termination.md)
