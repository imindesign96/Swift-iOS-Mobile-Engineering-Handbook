---
title: "Hang, watchdog và stack evidence"
phase: "Production"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "hang-watchdog-va-stack-evidence"
  - "global-commerce"
---
...
Trong Global Commerce, một lỗi hiếm chỉ xuất hiện trên thiết bị thật và hàng triệu session, nơi đoán mò không còn hiệu quả. Chapter này tập trung vào **Hang, watchdog và stack evidence**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng performance work cần baseline, trace và budget trên device đại diện;
- nhận diện failure mode chính: optimize theo cảm giác hoặc chỉ đo simulator/debug;
- chọn giải pháp bằng rule: giảm critical-path work và đo trước/sau;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng call tree, signpost, launch/hang/energy metrics;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [EXC_BAD_ACCESS, fatal error và index out of range](03-exc-bad-access-fatal-error-va-index-out-of-range.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Memory leak investigation](05-memory-leak-investigation.md) dùng contract của chapter này làm building block.
...
Symptom → evidence → hypotheses → measurement → root cause → fix → prevention
                 ↓
Focus: Hang, watchdog và stack evidence
                 ↓
Evidence: call tree, signpost, launch/hang/energy metrics
```
...
Trọng tâm của **Hang, watchdog và stack evidence** là: performance work cần baseline, trace và budget trên device đại diện. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng call tree, signpost, launch/hang/energy metrics.
...
Áp dụng khi giảm critical-path work và đo trước/sau. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là optimize theo cảm giác hoặc chỉ đo simulator/debug. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Hang, watchdog và stack evidence?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Optimize theo cảm giác hoặc chỉ đo simulator/debug tạo evidence gì?
...
Crash/hang/termination/performance regression tạo artifact khác nhau; trước hết phải phân loại đúng tín hiệu. Với **Hang, watchdog và stack evidence**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Hang, watchdog và stack evidence** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy optimize theo cảm giác hoặc chỉ đo simulator/debug.
...
Dùng call tree, signpost, launch/hang/energy metrics; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **giảm critical-path work và đo trước/sau**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **call tree, signpost, launch/hang/energy metrics**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Hang, watchdog và stack evidence** là code an toàn.
>
...
- Optimize theo cảm giác hoặc chỉ đo simulator/debug → behavior chỉ sai ở scale/lifecycle edge.
...
- Giảm critical-path work và đo trước/sau.
...
**Hỏi:** Hang, watchdog và stack evidence giải quyết vấn đề gì?

**30-second:** Performance work cần baseline, trace và budget trên device đại diện. Chọn nó khi giảm critical-path work và đo trước/sau; rủi ro chính là optimize theo cảm giác hoặc chỉ đo simulator/debug.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao optimize theo cảm giác hoặc chỉ đo simulator/debug phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **call tree, signpost, launch/hang/energy metrics**.
...
Viết một ví dụ nhỏ minh họa **Hang, watchdog và stack evidence** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **optimize theo cảm giác hoặc chỉ đo simulator/debug** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Hang, watchdog và stack evidence**. Thu evidence bằng call tree, signpost, launch/hang/energy metrics, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → performance work cần baseline, trace và budget trên device đại diện
Use when  → giảm critical-path work và đo trước/sau
Risk      → optimize theo cảm giác hoặc chỉ đo simulator/debug
Evidence  → call tree, signpost, launch/hang/energy metrics
...
3. Usage rule: giảm critical-path work và đo trước/sau.
4. Mistake nguy hiểm: optimize theo cảm giác hoặc chỉ đo simulator/debug.
5. Production lesson: kết luận bằng call tree, signpost, launch/hang/energy metrics, rồi bảo vệ bằng test và metric.
