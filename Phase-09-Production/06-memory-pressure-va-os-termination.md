---
title: "Memory pressure và OS termination"
phase: "Production"
difficulty: 5
importance: 5
...
used_later:
  - "Instruments workflow: measure before fix"
competencies:
  - "Production"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "memory-pressure-va-os-termination"
  - "global-commerce"
---
...
Trong Global Commerce, một lỗi hiếm chỉ xuất hiện trên thiết bị thật và hàng triệu session, nơi đoán mò không còn hiệu quả. Chapter này tập trung vào **Memory pressure và OS termination**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Memory leak investigation](05-memory-leak-investigation.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Instruments workflow: measure before fix](07-instruments-workflow-measure-before-fix.md) dùng contract của chapter này làm building block.
...
Symptom → evidence → hypotheses → measurement → root cause → fix → prevention
                 ↓
Focus: Memory pressure và OS termination
                 ↓
...
Trọng tâm của **Memory pressure và OS termination** là: leak là object đáng lẽ chết; pressure có thể đến từ object hợp lệ quá lớn. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Memory pressure và OS termination?
...
Crash/hang/termination/performance regression tạo artifact khác nhau; trước hết phải phân loại đúng tín hiệu. Với **Memory pressure và OS termination**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Memory pressure và OS termination** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Memory pressure và OS termination** là code an toàn.
>
...
**Hỏi:** Memory pressure và OS termination giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Memory pressure và OS termination** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Memory pressure và OS termination**. Thu evidence bằng repeated-flow plateau, allocations và decoded-size budget, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Memory leak investigation](05-memory-leak-investigation.md)
- [Instruments workflow: measure before fix](07-instruments-workflow-measure-before-fix.md)
