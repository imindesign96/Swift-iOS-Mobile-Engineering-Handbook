---
title: "Time Profiler và hot call tree"
phase: "Production"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "time-profiler-va-hot-call-tree"
  - "global-commerce"
---
...
Trong Global Commerce, một lỗi hiếm chỉ xuất hiện trên thiết bị thật và hàng triệu session, nơi đoán mò không còn hiệu quả. Chapter này tập trung vào **Time Profiler và hot call tree**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Instruments workflow: measure before fix](07-instruments-workflow-measure-before-fix.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Allocations, Leaks và Memory Graph](09-allocations-leaks-va-memory-graph.md) dùng contract của chapter này làm building block.
...
Symptom → evidence → hypotheses → measurement → root cause → fix → prevention
                 ↓
Focus: Time Profiler và hot call tree
                 ↓
...
Trọng tâm của **Time Profiler và hot call tree** là: performance work cần baseline, trace và budget trên device đại diện. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Time Profiler và hot call tree?
...
Crash/hang/termination/performance regression tạo artifact khác nhau; trước hết phải phân loại đúng tín hiệu. Với **Time Profiler và hot call tree**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Time Profiler và hot call tree** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Time Profiler và hot call tree** là code an toàn.
>
...
**Hỏi:** Time Profiler và hot call tree giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Time Profiler và hot call tree** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Time Profiler và hot call tree**. Thu evidence bằng call tree, signpost, launch/hang/energy metrics, vẽ owner/state graph, xác định root cause và thêm regression test.
