---
title: "Observability, SLO và incident response"
phase: "Production"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "observability-slo-va-incident-response"
  - "global-commerce"
---
...
Trong Global Commerce, một lỗi hiếm chỉ xuất hiện trên thiết bị thật và hàng triệu session, nơi đoán mò không còn hiệu quả. Chapter này tập trung vào **Observability, SLO và incident response**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Concurrency incidents ở quy mô session lớn](17-concurrency-incidents-o-quy-mo-session-lon.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
...
Symptom → evidence → hypotheses → measurement → root cause → fix → prevention
                 ↓
Focus: Observability, SLO và incident response
                 ↓
...
Trọng tâm của **Observability, SLO và incident response** là: telemetry phải trả lời user impact và hỗ trợ correlation mà không lộ dữ liệu. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Observability, SLO và incident response?
...
Crash/hang/termination/performance regression tạo artifact khác nhau; trước hết phải phân loại đúng tín hiệu. Với **Observability, SLO và incident response**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Observability, SLO và incident response** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Observability, SLO và incident response** là code an toàn.
>
...
**Hỏi:** Observability, SLO và incident response giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Observability, SLO và incident response** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Observability, SLO và incident response**. Thu evidence bằng dashboard theo SLO và trace/request ID, vẽ owner/state graph, xác định root cause và thêm regression test.
