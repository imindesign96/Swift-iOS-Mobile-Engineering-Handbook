---
title: "UseCase/Interactor: khi nào đáng thêm một layer"
phase: "Architecture"
difficulty: 5
importance: 5
...
levels:
  - L4
prerequisites:
  - "Service Locator và Singleton: trade-offs thật"
used_later:
  - "Clean Architecture principles trên mobile"
competencies:
  - "Architecture"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "usecase-interactor-khi-nao-ang-them-mot-layer"
  - "global-commerce"
---

# UseCase/Interactor: khi nào đáng thêm một layer
...
Trong Global Commerce, một thay đổi Checkout phải sửa ViewController, networking, cache và analytics cùng lúc vì responsibility không có ranh giới. Chapter này tập trung vào **UseCase/Interactor: khi nào đáng thêm một layer**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng actor serialize access vào isolated state, không làm toàn workflow atomic;
- nhận diện failure mode chính: expose mutable reference hoặc await giữa invariant nhiều bước;
- chọn giải pháp bằng rule: đặt shared state trong actor và thiết kế method theo transaction logic;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng strict concurrency diagnostics và transition logs;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Service Locator và Singleton: trade-offs thật](06-service-locator-va-singleton-trade-offs-that.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Clean Architecture principles trên mobile](08-clean-architecture-principles-tren-mobile.md) dùng contract của chapter này làm building block.
...
UI → application policy → repository boundary → remote/local implementation
                 ↓
Focus: UseCase/Interactor: khi nào đáng thêm một layer
                 ↓
Evidence: strict concurrency diagnostics và transition logs
```
...
Trọng tâm của **UseCase/Interactor: khi nào đáng thêm một layer** là: actor serialize access vào isolated state, không làm toàn workflow atomic. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
Concept tồn tại để giúp tạo dependency direction và ownership boundary vừa đủ cho complexity hiện tại. Không có boundary này, rule domain bị nhân bản, behavior phụ thuộc call order và incident khó điều tra vì không biết layer nào chịu trách nhiệm.
...
5. Đo observable behavior bằng strict concurrency diagnostics và transition logs.
...
Áp dụng khi đặt shared state trong actor và thiết kế method theo transaction logic. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là expose mutable reference hoặc await giữa invariant nhiều bước. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa UseCase/Interactor: khi nào đáng thêm một layer?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Expose mutable reference hoặc await giữa invariant nhiều bước tạo evidence gì?
...
Architecture không chạy thay framework; nó quyết định nơi state/work/lifetime được sở hữu và quan sát. Với **UseCase/Interactor: khi nào đáng thêm một layer**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Mỗi dependency phải có composition root và lifetime rõ; singleton vô tình biến lifetime thành toàn process.
...
Isolation boundary nên đi cùng ownership boundary; đừng để nhiều layer cùng mutate một state.
...
Thêm layer khi nó hấp thụ volatility hoặc tạo test seam, không thêm chỉ để giống diagram. Dependency hướng vào contract ổn định; implementation details nằm phía ngoài. Composition root tạo concrete dependencies và quyết định lifetime.
...
Feature Commerce áp dụng **UseCase/Interactor: khi nào đáng thêm một layer** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy expose mutable reference hoặc await giữa invariant nhiều bước.
...
Dùng strict concurrency diagnostics và transition logs; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **đặt shared state trong actor và thiết kế method theo transaction logic**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **strict concurrency diagnostics và transition logs**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **UseCase/Interactor: khi nào đáng thêm một layer** là code an toàn.
>
...
- Expose mutable reference hoặc await giữa invariant nhiều bước → behavior chỉ sai ở scale/lifecycle edge.
...
- Đặt shared state trong actor và thiết kế method theo transaction logic.
...
**Hỏi:** UseCase/Interactor: khi nào đáng thêm một layer giải quyết vấn đề gì?

**30-second:** Actor serialize access vào isolated state, không làm toàn workflow atomic. Chọn nó khi đặt shared state trong actor và thiết kế method theo transaction logic; rủi ro chính là expose mutable reference hoặc await giữa invariant nhiều bước.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao expose mutable reference hoặc await giữa invariant nhiều bước phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **strict concurrency diagnostics và transition logs**.
...
Viết một ví dụ nhỏ minh họa **UseCase/Interactor: khi nào đáng thêm một layer** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **expose mutable reference hoặc await giữa invariant nhiều bước** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **UseCase/Interactor: khi nào đáng thêm một layer**. Thu evidence bằng strict concurrency diagnostics và transition logs, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → actor serialize access vào isolated state, không làm toàn workflow atomic
Use when  → đặt shared state trong actor và thiết kế method theo transaction logic
Risk      → expose mutable reference hoặc await giữa invariant nhiều bước
Evidence  → strict concurrency diagnostics và transition logs
...
2. Mental model: UI → application policy → repository boundary → remote/local implementation.
3. Usage rule: đặt shared state trong actor và thiết kế method theo transaction logic.
4. Mistake nguy hiểm: expose mutable reference hoặc await giữa invariant nhiều bước.
5. Production lesson: kết luận bằng strict concurrency diagnostics và transition logs, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Service Locator và Singleton: trade-offs thật](06-service-locator-va-singleton-trade-offs-that.md)
- [Clean Architecture principles trên mobile](08-clean-architecture-principles-tren-mobile.md)
