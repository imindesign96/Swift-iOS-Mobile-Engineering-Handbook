---
title: "Migration strategy không big-bang rewrite"
phase: "Architecture"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "migration-strategy-khong-big-bang-rewrite"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi Checkout phải sửa ViewController, networking, cache và analytics cùng lúc vì responsibility không có ranh giới. Chapter này tập trung vào **Migration strategy không big-bang rewrite**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng migration tốt có seam, slice nhỏ, metric và rollback;
- nhận diện failure mode chính: rewrite toàn bộ trước khi tạo user value;
- chọn giải pháp bằng rule: strangler/incremental path với exit criteria;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng parity tests, adoption metric và defect rate;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Architecture Decision Record](12-architecture-decision-record.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Refactoring Massive ViewController/Model/God Service](14-refactoring-massive-viewcontroller-model-god-service.md) dùng contract của chapter này làm building block.
...
UI → application policy → repository boundary → remote/local implementation
                 ↓
Focus: Migration strategy không big-bang rewrite
                 ↓
Evidence: parity tests, adoption metric và defect rate
```
...
Trọng tâm của **Migration strategy không big-bang rewrite** là: migration tốt có seam, slice nhỏ, metric và rollback. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng parity tests, adoption metric và defect rate.
...
Áp dụng khi strangler/incremental path với exit criteria. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là rewrite toàn bộ trước khi tạo user value. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Migration strategy không big-bang rewrite?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Rewrite toàn bộ trước khi tạo user value tạo evidence gì?
...
Architecture không chạy thay framework; nó quyết định nơi state/work/lifetime được sở hữu và quan sát. Với **Migration strategy không big-bang rewrite**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Migration strategy không big-bang rewrite** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy rewrite toàn bộ trước khi tạo user value.
...
Dùng parity tests, adoption metric và defect rate; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **strangler/incremental path với exit criteria**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **parity tests, adoption metric và defect rate**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Migration strategy không big-bang rewrite** là code an toàn.
>
...
- Rewrite toàn bộ trước khi tạo user value → behavior chỉ sai ở scale/lifecycle edge.
...
- Strangler/incremental path với exit criteria.
...
**Hỏi:** Migration strategy không big-bang rewrite giải quyết vấn đề gì?

**30-second:** Migration tốt có seam, slice nhỏ, metric và rollback. Chọn nó khi strangler/incremental path với exit criteria; rủi ro chính là rewrite toàn bộ trước khi tạo user value.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao rewrite toàn bộ trước khi tạo user value phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **parity tests, adoption metric và defect rate**.
...
Viết một ví dụ nhỏ minh họa **Migration strategy không big-bang rewrite** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **rewrite toàn bộ trước khi tạo user value** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Migration strategy không big-bang rewrite**. Thu evidence bằng parity tests, adoption metric và defect rate, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → migration tốt có seam, slice nhỏ, metric và rollback
Use when  → strangler/incremental path với exit criteria
Risk      → rewrite toàn bộ trước khi tạo user value
Evidence  → parity tests, adoption metric và defect rate
...
3. Usage rule: strangler/incremental path với exit criteria.
4. Mistake nguy hiểm: rewrite toàn bộ trước khi tạo user value.
5. Production lesson: kết luận bằng parity tests, adoption metric và defect rate, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Architecture Decision Record](12-architecture-decision-record.md)
- [Refactoring Massive ViewController/Model/God Service](14-refactoring-massive-viewcontroller-model-god-service.md)
