---
title: "Refactoring Massive ViewController/Model/God Service"
phase: "Architecture"
difficulty: 5
importance: 5
...
levels:
  - L4
prerequisites:
  - "Migration strategy không big-bang rewrite"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "refactoring-massive-viewcontroller-model-god-service"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi Checkout phải sửa ViewController, networking, cache và analytics cùng lúc vì responsibility không có ranh giới. Chapter này tập trung vào **Refactoring Massive ViewController/Model/God Service**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Migration strategy không big-bang rewrite](13-migration-strategy-khong-big-bang-rewrite.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Phase Review: Architecture](99-phase-review.md) dùng contract của chapter này làm building block.
...
UI → application policy → repository boundary → remote/local implementation
                 ↓
Focus: Refactoring Massive ViewController/Model/God Service
                 ↓
...
Trọng tâm của **Refactoring Massive ViewController/Model/God Service** là: actor serialize access vào isolated state, không làm toàn workflow atomic. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Refactoring Massive ViewController/Model/God Service?
...
Architecture không chạy thay framework; nó quyết định nơi state/work/lifetime được sở hữu và quan sát. Với **Refactoring Massive ViewController/Model/God Service**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Refactoring Massive ViewController/Model/God Service** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Refactoring Massive ViewController/Model/God Service** là code an toàn.
>
...
**Hỏi:** Refactoring Massive ViewController/Model/God Service giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Refactoring Massive ViewController/Model/God Service** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Refactoring Massive ViewController/Model/God Service**. Thu evidence bằng strict concurrency diagnostics và transition logs, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Migration strategy không big-bang rewrite](13-migration-strategy-khong-big-bang-rewrite.md)
- [Phase Review: Architecture](99-phase-review.md)
