---
title: "Dependency Injection và constructor injection"
phase: "Architecture"
difficulty: 5
importance: 5
...
levels:
  - L4
prerequisites:
  - "Repository không chỉ là tên khác của API client"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "dependency-injection-va-constructor-injection"
  - "global-commerce"
---

# Dependency Injection và constructor injection
...
Trong Global Commerce, một thay đổi Checkout phải sửa ViewController, networking, cache và analytics cùng lúc vì responsibility không có ranh giới. Chapter này tập trung vào **Dependency Injection và constructor injection**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng constructor injection làm dependency/lifetime bắt buộc và nhìn thấy;
- nhận diện failure mode chính: optional dependency, service locator ẩn hoặc graph tạo rải rác;
- chọn giải pháp bằng rule: compose ở app/feature boundary; inject abstraction khi có volatility/seam;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng composition-root review và test replacement;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Repository không chỉ là tên khác của API client](04-repository-khong-chi-la-ten-khac-cua-api-client.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Service Locator và Singleton: trade-offs thật](06-service-locator-va-singleton-trade-offs-that.md) dùng contract của chapter này làm building block.
...
UI → application policy → repository boundary → remote/local implementation
                 ↓
Focus: Dependency Injection và constructor injection
                 ↓
Evidence: composition-root review và test replacement
```
...
Trọng tâm của **Dependency Injection và constructor injection** là: constructor injection làm dependency/lifetime bắt buộc và nhìn thấy. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng composition-root review và test replacement.
...
Áp dụng khi compose ở app/feature boundary; inject abstraction khi có volatility/seam. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là optional dependency, service locator ẩn hoặc graph tạo rải rác. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Dependency Injection và constructor injection?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Optional dependency, service locator ẩn hoặc graph tạo rải rác tạo evidence gì?
...
Architecture không chạy thay framework; nó quyết định nơi state/work/lifetime được sở hữu và quan sát. Với **Dependency Injection và constructor injection**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Dependency Injection và constructor injection** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy optional dependency, service locator ẩn hoặc graph tạo rải rác.
...
Dùng composition-root review và test replacement; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **compose ở app/feature boundary; inject abstraction khi có volatility/seam**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **composition-root review và test replacement**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Dependency Injection và constructor injection** là code an toàn.
>
...
- Optional dependency, service locator ẩn hoặc graph tạo rải rác → behavior chỉ sai ở scale/lifecycle edge.
...
- Compose ở app/feature boundary; inject abstraction khi có volatility/seam.
...
**Hỏi:** Dependency Injection và constructor injection giải quyết vấn đề gì?

**30-second:** Constructor injection làm dependency/lifetime bắt buộc và nhìn thấy. Chọn nó khi compose ở app/feature boundary; inject abstraction khi có volatility/seam; rủi ro chính là optional dependency, service locator ẩn hoặc graph tạo rải rác.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao optional dependency, service locator ẩn hoặc graph tạo rải rác phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **composition-root review và test replacement**.
...
Viết một ví dụ nhỏ minh họa **Dependency Injection và constructor injection** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **optional dependency, service locator ẩn hoặc graph tạo rải rác** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Dependency Injection và constructor injection**. Thu evidence bằng composition-root review và test replacement, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → constructor injection làm dependency/lifetime bắt buộc và nhìn thấy
Use when  → compose ở app/feature boundary; inject abstraction khi có volatility/seam
Risk      → optional dependency, service locator ẩn hoặc graph tạo rải rác
Evidence  → composition-root review và test replacement
...
3. Usage rule: compose ở app/feature boundary; inject abstraction khi có volatility/seam.
4. Mistake nguy hiểm: optional dependency, service locator ẩn hoặc graph tạo rải rác.
5. Production lesson: kết luận bằng composition-root review và test replacement, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Repository không chỉ là tên khác của API client](04-repository-khong-chi-la-ten-khac-cua-api-client.md)
