---
title: "Coordinator và navigation ownership"
phase: "Architecture"
difficulty: 4
importance: 5
...
levels:
  - L4
prerequisites:
  - "MVVM và ranh giới trách nhiệm ViewModel"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "coordinator-va-navigation-ownership"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi Checkout phải sửa ViewController, networking, cache và analytics cùng lúc vì responsibility không có ranh giới. Chapter này tập trung vào **Coordinator và navigation ownership**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [MVVM và ranh giới trách nhiệm ViewModel](02-mvvm-va-ranh-gioi-trach-nhiem-viewmodel.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Repository không chỉ là tên khác của API client](04-repository-khong-chi-la-ten-khac-cua-api-client.md) dùng contract của chapter này làm building block.
...
UI → application policy → repository boundary → remote/local implementation
                 ↓
Focus: Coordinator và navigation ownership
                 ↓
...
Trọng tâm của **Coordinator và navigation ownership** là: ARC quản lý lifetime qua strong references trong ownership graph. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Coordinator và navigation ownership?
...
Architecture không chạy thay framework; nó quyết định nơi state/work/lifetime được sở hữu và quan sát. Với **Coordinator và navigation ownership**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Coordinator và navigation ownership** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Coordinator và navigation ownership** là code an toàn.
>
...
**Hỏi:** Coordinator và navigation ownership giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Coordinator và navigation ownership** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Coordinator và navigation ownership**. Thu evidence bằng Memory Graph + deinit expectation, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [MVVM và ranh giới trách nhiệm ViewModel](02-mvvm-va-ranh-gioi-trach-nhiem-viewmodel.md)
