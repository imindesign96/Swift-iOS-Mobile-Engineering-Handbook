---
title: "Feature boundary và ngăn circular dependency"
phase: "Architecture"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "feature-boundary-va-ngan-circular-dependency"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi Checkout phải sửa ViewController, networking, cache và analytics cùng lúc vì responsibility không có ranh giới. Chapter này tập trung vào **Feature boundary và ngăn circular dependency**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [SPM modularization và dependency direction](10-spm-modularization-va-dependency-direction.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Architecture Decision Record](12-architecture-decision-record.md) dùng contract của chapter này làm building block.
...
UI → application policy → repository boundary → remote/local implementation
                 ↓
Focus: Feature boundary và ngăn circular dependency
                 ↓
...
Trọng tâm của **Feature boundary và ngăn circular dependency** là: module boundary kiểm soát dependency, build/team ownership và API surface. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Feature boundary và ngăn circular dependency?
...
Architecture không chạy thay framework; nó quyết định nơi state/work/lifetime được sở hữu và quan sát. Với **Feature boundary và ngăn circular dependency**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Feature boundary và ngăn circular dependency** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Feature boundary và ngăn circular dependency** là code an toàn.
>
...
**Hỏi:** Feature boundary và ngăn circular dependency giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Feature boundary và ngăn circular dependency** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Feature boundary và ngăn circular dependency**. Thu evidence bằng dependency graph/build metrics, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Architecture Decision Record](12-architecture-decision-record.md)
