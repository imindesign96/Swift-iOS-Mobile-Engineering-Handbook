---
title: "Service Locator và Singleton: trade-offs thật"
phase: "Architecture"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "service-locator-va-singleton-trade-offs-that"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi Checkout phải sửa ViewController, networking, cache và analytics cùng lúc vì responsibility không có ranh giới. Chapter này tập trung vào **Service Locator và Singleton: trade-offs thật**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng global access tiện nhưng che dependency và mở rộng lifetime/state sharing;
- nhận diện failure mode chính: test phụ thuộc order hoặc account state rò toàn app;
- chọn giải pháp bằng rule: giữ immutable/stateless service hoặc bọc sau injected boundary;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng global-state inventory và parallel-test failures;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Dependency Injection và constructor injection](05-dependency-injection-va-constructor-injection.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [UseCase/Interactor: khi nào đáng thêm một layer](07-usecase-interactor-khi-nao-ang-them-mot-layer.md) dùng contract của chapter này làm building block.
...
UI → application policy → repository boundary → remote/local implementation
                 ↓
Focus: Service Locator và Singleton: trade-offs thật
                 ↓
Evidence: global-state inventory và parallel-test failures
```
...
Trọng tâm của **Service Locator và Singleton: trade-offs thật** là: global access tiện nhưng che dependency và mở rộng lifetime/state sharing. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng global-state inventory và parallel-test failures.
...
Áp dụng khi giữ immutable/stateless service hoặc bọc sau injected boundary. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là test phụ thuộc order hoặc account state rò toàn app. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Service Locator và Singleton: trade-offs thật?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Test phụ thuộc order hoặc account state rò toàn app tạo evidence gì?
4. Rule chọn giải pháp là gì, và constraint nào khiến rule đổi?

## Runnable Swift Example

```swift
let repository: any ProductRepository = APIProductRepository(client: client)
```
...
Architecture không chạy thay framework; nó quyết định nơi state/work/lifetime được sở hữu và quan sát. Với **Service Locator và Singleton: trade-offs thật**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Service Locator và Singleton: trade-offs thật** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy test phụ thuộc order hoặc account state rò toàn app.
...
Dùng global-state inventory và parallel-test failures; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **giữ immutable/stateless service hoặc bọc sau injected boundary**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **global-state inventory và parallel-test failures**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Service Locator và Singleton: trade-offs thật** là code an toàn.
>
...
- Test phụ thuộc order hoặc account state rò toàn app → behavior chỉ sai ở scale/lifecycle edge.
...
- Giữ immutable/stateless service hoặc bọc sau injected boundary.
...
**Hỏi:** Service Locator và Singleton: trade-offs thật giải quyết vấn đề gì?

**30-second:** Global access tiện nhưng che dependency và mở rộng lifetime/state sharing. Chọn nó khi giữ immutable/stateless service hoặc bọc sau injected boundary; rủi ro chính là test phụ thuộc order hoặc account state rò toàn app.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao test phụ thuộc order hoặc account state rò toàn app phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **global-state inventory và parallel-test failures**.
...
Viết một ví dụ nhỏ minh họa **Service Locator và Singleton: trade-offs thật** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **test phụ thuộc order hoặc account state rò toàn app** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Service Locator và Singleton: trade-offs thật**. Thu evidence bằng global-state inventory và parallel-test failures, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → global access tiện nhưng che dependency và mở rộng lifetime/state sharing
Use when  → giữ immutable/stateless service hoặc bọc sau injected boundary
Risk      → test phụ thuộc order hoặc account state rò toàn app
Evidence  → global-state inventory và parallel-test failures
...
3. Usage rule: giữ immutable/stateless service hoặc bọc sau injected boundary.
4. Mistake nguy hiểm: test phụ thuộc order hoặc account state rò toàn app.
5. Production lesson: kết luận bằng global-state inventory và parallel-test failures, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Dependency Injection và constructor injection](05-dependency-injection-va-constructor-injection.md)
- [UseCase/Interactor: khi nào đáng thêm một layer](07-usecase-interactor-khi-nao-ang-them-mot-layer.md)
