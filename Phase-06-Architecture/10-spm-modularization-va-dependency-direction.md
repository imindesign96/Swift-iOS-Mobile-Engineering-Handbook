---
title: "SPM modularization và dependency direction"
phase: "Architecture"
difficulty: 5
importance: 5
...
used_later:
  - "Feature boundary và ngăn circular dependency"
competencies:
  - "Architecture"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "spm-modularization-va-dependency-direction"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi Checkout phải sửa ViewController, networking, cache và analytics cùng lúc vì responsibility không có ranh giới. Chapter này tập trung vào **SPM modularization và dependency direction**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng module boundary kiểm soát dependency, build/team ownership và API surface;
- nhận diện failure mode chính: core dumping-ground hoặc feature import chéo;
- chọn giải pháp bằng rule: feature phụ thuộc capability protocol và lower-level modules, không vòng;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng dependency graph/build metrics;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [State machine thay cho Boolean explosion](09-state-machine-thay-cho-boolean-explosion.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Feature boundary và ngăn circular dependency](11-feature-boundary-va-ngan-circular-dependency.md) dùng contract của chapter này làm building block.
...
UI → application policy → repository boundary → remote/local implementation
                 ↓
Focus: SPM modularization và dependency direction
                 ↓
Evidence: dependency graph/build metrics
```
...
Trọng tâm của **SPM modularization và dependency direction** là: module boundary kiểm soát dependency, build/team ownership và API surface. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng dependency graph/build metrics.
...
Áp dụng khi feature phụ thuộc capability protocol và lower-level modules, không vòng. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là core dumping-ground hoặc feature import chéo. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa SPM modularization và dependency direction?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Core dumping-ground hoặc feature import chéo tạo evidence gì?
...
Architecture không chạy thay framework; nó quyết định nơi state/work/lifetime được sở hữu và quan sát. Với **SPM modularization và dependency direction**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **SPM modularization và dependency direction** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy core dumping-ground hoặc feature import chéo.
...
Dùng dependency graph/build metrics; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **feature phụ thuộc capability protocol và lower-level modules, không vòng**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **dependency graph/build metrics**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **SPM modularization và dependency direction** là code an toàn.
>
...
- Core dumping-ground hoặc feature import chéo → behavior chỉ sai ở scale/lifecycle edge.
...
- Feature phụ thuộc capability protocol và lower-level modules, không vòng.
...
**Hỏi:** SPM modularization và dependency direction giải quyết vấn đề gì?

**30-second:** Module boundary kiểm soát dependency, build/team ownership và api surface. Chọn nó khi feature phụ thuộc capability protocol và lower-level modules, không vòng; rủi ro chính là core dumping-ground hoặc feature import chéo.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao core dumping-ground hoặc feature import chéo phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **dependency graph/build metrics**.
...
Viết một ví dụ nhỏ minh họa **SPM modularization và dependency direction** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **core dumping-ground hoặc feature import chéo** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **SPM modularization và dependency direction**. Thu evidence bằng dependency graph/build metrics, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → module boundary kiểm soát dependency, build/team ownership và API surface
Use when  → feature phụ thuộc capability protocol và lower-level modules, không vòng
Risk      → core dumping-ground hoặc feature import chéo
Evidence  → dependency graph/build metrics
...
3. Usage rule: feature phụ thuộc capability protocol và lower-level modules, không vòng.
4. Mistake nguy hiểm: core dumping-ground hoặc feature import chéo.
5. Production lesson: kết luận bằng dependency graph/build metrics, rồi bảo vệ bằng test và metric.
...
- [Feature boundary và ngăn circular dependency](11-feature-boundary-va-ngan-circular-dependency.md)
