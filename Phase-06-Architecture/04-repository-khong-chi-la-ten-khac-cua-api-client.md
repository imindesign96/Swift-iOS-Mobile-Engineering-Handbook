---
title: "Repository không chỉ là tên khác của API client"
phase: "Architecture"
difficulty: 5
importance: 5
...
levels:
  - L4
prerequisites:
  - "Coordinator và navigation ownership"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "repository-khong-chi-la-ten-khac-cua-api-client"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi Checkout phải sửa ViewController, networking, cache và analytics cùng lúc vì responsibility không có ranh giới. Chapter này tập trung vào **Repository không chỉ là tên khác của API client**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng Endpoint làm request construction typed; API client thực thi transport và map lỗi;
- nhận diện failure mode chính: generic abstraction che method/body/auth policy;
- chọn giải pháp bằng rule: abstraction phải giảm duplication mà vẫn lộ semantics quan trọng;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng request snapshot test và integration fixture;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Coordinator và navigation ownership](03-coordinator-va-navigation-ownership.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Dependency Injection và constructor injection](05-dependency-injection-va-constructor-injection.md) dùng contract của chapter này làm building block.
...
UI → application policy → repository boundary → remote/local implementation
                 ↓
Focus: Repository không chỉ là tên khác của API client
                 ↓
Evidence: request snapshot test và integration fixture
```
...
Trọng tâm của **Repository không chỉ là tên khác của API client** là: Endpoint làm request construction typed; API client thực thi transport và map lỗi. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng request snapshot test và integration fixture.
...
Áp dụng khi abstraction phải giảm duplication mà vẫn lộ semantics quan trọng. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là generic abstraction che method/body/auth policy. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Repository không chỉ là tên khác của API client?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Generic abstraction che method/body/auth policy tạo evidence gì?
...
Architecture không chạy thay framework; nó quyết định nơi state/work/lifetime được sở hữu và quan sát. Với **Repository không chỉ là tên khác của API client**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Repository không chỉ là tên khác của API client** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy generic abstraction che method/body/auth policy.
...
Dùng request snapshot test và integration fixture; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **abstraction phải giảm duplication mà vẫn lộ semantics quan trọng**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **request snapshot test và integration fixture**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Repository không chỉ là tên khác của API client** là code an toàn.
>
...
- Generic abstraction che method/body/auth policy → behavior chỉ sai ở scale/lifecycle edge.
...
- Abstraction phải giảm duplication mà vẫn lộ semantics quan trọng.
...
**Hỏi:** Repository không chỉ là tên khác của API client giải quyết vấn đề gì?

**30-second:** Endpoint làm request construction typed; api client thực thi transport và map lỗi. Chọn nó khi abstraction phải giảm duplication mà vẫn lộ semantics quan trọng; rủi ro chính là generic abstraction che method/body/auth policy.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao generic abstraction che method/body/auth policy phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **request snapshot test và integration fixture**.
...
Viết một ví dụ nhỏ minh họa **Repository không chỉ là tên khác của API client** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **generic abstraction che method/body/auth policy** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Repository không chỉ là tên khác của API client**. Thu evidence bằng request snapshot test và integration fixture, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → Endpoint làm request construction typed; API client thực thi transport và map lỗi
Use when  → abstraction phải giảm duplication mà vẫn lộ semantics quan trọng
Risk      → generic abstraction che method/body/auth policy
Evidence  → request snapshot test và integration fixture
...
3. Usage rule: abstraction phải giảm duplication mà vẫn lộ semantics quan trọng.
4. Mistake nguy hiểm: generic abstraction che method/body/auth policy.
5. Production lesson: kết luận bằng request snapshot test và integration fixture, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Coordinator và navigation ownership](03-coordinator-va-navigation-ownership.md)
