...
Trong Global Commerce, một thay đổi Checkout phải sửa ViewController, networking, cache và analytics cùng lúc vì responsibility không có ranh giới. Chapter này tập trung vào **Architecture Decision Record**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Feature boundary và ngăn circular dependency](11-feature-boundary-va-ngan-circular-dependency.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Migration strategy không big-bang rewrite](13-migration-strategy-khong-big-bang-rewrite.md) dùng contract của chapter này làm building block.
...
UI → application policy → repository boundary → remote/local implementation
                 ↓
Focus: Architecture Decision Record
                 ↓
...
Trọng tâm của **Architecture Decision Record** là: ARC quản lý lifetime qua strong references trong ownership graph. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Architecture Decision Record?
...
Architecture không chạy thay framework; nó quyết định nơi state/work/lifetime được sở hữu và quan sát. Với **Architecture Decision Record**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Architecture Decision Record** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Architecture Decision Record** là code an toàn.
>
...
**Hỏi:** Architecture Decision Record giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Architecture Decision Record** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Architecture Decision Record**. Thu evidence bằng Memory Graph + deinit expectation, vẽ owner/state graph, xác định root cause và thêm regression test.
