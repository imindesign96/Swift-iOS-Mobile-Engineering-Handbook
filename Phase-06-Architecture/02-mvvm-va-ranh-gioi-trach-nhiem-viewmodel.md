---
title: "MVVM và ranh giới trách nhiệm ViewModel"
phase: "Architecture"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "mvvm-va-ranh-gioi-trach-nhiem-viewmodel"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi Checkout phải sửa ViewController, networking, cache và analytics cùng lúc vì responsibility không có ranh giới. Chapter này tập trung vào **MVVM và ranh giới trách nhiệm ViewModel**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [MVC và Massive ViewController](01-mvc-va-massive-viewcontroller.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Coordinator và navigation ownership](03-coordinator-va-navigation-ownership.md) dùng contract của chapter này làm building block.
...
UI → application policy → repository boundary → remote/local implementation
                 ↓
Focus: MVVM và ranh giới trách nhiệm ViewModel
                 ↓
...
Trọng tâm của **MVVM và ranh giới trách nhiệm ViewModel** là: ViewModel chuyển event thành presentation state, không phải nơi chứa mọi thứ. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa MVVM và ranh giới trách nhiệm ViewModel?
...
Architecture không chạy thay framework; nó quyết định nơi state/work/lifetime được sở hữu và quan sát. Với **MVVM và ranh giới trách nhiệm ViewModel**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **MVVM và ranh giới trách nhiệm ViewModel** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **MVVM và ranh giới trách nhiệm ViewModel** là code an toàn.
>
...
**Hỏi:** MVVM và ranh giới trách nhiệm ViewModel giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **MVVM và ranh giới trách nhiệm ViewModel** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **MVVM và ranh giới trách nhiệm ViewModel**. Thu evidence bằng state-transition tests và dependency graph, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [MVC và Massive ViewController](01-mvc-va-massive-viewcontroller.md)
