---
title: "State machine thay cho Boolean explosion"
phase: "Architecture"
difficulty: 5
importance: 5
...
used_later:
  - "SPM modularization và dependency direction"
competencies:
  - "Architecture"
  - "Production"
  - "Debugging"
...
Trong Global Commerce, một thay đổi Checkout phải sửa ViewController, networking, cache và analytics cùng lúc vì responsibility không có ranh giới. Chapter này tập trung vào **State machine thay cho Boolean explosion**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng state machine chỉ cho phép transition hợp lệ giữa các state loại trừ nhau;
- nhận diện failure mode chính: nhiều Boolean tạo tổ hợp bất khả thi và duplicate side effect;
- chọn giải pháp bằng rule: dùng enum state + event/reducer khi workflow có nhiều bước, retry hoặc interruption;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng transition log, exhaustive tests và idempotency metric;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Clean Architecture principles trên mobile](08-clean-architecture-principles-tren-mobile.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [SPM modularization và dependency direction](10-spm-modularization-va-dependency-direction.md) dùng contract của chapter này làm building block.
...
UI → application policy → repository boundary → remote/local implementation
                 ↓
Focus: State machine thay cho Boolean explosion
                 ↓
Evidence: transition log, exhaustive tests và idempotency metric
```
...
Trọng tâm của **State machine thay cho Boolean explosion** là: state machine chỉ cho phép transition hợp lệ giữa các state loại trừ nhau. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng transition log, exhaustive tests và idempotency metric.
...
Áp dụng khi dùng enum state + event/reducer khi workflow có nhiều bước, retry hoặc interruption. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là nhiều Boolean tạo tổ hợp bất khả thi và duplicate side effect. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa State machine thay cho Boolean explosion?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Nhiều boolean tạo tổ hợp bất khả thi và duplicate side effect tạo evidence gì?
...
Architecture không chạy thay framework; nó quyết định nơi state/work/lifetime được sở hữu và quan sát. Với **State machine thay cho Boolean explosion**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **State machine thay cho Boolean explosion** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy nhiều Boolean tạo tổ hợp bất khả thi và duplicate side effect.
...
Dùng transition log, exhaustive tests và idempotency metric; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **dùng enum state + event/reducer khi workflow có nhiều bước, retry hoặc interruption**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **transition log, exhaustive tests và idempotency metric**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **State machine thay cho Boolean explosion** là code an toàn.
>
...
- Nhiều boolean tạo tổ hợp bất khả thi và duplicate side effect → behavior chỉ sai ở scale/lifecycle edge.
...
**Hỏi:** State machine thay cho Boolean explosion giải quyết vấn đề gì?

**30-second:** State machine chỉ cho phép transition hợp lệ giữa các state loại trừ nhau. Chọn nó khi dùng enum state + event/reducer khi workflow có nhiều bước, retry hoặc interruption; rủi ro chính là nhiều Boolean tạo tổ hợp bất khả thi và duplicate side effect.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao nhiều Boolean tạo tổ hợp bất khả thi và duplicate side effect phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **transition log, exhaustive tests và idempotency metric**.
...
Viết một ví dụ nhỏ minh họa **State machine thay cho Boolean explosion** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **nhiều Boolean tạo tổ hợp bất khả thi và duplicate side effect** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **State machine thay cho Boolean explosion**. Thu evidence bằng transition log, exhaustive tests và idempotency metric, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → state machine chỉ cho phép transition hợp lệ giữa các state loại trừ nhau
Use when  → dùng enum state + event/reducer khi workflow có nhiều bước, retry hoặc interruption
Risk      → nhiều Boolean tạo tổ hợp bất khả thi và duplicate side effect
Evidence  → transition log, exhaustive tests và idempotency metric
...
3. Usage rule: dùng enum state + event/reducer khi workflow có nhiều bước, retry hoặc interruption.
4. Mistake nguy hiểm: nhiều Boolean tạo tổ hợp bất khả thi và duplicate side effect.
5. Production lesson: kết luận bằng transition log, exhaustive tests và idempotency metric, rồi bảo vệ bằng test và metric.
...
- [SPM modularization và dependency direction](10-spm-modularization-va-dependency-direction.md)
