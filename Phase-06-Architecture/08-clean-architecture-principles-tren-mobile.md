---
title: "Clean Architecture principles trên mobile"
phase: "Architecture"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "clean-architecture-principles-tren-mobile"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi Checkout phải sửa ViewController, networking, cache và analytics cùng lúc vì responsibility không có ranh giới. Chapter này tập trung vào **Clean Architecture principles trên mobile**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng ARC quản lý lifetime qua strong references trong ownership graph;
- nhận diện failure mode chính: đếm reference thủ công trong đầu hoặc dùng weak để chữa symptom;
- chọn giải pháp bằng rule: loại bỏ strong edge sai hoặc định nghĩa owner đúng;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng Memory Graph + deinit expectation;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [UseCase/Interactor: khi nào đáng thêm một layer](07-usecase-interactor-khi-nao-ang-them-mot-layer.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [State machine thay cho Boolean explosion](09-state-machine-thay-cho-boolean-explosion.md) dùng contract của chapter này làm building block.
...
UI → application policy → repository boundary → remote/local implementation
                 ↓
Focus: Clean Architecture principles trên mobile
                 ↓
Evidence: Memory Graph + deinit expectation
```
...
Trọng tâm của **Clean Architecture principles trên mobile** là: ARC quản lý lifetime qua strong references trong ownership graph. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng Memory Graph + deinit expectation.
...
Áp dụng khi loại bỏ strong edge sai hoặc định nghĩa owner đúng. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là đếm reference thủ công trong đầu hoặc dùng weak để chữa symptom. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Clean Architecture principles trên mobile?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Đếm reference thủ công trong đầu hoặc dùng weak để chữa symptom tạo evidence gì?
...
Architecture không chạy thay framework; nó quyết định nơi state/work/lifetime được sở hữu và quan sát. Với **Clean Architecture principles trên mobile**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Clean Architecture principles trên mobile** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy đếm reference thủ công trong đầu hoặc dùng weak để chữa symptom.
...
Dùng Memory Graph + deinit expectation; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **loại bỏ strong edge sai hoặc định nghĩa owner đúng**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **Memory Graph + deinit expectation**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Clean Architecture principles trên mobile** là code an toàn.
>
...
- Đếm reference thủ công trong đầu hoặc dùng weak để chữa symptom → behavior chỉ sai ở scale/lifecycle edge.
...
- Loại bỏ strong edge sai hoặc định nghĩa owner đúng.
...
**Hỏi:** Clean Architecture principles trên mobile giải quyết vấn đề gì?

**30-second:** Arc quản lý lifetime qua strong references trong ownership graph. Chọn nó khi loại bỏ strong edge sai hoặc định nghĩa owner đúng; rủi ro chính là đếm reference thủ công trong đầu hoặc dùng weak để chữa symptom.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao đếm reference thủ công trong đầu hoặc dùng weak để chữa symptom phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **Memory Graph + deinit expectation**.
...
Viết một ví dụ nhỏ minh họa **Clean Architecture principles trên mobile** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **đếm reference thủ công trong đầu hoặc dùng weak để chữa symptom** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Clean Architecture principles trên mobile**. Thu evidence bằng Memory Graph + deinit expectation, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → ARC quản lý lifetime qua strong references trong ownership graph
Use when  → loại bỏ strong edge sai hoặc định nghĩa owner đúng
Risk      → đếm reference thủ công trong đầu hoặc dùng weak để chữa symptom
Evidence  → Memory Graph + deinit expectation
...
3. Usage rule: loại bỏ strong edge sai hoặc định nghĩa owner đúng.
4. Mistake nguy hiểm: đếm reference thủ công trong đầu hoặc dùng weak để chữa symptom.
5. Production lesson: kết luận bằng Memory Graph + deinit expectation, rồi bảo vệ bằng test và metric.
...
- [State machine thay cho Boolean explosion](09-state-machine-thay-cho-boolean-explosion.md)
