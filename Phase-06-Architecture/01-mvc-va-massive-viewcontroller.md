---
title: "MVC và Massive ViewController"
phase: "Architecture"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "mvc-va-massive-viewcontroller"
  - "global-commerce"
---

# MVC và Massive ViewController
...
Trong Global Commerce, một thay đổi Checkout phải sửa ViewController, networking, cache và analytics cùng lúc vì responsibility không có ranh giới. Chapter này tập trung vào **MVC và Massive ViewController**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng MVC không tự gây massive; responsibility và dependency direction mới là vấn đề;
- nhận diện failure mode chính: controller sở hữu networking, mapping, persistence và navigation;
- chọn giải pháp bằng rule: extract theo reason-to-change trước khi chọn pattern mới;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng change-impact/code review và unit-test seam;
...
- [MVVM và ranh giới trách nhiệm ViewModel](02-mvvm-va-ranh-gioi-trach-nhiem-viewmodel.md) dùng contract của chapter này làm building block.
...
UI → application policy → repository boundary → remote/local implementation
                 ↓
Focus: MVC và Massive ViewController
                 ↓
Evidence: change-impact/code review và unit-test seam
```
...
Trọng tâm của **MVC và Massive ViewController** là: MVC không tự gây massive; responsibility và dependency direction mới là vấn đề. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng change-impact/code review và unit-test seam.
...
Áp dụng khi extract theo reason-to-change trước khi chọn pattern mới. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là controller sở hữu networking, mapping, persistence và navigation. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa MVC và Massive ViewController?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Controller sở hữu networking, mapping, persistence và navigation tạo evidence gì?
...
Architecture không chạy thay framework; nó quyết định nơi state/work/lifetime được sở hữu và quan sát. Với **MVC và Massive ViewController**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **MVC và Massive ViewController** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy controller sở hữu networking, mapping, persistence và navigation.
...
Dùng change-impact/code review và unit-test seam; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **extract theo reason-to-change trước khi chọn pattern mới**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **change-impact/code review và unit-test seam**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **MVC và Massive ViewController** là code an toàn.
>
...
- Controller sở hữu networking, mapping, persistence và navigation → behavior chỉ sai ở scale/lifecycle edge.
...
- Extract theo reason-to-change trước khi chọn pattern mới.
...
**Hỏi:** MVC và Massive ViewController giải quyết vấn đề gì?

**30-second:** Mvc không tự gây massive; responsibility và dependency direction mới là vấn đề. Chọn nó khi extract theo reason-to-change trước khi chọn pattern mới; rủi ro chính là controller sở hữu networking, mapping, persistence và navigation.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao controller sở hữu networking, mapping, persistence và navigation phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **change-impact/code review và unit-test seam**.
...
Viết một ví dụ nhỏ minh họa **MVC và Massive ViewController** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **controller sở hữu networking, mapping, persistence và navigation** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **MVC và Massive ViewController**. Thu evidence bằng change-impact/code review và unit-test seam, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → MVC không tự gây massive; responsibility và dependency direction mới là vấn đề
Use when  → extract theo reason-to-change trước khi chọn pattern mới
Risk      → controller sở hữu networking, mapping, persistence và navigation
Evidence  → change-impact/code review và unit-test seam
...
3. Usage rule: extract theo reason-to-change trước khi chọn pattern mới.
4. Mistake nguy hiểm: controller sở hữu networking, mapping, persistence và navigation.
5. Production lesson: kết luận bằng change-impact/code review và unit-test seam, rồi bảo vệ bằng test và metric.
