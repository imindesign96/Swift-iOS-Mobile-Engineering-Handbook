---
title: "Dependency Injection tạo testability"
phase: "Testing"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "dependency-injection-tao-testability"
  - "global-commerce"
---
...
Trong Global Commerce, test suite xanh nhưng Checkout vẫn lỗi production vì chỉ test implementation detail và bỏ qua boundary quan trọng. Chapter này tập trung vào **Dependency Injection tạo testability**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Mock, Stub, Fake và Spy](03-mock-stub-fake-va-spy.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Async test không sleep](05-async-test-khong-sleep.md) dùng contract của chapter này làm building block.
...
Risk → observable behavior → controlled dependency → assertion/evidence → regression signal
                 ↓
Focus: Dependency Injection tạo testability
                 ↓
...
Trọng tâm của **Dependency Injection tạo testability** là: constructor injection làm dependency/lifetime bắt buộc và nhìn thấy. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Dependency Injection tạo testability?
...
Test runner và concurrency scheduler vẫn có timing; test không được dựa vào sleep hoặc order ngẫu nhiên. Với **Dependency Injection tạo testability**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Dependency Injection tạo testability** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Dependency Injection tạo testability** là code an toàn.
>
...
**Hỏi:** Dependency Injection tạo testability giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Dependency Injection tạo testability** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Dependency Injection tạo testability**. Thu evidence bằng composition-root review và test replacement, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Mock, Stub, Fake và Spy](03-mock-stub-fake-va-spy.md)
