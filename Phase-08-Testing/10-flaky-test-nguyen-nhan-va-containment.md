---
title: "Flaky test: nguyên nhân và containment"
phase: "Testing"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "flaky-test-nguyen-nhan-va-containment"
  - "global-commerce"
---
...
Trong Global Commerce, test suite xanh nhưng Checkout vẫn lỗi production vì chỉ test implementation detail và bỏ qua boundary quan trọng. Chapter này tập trung vào **Flaky test: nguyên nhân và containment**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng async test chờ condition có nghĩa và kiểm soát clock/dependency;
- nhận diện failure mode chính: sleep hoặc shared global state;
- chọn giải pháp bằng rule: inject clock/transport và await deterministic signal;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng rerun/seed/timing artifact;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [UI test cho critical user flow](09-ui-test-cho-critical-user-flow.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Snapshot testing: value và brittleness](11-snapshot-testing-value-va-brittleness.md) dùng contract của chapter này làm building block.
...
Risk → observable behavior → controlled dependency → assertion/evidence → regression signal
                 ↓
Focus: Flaky test: nguyên nhân và containment
                 ↓
Evidence: rerun/seed/timing artifact
```
...
Trọng tâm của **Flaky test: nguyên nhân và containment** là: async test chờ condition có nghĩa và kiểm soát clock/dependency. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng rerun/seed/timing artifact.
...
Áp dụng khi inject clock/transport và await deterministic signal. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là sleep hoặc shared global state. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Flaky test: nguyên nhân và containment?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Sleep hoặc shared global state tạo evidence gì?
...
Test runner và concurrency scheduler vẫn có timing; test không được dựa vào sleep hoặc order ngẫu nhiên. Với **Flaky test: nguyên nhân và containment**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Flaky test: nguyên nhân và containment** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy sleep hoặc shared global state.
...
Dùng rerun/seed/timing artifact; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **inject clock/transport và await deterministic signal**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **rerun/seed/timing artifact**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Flaky test: nguyên nhân và containment** là code an toàn.
>
...
- Sleep hoặc shared global state → behavior chỉ sai ở scale/lifecycle edge.
...
- Inject clock/transport và await deterministic signal.
...
**Hỏi:** Flaky test: nguyên nhân và containment giải quyết vấn đề gì?

**30-second:** Async test chờ condition có nghĩa và kiểm soát clock/dependency. Chọn nó khi inject clock/transport và await deterministic signal; rủi ro chính là sleep hoặc shared global state.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao sleep hoặc shared global state phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **rerun/seed/timing artifact**.
...
Viết một ví dụ nhỏ minh họa **Flaky test: nguyên nhân và containment** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **sleep hoặc shared global state** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Flaky test: nguyên nhân và containment**. Thu evidence bằng rerun/seed/timing artifact, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → async test chờ condition có nghĩa và kiểm soát clock/dependency
Use when  → inject clock/transport và await deterministic signal
Risk      → sleep hoặc shared global state
Evidence  → rerun/seed/timing artifact
...
3. Usage rule: inject clock/transport và await deterministic signal.
4. Mistake nguy hiểm: sleep hoặc shared global state.
5. Production lesson: kết luận bằng rerun/seed/timing artifact, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [UI test cho critical user flow](09-ui-test-cho-critical-user-flow.md)
