...
Trong Global Commerce, test suite xanh nhưng Checkout vẫn lỗi production vì chỉ test implementation detail và bỏ qua boundary quan trọng. Chapter này tập trung vào **TDD và design feedback**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Snapshot testing: value và brittleness](11-snapshot-testing-value-va-brittleness.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Code coverage: tín hiệu và giới hạn](13-code-coverage-tin-hieu-va-gioi-han.md) dùng contract của chapter này làm building block.
...
Trọng tâm của **TDD và design feedback** là: coverage là tín hiệu executed lines, không chứng minh assertion/risk. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa TDD và design feedback?
...
Test runner và concurrency scheduler vẫn có timing; test không được dựa vào sleep hoặc order ngẫu nhiên. Với **TDD và design feedback**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **TDD và design feedback** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **TDD và design feedback** là code an toàn.
>
...
**Hỏi:** TDD và design feedback giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **TDD và design feedback** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **TDD và design feedback**. Thu evidence bằng mutation review, failure usefulness và review cost, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Snapshot testing: value và brittleness](11-snapshot-testing-value-va-brittleness.md)
