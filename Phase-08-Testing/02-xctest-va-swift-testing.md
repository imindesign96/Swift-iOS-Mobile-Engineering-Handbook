...
Trong Global Commerce, test suite xanh nhưng Checkout vẫn lỗi production vì chỉ test implementation detail và bỏ qua boundary quan trọng. Chapter này tập trung vào **XCTest và Swift Testing**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Test strategy và test pyramid trên iOS](01-test-strategy-va-test-pyramid-tren-ios.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Mock, Stub, Fake và Spy](03-mock-stub-fake-va-spy.md) dùng contract của chapter này làm building block.
...
Trọng tâm của **XCTest và Swift Testing** là: test level được chọn theo risk, feedback speed và boundary. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa XCTest và Swift Testing?
...
Test runner và concurrency scheduler vẫn có timing; test không được dựa vào sleep hoặc order ngẫu nhiên. Với **XCTest và Swift Testing**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **XCTest và Swift Testing** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **XCTest và Swift Testing** là code an toàn.
>
...
**Hỏi:** XCTest và Swift Testing giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **XCTest và Swift Testing** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **XCTest và Swift Testing**. Thu evidence bằng failure diagnostic và flake rate, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Test strategy và test pyramid trên iOS](01-test-strategy-va-test-pyramid-tren-ios.md)
