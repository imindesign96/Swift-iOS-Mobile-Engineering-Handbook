---
title: "Networking test với URLProtocol/fake transport"
phase: "Testing"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "networking-test-voi-urlprotocol-fake-transport"
  - "global-commerce"
---
...
Trong Global Commerce, test suite xanh nhưng Checkout vẫn lỗi production vì chỉ test implementation detail và bỏ qua boundary quan trọng. Chapter này tập trung vào **Networking test với URLProtocol/fake transport**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng protocol mô tả capability/contract; generic giữ type relationship tại compile time;
- nhận diện failure mode chính: abstraction không có nhiều implementation hoặc existential xóa thông tin cần thiết;
- chọn giải pháp bằng rule: thêm protocol tại boundary volatility/test seam, không cho mọi concrete type;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng call-site substitutability và compile-time constraints;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [ViewModel testing theo state transition](06-viewmodel-testing-theo-state-transition.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Repository integration test](08-repository-integration-test.md) dùng contract của chapter này làm building block.
...
Risk → observable behavior → controlled dependency → assertion/evidence → regression signal
                 ↓
Focus: Networking test với URLProtocol/fake transport
                 ↓
Evidence: call-site substitutability và compile-time constraints
```
...
Trọng tâm của **Networking test với URLProtocol/fake transport** là: protocol mô tả capability/contract; generic giữ type relationship tại compile time. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng call-site substitutability và compile-time constraints.
...
Áp dụng khi thêm protocol tại boundary volatility/test seam, không cho mọi concrete type. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là abstraction không có nhiều implementation hoặc existential xóa thông tin cần thiết. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Networking test với URLProtocol/fake transport?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Abstraction không có nhiều implementation hoặc existential xóa thông tin cần thiết tạo evidence gì?
...
Test runner và concurrency scheduler vẫn có timing; test không được dựa vào sleep hoặc order ngẫu nhiên. Với **Networking test với URLProtocol/fake transport**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Networking test với URLProtocol/fake transport** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy abstraction không có nhiều implementation hoặc existential xóa thông tin cần thiết.
...
Dùng call-site substitutability và compile-time constraints; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **thêm protocol tại boundary volatility/test seam, không cho mọi concrete type**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **call-site substitutability và compile-time constraints**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Networking test với URLProtocol/fake transport** là code an toàn.
>
...
- Abstraction không có nhiều implementation hoặc existential xóa thông tin cần thiết → behavior chỉ sai ở scale/lifecycle edge.
...
- Thêm protocol tại boundary volatility/test seam, không cho mọi concrete type.
...
**Hỏi:** Networking test với URLProtocol/fake transport giải quyết vấn đề gì?

**30-second:** Protocol mô tả capability/contract; generic giữ type relationship tại compile time. Chọn nó khi thêm protocol tại boundary volatility/test seam, không cho mọi concrete type; rủi ro chính là abstraction không có nhiều implementation hoặc existential xóa thông tin cần thiết.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao abstraction không có nhiều implementation hoặc existential xóa thông tin cần thiết phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **call-site substitutability và compile-time constraints**.
...
Viết một ví dụ nhỏ minh họa **Networking test với URLProtocol/fake transport** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **abstraction không có nhiều implementation hoặc existential xóa thông tin cần thiết** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Networking test với URLProtocol/fake transport**. Thu evidence bằng call-site substitutability và compile-time constraints, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → protocol mô tả capability/contract; generic giữ type relationship tại compile time
Use when  → thêm protocol tại boundary volatility/test seam, không cho mọi concrete type
Risk      → abstraction không có nhiều implementation hoặc existential xóa thông tin cần thiết
Evidence  → call-site substitutability và compile-time constraints
...
3. Usage rule: thêm protocol tại boundary volatility/test seam, không cho mọi concrete type.
4. Mistake nguy hiểm: abstraction không có nhiều implementation hoặc existential xóa thông tin cần thiết.
5. Production lesson: kết luận bằng call-site substitutability và compile-time constraints, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [ViewModel testing theo state transition](06-viewmodel-testing-theo-state-transition.md)
- [Repository integration test](08-repository-integration-test.md)
