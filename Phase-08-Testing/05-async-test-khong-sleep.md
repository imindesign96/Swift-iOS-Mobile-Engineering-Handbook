...
Trong Global Commerce, test suite xanh nhưng Checkout vẫn lỗi production vì chỉ test implementation detail và bỏ qua boundary quan trọng. Chapter này tập trung vào **Async test không sleep**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Dependency Injection tạo testability](04-dependency-injection-tao-testability.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [ViewModel testing theo state transition](06-viewmodel-testing-theo-state-transition.md) dùng contract của chapter này làm building block.
...
Trọng tâm của **Async test không sleep** là: async test chờ condition có nghĩa và kiểm soát clock/dependency. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Async test không sleep?
...
Test runner và concurrency scheduler vẫn có timing; test không được dựa vào sleep hoặc order ngẫu nhiên. Với **Async test không sleep**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Async test không sleep** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Async test không sleep** là code an toàn.
>
...
**Hỏi:** Async test không sleep giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Async test không sleep** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Async test không sleep**. Thu evidence bằng rerun/seed/timing artifact, vẽ owner/state graph, xác định root cause và thêm regression test.
