---
title: "Snapshot testing: value và brittleness"
phase: "Testing"
difficulty: 5
importance: 5
...
levels:
  - L5
prerequisites:
  - "Flaky test: nguyên nhân và containment"
used_later:
  - "TDD và design feedback"
competencies:
  - "Testing"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "snapshot-testing-value-va-brittleness"
  - "global-commerce"
---

# Snapshot testing: value và brittleness
...
Trong Global Commerce, test suite xanh nhưng Checkout vẫn lỗi production vì chỉ test implementation detail và bỏ qua boundary quan trọng. Chapter này tập trung vào **Snapshot testing: value và brittleness**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng coverage là tín hiệu executed lines, không chứng minh assertion/risk;
- nhận diện failure mode chính: chạy theo phần trăm hoặc snapshot khổng lồ;
- chọn giải pháp bằng rule: dùng TDD khi feedback thiết kế hữu ích; snapshot cho stable visual contract;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng mutation review, failure usefulness và review cost;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Flaky test: nguyên nhân và containment](10-flaky-test-nguyen-nhan-va-containment.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [TDD và design feedback](12-tdd-va-design-feedback.md) dùng contract của chapter này làm building block.
...
Risk → observable behavior → controlled dependency → assertion/evidence → regression signal
                 ↓
Focus: Snapshot testing: value và brittleness
                 ↓
Evidence: mutation review, failure usefulness và review cost
```
...
Trọng tâm của **Snapshot testing: value và brittleness** là: coverage là tín hiệu executed lines, không chứng minh assertion/risk. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng mutation review, failure usefulness và review cost.
...
Áp dụng khi dùng TDD khi feedback thiết kế hữu ích; snapshot cho stable visual contract. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là chạy theo phần trăm hoặc snapshot khổng lồ. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Snapshot testing: value và brittleness?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Chạy theo phần trăm hoặc snapshot khổng lồ tạo evidence gì?
...
Test runner và concurrency scheduler vẫn có timing; test không được dựa vào sleep hoặc order ngẫu nhiên. Với **Snapshot testing: value và brittleness**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Snapshot testing: value và brittleness** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy chạy theo phần trăm hoặc snapshot khổng lồ.
...
Dùng mutation review, failure usefulness và review cost; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **dùng TDD khi feedback thiết kế hữu ích; snapshot cho stable visual contract**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **mutation review, failure usefulness và review cost**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Snapshot testing: value và brittleness** là code an toàn.
>
...
- Chạy theo phần trăm hoặc snapshot khổng lồ → behavior chỉ sai ở scale/lifecycle edge.
...
- Dùng tdd khi feedback thiết kế hữu ích; snapshot cho stable visual contract.
...
**Hỏi:** Snapshot testing: value và brittleness giải quyết vấn đề gì?

**30-second:** Coverage là tín hiệu executed lines, không chứng minh assertion/risk. Chọn nó khi dùng TDD khi feedback thiết kế hữu ích; snapshot cho stable visual contract; rủi ro chính là chạy theo phần trăm hoặc snapshot khổng lồ.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao chạy theo phần trăm hoặc snapshot khổng lồ phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **mutation review, failure usefulness và review cost**.
...
Viết một ví dụ nhỏ minh họa **Snapshot testing: value và brittleness** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **chạy theo phần trăm hoặc snapshot khổng lồ** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Snapshot testing: value và brittleness**. Thu evidence bằng mutation review, failure usefulness và review cost, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → coverage là tín hiệu executed lines, không chứng minh assertion/risk
Use when  → dùng TDD khi feedback thiết kế hữu ích; snapshot cho stable visual contract
Risk      → chạy theo phần trăm hoặc snapshot khổng lồ
Evidence  → mutation review, failure usefulness và review cost
...
3. Usage rule: dùng TDD khi feedback thiết kế hữu ích; snapshot cho stable visual contract.
4. Mistake nguy hiểm: chạy theo phần trăm hoặc snapshot khổng lồ.
5. Production lesson: kết luận bằng mutation review, failure usefulness và review cost, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Flaky test: nguyên nhân và containment](10-flaky-test-nguyen-nhan-va-containment.md)
- [TDD và design feedback](12-tdd-va-design-feedback.md)
