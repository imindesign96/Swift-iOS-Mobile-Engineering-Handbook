---
title: "Code coverage: tín hiệu và giới hạn"
phase: "Testing"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "code-coverage-tin-hieu-va-gioi-han"
  - "global-commerce"
---
...
Trong Global Commerce, test suite xanh nhưng Checkout vẫn lỗi production vì chỉ test implementation detail và bỏ qua boundary quan trọng. Chapter này tập trung vào **Code coverage: tín hiệu và giới hạn**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [TDD và design feedback](12-tdd-va-design-feedback.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Regression test từ production incident](14-regression-test-tu-production-incident.md) dùng contract của chapter này làm building block.
...
Risk → observable behavior → controlled dependency → assertion/evidence → regression signal
                 ↓
Focus: Code coverage: tín hiệu và giới hạn
                 ↓
...
Trọng tâm của **Code coverage: tín hiệu và giới hạn** là: coverage là tín hiệu executed lines, không chứng minh assertion/risk. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Code coverage: tín hiệu và giới hạn?
...
Test runner và concurrency scheduler vẫn có timing; test không được dựa vào sleep hoặc order ngẫu nhiên. Với **Code coverage: tín hiệu và giới hạn**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Code coverage: tín hiệu và giới hạn** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Code coverage: tín hiệu và giới hạn** là code an toàn.
>
...
**Hỏi:** Code coverage: tín hiệu và giới hạn giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Code coverage: tín hiệu và giới hạn** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Code coverage: tín hiệu và giới hạn**. Thu evidence bằng mutation review, failure usefulness và review cost, vẽ owner/state graph, xác định root cause và thêm regression test.
