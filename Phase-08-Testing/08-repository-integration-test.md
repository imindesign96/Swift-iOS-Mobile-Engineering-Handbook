...
Trong Global Commerce, test suite xanh nhưng Checkout vẫn lỗi production vì chỉ test implementation detail và bỏ qua boundary quan trọng. Chapter này tập trung vào **Repository integration test**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng Repository biểu diễn domain data operations và phối hợp source/policy;
- nhận diện failure mode chính: đổi tên APIClient thành Repository hoặc tạo God Repository;
- chọn giải pháp bằng rule: chia theo aggregate/capability domain;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng contract tests và cache/source metrics;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Networking test với URLProtocol/fake transport](07-networking-test-voi-urlprotocol-fake-transport.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [UI test cho critical user flow](09-ui-test-cho-critical-user-flow.md) dùng contract của chapter này làm building block.
...
Risk → observable behavior → controlled dependency → assertion/evidence → regression signal
                 ↓
Focus: Repository integration test
                 ↓
Evidence: contract tests và cache/source metrics
```
...
Trọng tâm của **Repository integration test** là: Repository biểu diễn domain data operations và phối hợp source/policy. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng contract tests và cache/source metrics.
...
Áp dụng khi chia theo aggregate/capability domain. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là đổi tên APIClient thành Repository hoặc tạo God Repository. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Repository integration test?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Đổi tên apiclient thành repository hoặc tạo god repository tạo evidence gì?
...
Test runner và concurrency scheduler vẫn có timing; test không được dựa vào sleep hoặc order ngẫu nhiên. Với **Repository integration test**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Repository integration test** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy đổi tên APIClient thành Repository hoặc tạo God Repository.
...
Dùng contract tests và cache/source metrics; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **chia theo aggregate/capability domain**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **contract tests và cache/source metrics**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Repository integration test** là code an toàn.
>
...
- Đổi tên apiclient thành repository hoặc tạo god repository → behavior chỉ sai ở scale/lifecycle edge.
...
- Chia theo aggregate/capability domain.
...
**Hỏi:** Repository integration test giải quyết vấn đề gì?

**30-second:** Repository biểu diễn domain data operations và phối hợp source/policy. Chọn nó khi chia theo aggregate/capability domain; rủi ro chính là đổi tên APIClient thành Repository hoặc tạo God Repository.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao đổi tên APIClient thành Repository hoặc tạo God Repository phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **contract tests và cache/source metrics**.
...
Viết một ví dụ nhỏ minh họa **Repository integration test** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **đổi tên APIClient thành Repository hoặc tạo God Repository** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Repository integration test**. Thu evidence bằng contract tests và cache/source metrics, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → Repository biểu diễn domain data operations và phối hợp source/policy
Use when  → chia theo aggregate/capability domain
Risk      → đổi tên APIClient thành Repository hoặc tạo God Repository
Evidence  → contract tests và cache/source metrics
...
3. Usage rule: chia theo aggregate/capability domain.
4. Mistake nguy hiểm: đổi tên APIClient thành Repository hoặc tạo God Repository.
5. Production lesson: kết luận bằng contract tests và cache/source metrics, rồi bảo vệ bằng test và metric.
