---
title: "Stack/Heap: mental model hữu ích và giới hạn"
...
competencies:
  ...
  - "Interview"
tags:
  - "stack-heap-mental-model-huu-ich-va-gioi-han"
  - "global-commerce"
---
...
Trong Global Commerce, Product Detail đã đóng nhưng memory vẫn tăng sau mỗi lần mở vì lifetime thực tế khác lifetime mong đợi. Chapter này tập trung vào **Stack/Heap: mental model hữu ích và giới hạn**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng stack/heap là mental model về storage/lifetime, không phải luật struct/class tuyệt đối;
- nhận diện failure mode chính: khẳng định mọi struct ở stack và mọi class ở heap;
- chọn giải pháp bằng rule: dùng model để hỏi lifetime/copy, không đoán placement được tối ưu;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng Memory Graph/Allocations và optimized build;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Glossary](../GLOSSARY.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Copy, value semantics và mutation](02-copy-value-semantics-va-mutation.md) dùng contract của chapter này làm building block.
...
Creator → strong ownership graph → release edges → reference count về 0 → deinit
                 ↓
Focus: Stack/Heap: mental model hữu ích và giới hạn
                 ↓
Evidence: Memory Graph/Allocations và optimized build
```
...
Trọng tâm của **Stack/Heap: mental model hữu ích và giới hạn** là: stack/heap là mental model về storage/lifetime, không phải luật struct/class tuyệt đối. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng Memory Graph/Allocations và optimized build.
...
Áp dụng khi dùng model để hỏi lifetime/copy, không đoán placement được tối ưu. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là khẳng định mọi struct ở stack và mọi class ở heap. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Stack/Heap: mental model hữu ích và giới hạn?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Khẳng định mọi struct ở stack và mọi class ở heap tạo evidence gì?
...
ARC chèn retain/release theo semantics của chương trình; graph ownership, không phải một dòng weak, quyết định lifetime. Với **Stack/Heap: mental model hữu ích và giới hạn**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Stack/Heap: mental model hữu ích và giới hạn** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy khẳng định mọi struct ở stack và mọi class ở heap.
...
Dùng Memory Graph/Allocations và optimized build; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **dùng model để hỏi lifetime/copy, không đoán placement được tối ưu**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **Memory Graph/Allocations và optimized build**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Stack/Heap: mental model hữu ích và giới hạn** là code an toàn.
>
...
- Khẳng định mọi struct ở stack và mọi class ở heap → behavior chỉ sai ở scale/lifecycle edge.
...
- Dùng model để hỏi lifetime/copy, không đoán placement được tối ưu.
...
**Hỏi:** Stack/Heap: mental model hữu ích và giới hạn giải quyết vấn đề gì?

**30-second:** Stack/heap là mental model về storage/lifetime, không phải luật struct/class tuyệt đối. Chọn nó khi dùng model để hỏi lifetime/copy, không đoán placement được tối ưu; rủi ro chính là khẳng định mọi struct ở stack và mọi class ở heap.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao khẳng định mọi struct ở stack và mọi class ở heap phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **Memory Graph/Allocations và optimized build**.
...
Viết một ví dụ nhỏ minh họa **Stack/Heap: mental model hữu ích và giới hạn** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **khẳng định mọi struct ở stack và mọi class ở heap** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Stack/Heap: mental model hữu ích và giới hạn**. Thu evidence bằng Memory Graph/Allocations và optimized build, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → stack/heap là mental model về storage/lifetime, không phải luật struct/class tuyệt đối
Use when  → dùng model để hỏi lifetime/copy, không đoán placement được tối ưu
Risk      → khẳng định mọi struct ở stack và mọi class ở heap
Evidence  → Memory Graph/Allocations và optimized build
...
3. Usage rule: dùng model để hỏi lifetime/copy, không đoán placement được tối ưu.
4. Mistake nguy hiểm: khẳng định mọi struct ở stack và mọi class ở heap.
5. Production lesson: kết luận bằng Memory Graph/Allocations và optimized build, rồi bảo vệ bằng test và metric.
