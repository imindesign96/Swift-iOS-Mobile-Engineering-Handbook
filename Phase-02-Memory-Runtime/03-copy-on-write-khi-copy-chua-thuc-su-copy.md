---
title: "Copy-on-Write: khi copy chưa thực sự copy"
...
competencies:
  ...
  - "Interview"
tags:
  - "copy-on-write-khi-copy-chua-thuc-su-copy"
  - "global-commerce"
---
...
Trong Global Commerce, Product Detail đã đóng nhưng memory vẫn tăng sau mỗi lần mở vì lifetime thực tế khác lifetime mong đợi. Chapter này tập trung vào **Copy-on-Write: khi copy chưa thực sự copy**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Copy, value semantics và mutation](02-copy-value-semantics-va-mutation.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [ARC và ownership graph](04-arc-va-ownership-graph.md) dùng contract của chapter này làm building block.
...
Creator → strong ownership graph → release edges → reference count về 0 → deinit
                 ↓
Focus: Copy-on-Write: khi copy chưa thực sự copy
                 ↓
...
Trọng tâm của **Copy-on-Write: khi copy chưa thực sự copy** là: value mutation không làm thay đổi value độc lập; reference chia sẻ identity. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Copy-on-Write: khi copy chưa thực sự copy?
...
ARC chèn retain/release theo semantics của chương trình; graph ownership, không phải một dòng weak, quyết định lifetime. Với **Copy-on-Write: khi copy chưa thực sự copy**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Copy-on-Write: khi copy chưa thực sự copy** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Copy-on-Write: khi copy chưa thực sự copy** là code an toàn.
>
...
**Hỏi:** Copy-on-Write: khi copy chưa thực sự copy giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Copy-on-Write: khi copy chưa thực sự copy** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Copy-on-Write: khi copy chưa thực sự copy**. Thu evidence bằng mutation isolation test và profiling khi copy cost đáng kể, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Copy, value semantics và mutation](02-copy-value-semantics-va-mutation.md)
- [ARC và ownership graph](04-arc-va-ownership-graph.md)
