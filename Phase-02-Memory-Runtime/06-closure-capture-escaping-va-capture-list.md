---
title: "Closure capture, escaping và capture list"
...
levels:
  - L2
prerequisites:
  - "`strong`, `weak`, `unowned` theo lifetime"
used_later:
  - "Vì sao ViewController không `deinit`?"
competencies:
  - "Memory & Runtime"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "closure-capture-escaping-va-capture-list"
  - "global-commerce"
---

# Closure capture, escaping và capture list
...
Trong Global Commerce, Product Detail đã đóng nhưng memory vẫn tăng sau mỗi lần mở vì lifetime thực tế khác lifetime mong đợi. Chapter này tập trung vào **Closure capture, escaping và capture list**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng closure là behavior value có capture và lifetime;
- nhận diện failure mode chính: capture mạnh kéo dài owner hoặc callback gọi nhiều lần ngoài dự kiến;
- chọn giải pháp bằng rule: chọn capture list từ lifetime contract, không dùng weak theo phản xạ;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng ownership graph và thời điểm closure được release;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [`strong`, `weak`, `unowned` theo lifetime](05-strong-weak-unowned-theo-lifetime.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Vì sao ViewController không `deinit`?](07-vi-sao-viewcontroller-khong-deinit.md) dùng contract của chapter này làm building block.
...
Creator → strong ownership graph → release edges → reference count về 0 → deinit
                 ↓
Focus: Closure capture, escaping và capture list
                 ↓
Evidence: ownership graph và thời điểm closure được release
```
...
Trọng tâm của **Closure capture, escaping và capture list** là: closure là behavior value có capture và lifetime. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng ownership graph và thời điểm closure được release.
...
Áp dụng khi chọn capture list từ lifetime contract, không dùng weak theo phản xạ. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là capture mạnh kéo dài owner hoặc callback gọi nhiều lần ngoài dự kiến. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Closure capture, escaping và capture list?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Capture mạnh kéo dài owner hoặc callback gọi nhiều lần ngoài dự kiến tạo evidence gì?
...
ARC chèn retain/release theo semantics của chương trình; graph ownership, không phải một dòng weak, quyết định lifetime. Với **Closure capture, escaping và capture list**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Closure capture, escaping và capture list** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy capture mạnh kéo dài owner hoặc callback gọi nhiều lần ngoài dự kiến.
...
Dùng ownership graph và thời điểm closure được release; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **chọn capture list từ lifetime contract, không dùng weak theo phản xạ**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **ownership graph và thời điểm closure được release**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Closure capture, escaping và capture list** là code an toàn.
>
...
- Capture mạnh kéo dài owner hoặc callback gọi nhiều lần ngoài dự kiến → behavior chỉ sai ở scale/lifecycle edge.
...
- Chọn capture list từ lifetime contract, không dùng weak theo phản xạ.
...
**Hỏi:** Closure capture, escaping và capture list giải quyết vấn đề gì?

**30-second:** Closure là behavior value có capture và lifetime. Chọn nó khi chọn capture list từ lifetime contract, không dùng weak theo phản xạ; rủi ro chính là capture mạnh kéo dài owner hoặc callback gọi nhiều lần ngoài dự kiến.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao capture mạnh kéo dài owner hoặc callback gọi nhiều lần ngoài dự kiến phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **ownership graph và thời điểm closure được release**.
...
Viết một ví dụ nhỏ minh họa **Closure capture, escaping và capture list** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **capture mạnh kéo dài owner hoặc callback gọi nhiều lần ngoài dự kiến** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Closure capture, escaping và capture list**. Thu evidence bằng ownership graph và thời điểm closure được release, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → closure là behavior value có capture và lifetime
Use when  → chọn capture list từ lifetime contract, không dùng weak theo phản xạ
Risk      → capture mạnh kéo dài owner hoặc callback gọi nhiều lần ngoài dự kiến
Evidence  → ownership graph và thời điểm closure được release
...
3. Usage rule: chọn capture list từ lifetime contract, không dùng weak theo phản xạ.
4. Mistake nguy hiểm: capture mạnh kéo dài owner hoặc callback gọi nhiều lần ngoài dự kiến.
5. Production lesson: kết luận bằng ownership graph và thời điểm closure được release, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [`strong`, `weak`, `unowned` theo lifetime](05-strong-weak-unowned-theo-lifetime.md)
- [Vì sao ViewController không `deinit`?](07-vi-sao-viewcontroller-khong-deinit.md)
