---
title: "Copy, value semantics và mutation"
...
levels:
  - L2
prerequisites:
  - "Stack/Heap: mental model hữu ích và giới hạn"
used_later:
  - "Copy-on-Write: khi copy chưa thực sự copy"
competencies:
  - "Memory & Runtime"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "copy-value-semantics-va-mutation"
  - "global-commerce"
---

# Copy, value semantics và mutation
...
Trong Global Commerce, Product Detail đã đóng nhưng memory vẫn tăng sau mỗi lần mở vì lifetime thực tế khác lifetime mong đợi. Chapter này tập trung vào **Copy, value semantics và mutation**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng value mutation không làm thay đổi value độc lập; reference chia sẻ identity;
- nhận diện failure mode chính: tưởng assignment luôn deep-copy hoặc tưởng COW là reference semantics;
- chọn giải pháp bằng rule: dùng semantics giúp invariant và concurrency reasoning rõ hơn;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng mutation isolation test và profiling khi copy cost đáng kể;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Stack/Heap: mental model hữu ích và giới hạn](01-stack-heap-mental-model-huu-ich-va-gioi-han.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Copy-on-Write: khi copy chưa thực sự copy](03-copy-on-write-khi-copy-chua-thuc-su-copy.md) dùng contract của chapter này làm building block.
...
Creator → strong ownership graph → release edges → reference count về 0 → deinit
                 ↓
Focus: Copy, value semantics và mutation
                 ↓
Evidence: mutation isolation test và profiling khi copy cost đáng kể
```
...
Trọng tâm của **Copy, value semantics và mutation** là: value mutation không làm thay đổi value độc lập; reference chia sẻ identity. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
Concept tồn tại để giúp lý luận object graph bằng creator, owner, release và expected deinit. Không có boundary này, rule domain bị nhân bản, behavior phụ thuộc call order và incident khó điều tra vì không biết layer nào chịu trách nhiệm.
...
5. Đo observable behavior bằng mutation isolation test và profiling khi copy cost đáng kể.
...
Áp dụng khi dùng semantics giúp invariant và concurrency reasoning rõ hơn. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là tưởng assignment luôn deep-copy hoặc tưởng COW là reference semantics. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Copy, value semantics và mutation?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Tưởng assignment luôn deep-copy hoặc tưởng cow là reference semantics tạo evidence gì?
...
ARC chèn retain/release theo semantics của chương trình; graph ownership, không phải một dòng weak, quyết định lifetime. Với **Copy, value semantics và mutation**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Vẽ graph strong/weak/unowned và phân biệt leak với legitimate memory pressure.
...
Task, callback và actor có thể kéo dài lifetime; cancellation không đồng nghĩa object được giải phóng ngay.
...
Boundary delegate/coordinator/cache phải có ownership contract, đặc biệt với screen-scoped dependency. Dependency hướng vào contract ổn định; implementation details nằm phía ngoài. Composition root tạo concrete dependencies và quyết định lifetime.
...
Feature Commerce áp dụng **Copy, value semantics và mutation** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy tưởng assignment luôn deep-copy hoặc tưởng COW là reference semantics.
...
Dùng mutation isolation test và profiling khi copy cost đáng kể; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **dùng semantics giúp invariant và concurrency reasoning rõ hơn**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **mutation isolation test và profiling khi copy cost đáng kể**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Copy, value semantics và mutation** là code an toàn.
>
...
- Tưởng assignment luôn deep-copy hoặc tưởng cow là reference semantics → behavior chỉ sai ở scale/lifecycle edge.
...
- Dùng semantics giúp invariant và concurrency reasoning rõ hơn.
...
**Hỏi:** Copy, value semantics và mutation giải quyết vấn đề gì?

**30-second:** Value mutation không làm thay đổi value độc lập; reference chia sẻ identity. Chọn nó khi dùng semantics giúp invariant và concurrency reasoning rõ hơn; rủi ro chính là tưởng assignment luôn deep-copy hoặc tưởng COW là reference semantics.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao tưởng assignment luôn deep-copy hoặc tưởng COW là reference semantics phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **mutation isolation test và profiling khi copy cost đáng kể**.
...
Viết một ví dụ nhỏ minh họa **Copy, value semantics và mutation** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **tưởng assignment luôn deep-copy hoặc tưởng COW là reference semantics** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Copy, value semantics và mutation**. Thu evidence bằng mutation isolation test và profiling khi copy cost đáng kể, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → value mutation không làm thay đổi value độc lập; reference chia sẻ identity
Use when  → dùng semantics giúp invariant và concurrency reasoning rõ hơn
Risk      → tưởng assignment luôn deep-copy hoặc tưởng COW là reference semantics
Evidence  → mutation isolation test và profiling khi copy cost đáng kể
...
2. Mental model: Creator → strong ownership graph → release edges → reference count về 0 → deinit.
3. Usage rule: dùng semantics giúp invariant và concurrency reasoning rõ hơn.
4. Mistake nguy hiểm: tưởng assignment luôn deep-copy hoặc tưởng COW là reference semantics.
5. Production lesson: kết luận bằng mutation isolation test và profiling khi copy cost đáng kể, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Stack/Heap: mental model hữu ích và giới hạn](01-stack-heap-mental-model-huu-ich-va-gioi-han.md)
- [Copy-on-Write: khi copy chưa thực sự copy](03-copy-on-write-khi-copy-chua-thuc-su-copy.md)
