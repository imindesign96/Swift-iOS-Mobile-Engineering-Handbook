...
Trong Global Commerce, Product Detail đã đóng nhưng memory vẫn tăng sau mỗi lần mở vì lifetime thực tế khác lifetime mong đợi. Chapter này tập trung vào **ARC và ownership graph**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Copy-on-Write: khi copy chưa thực sự copy](03-copy-on-write-khi-copy-chua-thuc-su-copy.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [`strong`, `weak`, `unowned` theo lifetime](05-strong-weak-unowned-theo-lifetime.md) dùng contract của chapter này làm building block.
...
Trọng tâm của **ARC và ownership graph** là: ARC quản lý lifetime qua strong references trong ownership graph. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa ARC và ownership graph?
...
ARC chèn retain/release theo semantics của chương trình; graph ownership, không phải một dòng weak, quyết định lifetime. Với **ARC và ownership graph**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **ARC và ownership graph** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **ARC và ownership graph** là code an toàn.
>
...
**Hỏi:** ARC và ownership graph giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **ARC và ownership graph** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **ARC và ownership graph**. Thu evidence bằng Memory Graph + deinit expectation, vẽ owner/state graph, xác định root cause và thêm regression test.
