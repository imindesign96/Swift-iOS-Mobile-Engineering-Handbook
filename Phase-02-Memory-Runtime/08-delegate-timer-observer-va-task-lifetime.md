---
title: "Delegate, timer, observer và task lifetime"
...
used_later:
  - "`deinit` và lifecycle diagnostics"
competencies:
  - "Memory & Runtime"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "delegate-timer-observer-va-task-lifetime"
  - "global-commerce"
---

# Delegate, timer, observer và task lifetime
...
Trong Global Commerce, Product Detail đã đóng nhưng memory vẫn tăng sau mỗi lần mở vì lifetime thực tế khác lifetime mong đợi. Chapter này tập trung vào **Delegate, timer, observer và task lifetime**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng registration/work tạo lifetime edge cần cleanup contract;
- nhận diện failure mode chính: observer/timer/task sống lâu hơn feature;
- chọn giải pháp bằng rule: owner tạo resource cũng chịu trách nhiệm stop/cancel/remove;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng deinit probe, cancellation log và path to root;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Vì sao ViewController không `deinit`?](07-vi-sao-viewcontroller-khong-deinit.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [`deinit` và lifecycle diagnostics](09-deinit-va-lifecycle-diagnostics.md) dùng contract của chapter này làm building block.
...
Creator → strong ownership graph → release edges → reference count về 0 → deinit
                 ↓
Focus: Delegate, timer, observer và task lifetime
                 ↓
Evidence: deinit probe, cancellation log và path to root
```
...
Trọng tâm của **Delegate, timer, observer và task lifetime** là: registration/work tạo lifetime edge cần cleanup contract. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng deinit probe, cancellation log và path to root.
...
Áp dụng khi owner tạo resource cũng chịu trách nhiệm stop/cancel/remove. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là observer/timer/task sống lâu hơn feature. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Delegate, timer, observer và task lifetime?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Observer/timer/task sống lâu hơn feature tạo evidence gì?
...
ARC chèn retain/release theo semantics của chương trình; graph ownership, không phải một dòng weak, quyết định lifetime. Với **Delegate, timer, observer và task lifetime**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Delegate, timer, observer và task lifetime** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy observer/timer/task sống lâu hơn feature.
...
Dùng deinit probe, cancellation log và path to root; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **owner tạo resource cũng chịu trách nhiệm stop/cancel/remove**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **deinit probe, cancellation log và path to root**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Delegate, timer, observer và task lifetime** là code an toàn.
>
...
- Observer/timer/task sống lâu hơn feature → behavior chỉ sai ở scale/lifecycle edge.
...
- Owner tạo resource cũng chịu trách nhiệm stop/cancel/remove.
...
**Hỏi:** Delegate, timer, observer và task lifetime giải quyết vấn đề gì?

**30-second:** Registration/work tạo lifetime edge cần cleanup contract. Chọn nó khi owner tạo resource cũng chịu trách nhiệm stop/cancel/remove; rủi ro chính là observer/timer/task sống lâu hơn feature.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao observer/timer/task sống lâu hơn feature phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **deinit probe, cancellation log và path to root**.
...
Viết một ví dụ nhỏ minh họa **Delegate, timer, observer và task lifetime** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **observer/timer/task sống lâu hơn feature** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Delegate, timer, observer và task lifetime**. Thu evidence bằng deinit probe, cancellation log và path to root, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → registration/work tạo lifetime edge cần cleanup contract
Use when  → owner tạo resource cũng chịu trách nhiệm stop/cancel/remove
Risk      → observer/timer/task sống lâu hơn feature
Evidence  → deinit probe, cancellation log và path to root
...
3. Usage rule: owner tạo resource cũng chịu trách nhiệm stop/cancel/remove.
4. Mistake nguy hiểm: observer/timer/task sống lâu hơn feature.
5. Production lesson: kết luận bằng deinit probe, cancellation log và path to root, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Vì sao ViewController không `deinit`?](07-vi-sao-viewcontroller-khong-deinit.md)
- [`deinit` và lifecycle diagnostics](09-deinit-va-lifecycle-diagnostics.md)
