...
levels:
  - L2
prerequisites:
  - "Memory Graph, Leaks và Allocations"
used_later:
...
Trong Global Commerce, Product Detail đã đóng nhưng memory vẫn tăng sau mỗi lần mở vì lifetime thực tế khác lifetime mong đợi. Chapter này tập trung vào **Leak vs memory pressure**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng leak là object đáng lẽ chết; pressure có thể đến từ object hợp lệ quá lớn;
- nhận diện failure mode chính: dùng Leaks để kết luận mọi memory growth;
- chọn giải pháp bằng rule: sửa ownership cho leak; budget/eviction/downsampling cho pressure;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng repeated-flow plateau, allocations và decoded-size budget;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Memory Graph, Leaks và Allocations](10-memory-graph-leaks-va-allocations.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Phase Review: Memory & Runtime](99-phase-review.md) dùng contract của chapter này làm building block.
...
Creator → strong ownership graph → release edges → reference count về 0 → deinit
                 ↓
Focus: Leak vs memory pressure
                 ↓
Evidence: repeated-flow plateau, allocations và decoded-size budget
```
...
Trọng tâm của **Leak vs memory pressure** là: leak là object đáng lẽ chết; pressure có thể đến từ object hợp lệ quá lớn. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng repeated-flow plateau, allocations và decoded-size budget.
...
Áp dụng khi sửa ownership cho leak; budget/eviction/downsampling cho pressure. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là dùng Leaks để kết luận mọi memory growth. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Leak vs memory pressure?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Dùng leaks để kết luận mọi memory growth tạo evidence gì?
...
ARC chèn retain/release theo semantics của chương trình; graph ownership, không phải một dòng weak, quyết định lifetime. Với **Leak vs memory pressure**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Leak vs memory pressure** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy dùng Leaks để kết luận mọi memory growth.
...
Dùng repeated-flow plateau, allocations và decoded-size budget; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **sửa ownership cho leak; budget/eviction/downsampling cho pressure**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **repeated-flow plateau, allocations và decoded-size budget**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Leak vs memory pressure** là code an toàn.
>
...
- Dùng leaks để kết luận mọi memory growth → behavior chỉ sai ở scale/lifecycle edge.
...
- Sửa ownership cho leak; budget/eviction/downsampling cho pressure.
...
**Hỏi:** Leak vs memory pressure giải quyết vấn đề gì?

**30-second:** Leak là object đáng lẽ chết; pressure có thể đến từ object hợp lệ quá lớn. Chọn nó khi sửa ownership cho leak; budget/eviction/downsampling cho pressure; rủi ro chính là dùng Leaks để kết luận mọi memory growth.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao dùng Leaks để kết luận mọi memory growth phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **repeated-flow plateau, allocations và decoded-size budget**.
...
Viết một ví dụ nhỏ minh họa **Leak vs memory pressure** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **dùng Leaks để kết luận mọi memory growth** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Leak vs memory pressure**. Thu evidence bằng repeated-flow plateau, allocations và decoded-size budget, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → leak là object đáng lẽ chết; pressure có thể đến từ object hợp lệ quá lớn
Use when  → sửa ownership cho leak; budget/eviction/downsampling cho pressure
Risk      → dùng Leaks để kết luận mọi memory growth
Evidence  → repeated-flow plateau, allocations và decoded-size budget
...
3. Usage rule: sửa ownership cho leak; budget/eviction/downsampling cho pressure.
4. Mistake nguy hiểm: dùng Leaks để kết luận mọi memory growth.
5. Production lesson: kết luận bằng repeated-flow plateau, allocations và decoded-size budget, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Memory Graph, Leaks và Allocations](10-memory-graph-leaks-va-allocations.md)
- [Phase Review: Memory & Runtime](99-phase-review.md)
