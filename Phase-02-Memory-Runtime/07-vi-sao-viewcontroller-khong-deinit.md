---
title: "Vì sao ViewController không `deinit`?"
...
competencies:
  ...
  - "Interview"
tags:
  - "vi-sao-viewcontroller-khong-deinit"
  - "global-commerce"
---
...
Trong Global Commerce, Product Detail đã đóng nhưng memory vẫn tăng sau mỗi lần mở vì lifetime thực tế khác lifetime mong đợi. Chapter này tập trung vào **Vì sao ViewController không `deinit`?**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng screen không deinit là symptom của strong path còn tồn tại;
- nhận diện failure mode chính: chỉ thêm weak self mà không tìm path;
- chọn giải pháp bằng rule: vẽ graph coordinator/delegate/task/closure trước khi sửa;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng Memory Graph path to root và repeated push/pop;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Closure capture, escaping và capture list](06-closure-capture-escaping-va-capture-list.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Delegate, timer, observer và task lifetime](08-delegate-timer-observer-va-task-lifetime.md) dùng contract của chapter này làm building block.
...
Creator → strong ownership graph → release edges → reference count về 0 → deinit
                 ↓
Focus: Vì sao ViewController không `deinit`?
                 ↓
Evidence: Memory Graph path to root và repeated push/pop
```
...
Trọng tâm của **Vì sao ViewController không `deinit`?** là: screen không deinit là symptom của strong path còn tồn tại. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng Memory Graph path to root và repeated push/pop.
...
Áp dụng khi vẽ graph coordinator/delegate/task/closure trước khi sửa. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là chỉ thêm weak self mà không tìm path. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Vì sao ViewController không `deinit`??
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Chỉ thêm weak self mà không tìm path tạo evidence gì?
...
ARC chèn retain/release theo semantics của chương trình; graph ownership, không phải một dòng weak, quyết định lifetime. Với **Vì sao ViewController không `deinit`?**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Vì sao ViewController không `deinit`?** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy chỉ thêm weak self mà không tìm path.
...
Dùng Memory Graph path to root và repeated push/pop; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **vẽ graph coordinator/delegate/task/closure trước khi sửa**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **Memory Graph path to root và repeated push/pop**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Vì sao ViewController không `deinit`?** là code an toàn.
>
...
- Chỉ thêm weak self mà không tìm path → behavior chỉ sai ở scale/lifecycle edge.
...
- Vẽ graph coordinator/delegate/task/closure trước khi sửa.
...
**Hỏi:** Vì sao ViewController không `deinit`? giải quyết vấn đề gì?

**30-second:** Screen không deinit là symptom của strong path còn tồn tại. Chọn nó khi vẽ graph coordinator/delegate/task/closure trước khi sửa; rủi ro chính là chỉ thêm weak self mà không tìm path.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao chỉ thêm weak self mà không tìm path phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **Memory Graph path to root và repeated push/pop**.
...
Viết một ví dụ nhỏ minh họa **Vì sao ViewController không `deinit`?** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **chỉ thêm weak self mà không tìm path** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Vì sao ViewController không `deinit`?**. Thu evidence bằng Memory Graph path to root và repeated push/pop, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → screen không deinit là symptom của strong path còn tồn tại
Use when  → vẽ graph coordinator/delegate/task/closure trước khi sửa
Risk      → chỉ thêm weak self mà không tìm path
Evidence  → Memory Graph path to root và repeated push/pop
...
3. Usage rule: vẽ graph coordinator/delegate/task/closure trước khi sửa.
4. Mistake nguy hiểm: chỉ thêm weak self mà không tìm path.
5. Production lesson: kết luận bằng Memory Graph path to root và repeated push/pop, rồi bảo vệ bằng test và metric.
...
- [Delegate, timer, observer và task lifetime](08-delegate-timer-observer-va-task-lifetime.md)
