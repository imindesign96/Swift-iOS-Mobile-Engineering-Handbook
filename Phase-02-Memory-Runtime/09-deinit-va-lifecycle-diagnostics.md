---
title: "`deinit` và lifecycle diagnostics"
...
competencies:
  ...
  - "Interview"
tags:
  - "deinit-va-lifecycle-diagnostics"
  - "global-commerce"
---
...
Trong Global Commerce, Product Detail đã đóng nhưng memory vẫn tăng sau mỗi lần mở vì lifetime thực tế khác lifetime mong đợi. Chapter này tập trung vào **`deinit` và lifecycle diagnostics**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng `deinit` và lifecycle diagnostics phải được hiểu bằng behavior, constraint và trade-off quan sát được;
- nhận diện failure mode chính: dùng API/pattern theo tên gọi mà không xác định owner, state và failure mode;
- chọn giải pháp bằng rule: chọn giải pháp phục vụ mục tiêu: lý luận object graph bằng creator, owner, release và expected deinit;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng Xcode Memory Graph, Instruments Leaks/Allocations, deinit probe và repeated-flow measurement.;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Delegate, timer, observer và task lifetime](08-delegate-timer-observer-va-task-lifetime.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Memory Graph, Leaks và Allocations](10-memory-graph-leaks-va-allocations.md) dùng contract của chapter này làm building block.
...
Creator → strong ownership graph → release edges → reference count về 0 → deinit
                 ↓
Focus: `deinit` và lifecycle diagnostics
                 ↓
...
Trọng tâm của **`deinit` và lifecycle diagnostics** là: `deinit` và lifecycle diagnostics phải được hiểu bằng behavior, constraint và trade-off quan sát được. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng Xcode Memory Graph, Instruments Leaks/Allocations, deinit probe và repeated-flow measurement..
...
Áp dụng khi chọn giải pháp phục vụ mục tiêu: lý luận object graph bằng creator, owner, release và expected deinit. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
1. Observable behavior nào định nghĩa `deinit` và lifecycle diagnostics?
...
ARC chèn retain/release theo semantics của chương trình; graph ownership, không phải một dòng weak, quyết định lifetime. Với **`deinit` và lifecycle diagnostics**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **`deinit` và lifecycle diagnostics** trong flow có network, UI lifecycle và cache.
...
Dùng Xcode Memory Graph, Instruments Leaks/Allocations, deinit probe và repeated-flow measurement.; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **chọn giải pháp phục vụ mục tiêu: lý luận object graph bằng creator, owner, release và expected deinit**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **Xcode Memory Graph, Instruments Leaks/Allocations, deinit probe và repeated-flow measurement.**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **`deinit` và lifecycle diagnostics** là code an toàn.
>
...
- Chọn giải pháp phục vụ mục tiêu: lý luận object graph bằng creator, owner, release và expected deinit.
...
**Hỏi:** `deinit` và lifecycle diagnostics giải quyết vấn đề gì?

**30-second:** `deinit` và lifecycle diagnostics phải được hiểu bằng behavior, constraint và trade-off quan sát được. Chọn nó khi chọn giải pháp phục vụ mục tiêu: lý luận object graph bằng creator, owner, release và expected deinit; rủi ro chính là dùng API/pattern theo tên gọi mà không xác định owner, state và failure mode.
...
Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **Xcode Memory Graph, Instruments Leaks/Allocations, deinit probe và repeated-flow measurement.**.
...
Viết một ví dụ nhỏ minh họa **`deinit` và lifecycle diagnostics** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **`deinit` và lifecycle diagnostics**. Thu evidence bằng Xcode Memory Graph, Instruments Leaks/Allocations, deinit probe và repeated-flow measurement., vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → `deinit` và lifecycle diagnostics phải được hiểu bằng behavior, constraint và trade-off quan sát được
Use when  → chọn giải pháp phục vụ mục tiêu: lý luận object graph bằng creator, owner, release và expected deinit
...
3. Usage rule: chọn giải pháp phục vụ mục tiêu: lý luận object graph bằng creator, owner, release và expected deinit.
4. Mistake nguy hiểm: dùng API/pattern theo tên gọi mà không xác định owner, state và failure mode.
5. Production lesson: kết luận bằng Xcode Memory Graph, Instruments Leaks/Allocations, deinit probe và repeated-flow measurement., rồi bảo vệ bằng test và metric.
