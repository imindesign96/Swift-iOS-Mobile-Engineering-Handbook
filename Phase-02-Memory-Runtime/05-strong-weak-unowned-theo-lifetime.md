---
title: "`strong`, `weak`, `unowned` theo lifetime"
...
Trong Global Commerce, Product Detail đã đóng nhưng memory vẫn tăng sau mỗi lần mở vì lifetime thực tế khác lifetime mong đợi. Chapter này tập trung vào **`strong`, `weak`, `unowned` theo lifetime**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng strong sở hữu; weak/unowned không sở hữu nhưng khác lifetime guarantee;
- nhận diện failure mode chính: unowned với lifetime không chắc chắn hoặc weak làm dependency biến mất;
- chọn giải pháp bằng rule: weak khi target có thể kết thúc trước; unowned chỉ khi invariant lifetime chắc chắn;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng deinit/crash reproduction và graph;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [ARC và ownership graph](04-arc-va-ownership-graph.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Closure capture, escaping và capture list](06-closure-capture-escaping-va-capture-list.md) dùng contract của chapter này làm building block.
...
Creator → strong ownership graph → release edges → reference count về 0 → deinit
                 ↓
Focus: `strong`, `weak`, `unowned` theo lifetime
                 ↓
Evidence: deinit/crash reproduction và graph
```
...
Trọng tâm của **`strong`, `weak`, `unowned` theo lifetime** là: strong sở hữu; weak/unowned không sở hữu nhưng khác lifetime guarantee. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng deinit/crash reproduction và graph.
...
Áp dụng khi weak khi target có thể kết thúc trước; unowned chỉ khi invariant lifetime chắc chắn. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là unowned với lifetime không chắc chắn hoặc weak làm dependency biến mất. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa `strong`, `weak`, `unowned` theo lifetime?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Unowned với lifetime không chắc chắn hoặc weak làm dependency biến mất tạo evidence gì?
...
ARC chèn retain/release theo semantics của chương trình; graph ownership, không phải một dòng weak, quyết định lifetime. Với **`strong`, `weak`, `unowned` theo lifetime**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **`strong`, `weak`, `unowned` theo lifetime** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy unowned với lifetime không chắc chắn hoặc weak làm dependency biến mất.
...
Dùng deinit/crash reproduction và graph; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **weak khi target có thể kết thúc trước; unowned chỉ khi invariant lifetime chắc chắn**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **deinit/crash reproduction và graph**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **`strong`, `weak`, `unowned` theo lifetime** là code an toàn.
>
...
- Unowned với lifetime không chắc chắn hoặc weak làm dependency biến mất → behavior chỉ sai ở scale/lifecycle edge.
...
- Weak khi target có thể kết thúc trước; unowned chỉ khi invariant lifetime chắc chắn.
...
**Hỏi:** `strong`, `weak`, `unowned` theo lifetime giải quyết vấn đề gì?

**30-second:** Strong sở hữu; weak/unowned không sở hữu nhưng khác lifetime guarantee. Chọn nó khi weak khi target có thể kết thúc trước; unowned chỉ khi invariant lifetime chắc chắn; rủi ro chính là unowned với lifetime không chắc chắn hoặc weak làm dependency biến mất.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao unowned với lifetime không chắc chắn hoặc weak làm dependency biến mất phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **deinit/crash reproduction và graph**.
...
Viết một ví dụ nhỏ minh họa **`strong`, `weak`, `unowned` theo lifetime** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **unowned với lifetime không chắc chắn hoặc weak làm dependency biến mất** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **`strong`, `weak`, `unowned` theo lifetime**. Thu evidence bằng deinit/crash reproduction và graph, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → strong sở hữu; weak/unowned không sở hữu nhưng khác lifetime guarantee
Use when  → weak khi target có thể kết thúc trước; unowned chỉ khi invariant lifetime chắc chắn
Risk      → unowned với lifetime không chắc chắn hoặc weak làm dependency biến mất
Evidence  → deinit/crash reproduction và graph
...
3. Usage rule: weak khi target có thể kết thúc trước; unowned chỉ khi invariant lifetime chắc chắn.
4. Mistake nguy hiểm: unowned với lifetime không chắc chắn hoặc weak làm dependency biến mất.
5. Production lesson: kết luận bằng deinit/crash reproduction và graph, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [ARC và ownership graph](04-arc-va-ownership-graph.md)
- [Closure capture, escaping và capture list](06-closure-capture-escaping-va-capture-list.md)
