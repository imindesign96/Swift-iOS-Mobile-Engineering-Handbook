---
title: "SwiftUI declarative mental model"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **SwiftUI declarative mental model**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Delegate pattern trong UIKit](10-delegate-pattern-trong-uikit.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [State, Binding và source of truth](12-state-binding-va-source-of-truth.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: SwiftUI declarative mental model
                 ↓
...
Trọng tâm của **SwiftUI declarative mental model** là: SwiftUI state ownership và stable identity quyết định lifetime/update. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa SwiftUI declarative mental model?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **SwiftUI declarative mental model**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **SwiftUI declarative mental model** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **SwiftUI declarative mental model** là code an toàn.
>
...
**Hỏi:** SwiftUI declarative mental model giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **SwiftUI declarative mental model** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **SwiftUI declarative mental model**. Thu evidence bằng SwiftUI Instruments và state-transition log, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [State, Binding và source of truth](12-state-binding-va-source-of-truth.md)
