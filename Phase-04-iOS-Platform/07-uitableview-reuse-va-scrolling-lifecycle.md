---
title: "UITableView reuse và scrolling lifecycle"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
used_later:
  - "UICollectionView và compositional layout"
competencies:
  - "iOS Platform"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "uitableview-reuse-va-scrolling-lifecycle"
  - "global-commerce"
---
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **UITableView reuse và scrolling lifecycle**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng reuse tách cell identity khỏi item identity; snapshot cần stable domain ID;
- nhận diện failure mode chính: giữ stale task/image trong reused cell hoặc ID bằng index;
- chọn giải pháp bằng rule: cancel old work trong reuse và bind bằng immutable item ID;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng scroll trace, reuse log và diffable snapshot audit;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Hugging, compression resistance và ambiguous layout](06-hugging-compression-resistance-va-ambiguous-layout.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [UICollectionView và compositional layout](08-uicollectionview-va-compositional-layout.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: UITableView reuse và scrolling lifecycle
                 ↓
Evidence: scroll trace, reuse log và diffable snapshot audit
```
...
Trọng tâm của **UITableView reuse và scrolling lifecycle** là: reuse tách cell identity khỏi item identity; snapshot cần stable domain ID. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng scroll trace, reuse log và diffable snapshot audit.
...
Áp dụng khi cancel old work trong reuse và bind bằng immutable item ID. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là giữ stale task/image trong reused cell hoặc ID bằng index. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa UITableView reuse và scrolling lifecycle?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Giữ stale task/image trong reused cell hoặc id bằng index tạo evidence gì?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **UITableView reuse và scrolling lifecycle**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **UITableView reuse và scrolling lifecycle** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy giữ stale task/image trong reused cell hoặc ID bằng index.
...
Dùng scroll trace, reuse log và diffable snapshot audit; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **cancel old work trong reuse và bind bằng immutable item ID**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **scroll trace, reuse log và diffable snapshot audit**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **UITableView reuse và scrolling lifecycle** là code an toàn.
>
...
- Giữ stale task/image trong reused cell hoặc id bằng index → behavior chỉ sai ở scale/lifecycle edge.
...
- Cancel old work trong reuse và bind bằng immutable item id.
...
**Hỏi:** UITableView reuse và scrolling lifecycle giải quyết vấn đề gì?

**30-second:** Reuse tách cell identity khỏi item identity; snapshot cần stable domain id. Chọn nó khi cancel old work trong reuse và bind bằng immutable item ID; rủi ro chính là giữ stale task/image trong reused cell hoặc ID bằng index.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao giữ stale task/image trong reused cell hoặc ID bằng index phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **scroll trace, reuse log và diffable snapshot audit**.
...
Viết một ví dụ nhỏ minh họa **UITableView reuse và scrolling lifecycle** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **giữ stale task/image trong reused cell hoặc ID bằng index** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **UITableView reuse và scrolling lifecycle**. Thu evidence bằng scroll trace, reuse log và diffable snapshot audit, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → reuse tách cell identity khỏi item identity; snapshot cần stable domain ID
Use when  → cancel old work trong reuse và bind bằng immutable item ID
Risk      → giữ stale task/image trong reused cell hoặc ID bằng index
Evidence  → scroll trace, reuse log và diffable snapshot audit
...
3. Usage rule: cancel old work trong reuse và bind bằng immutable item ID.
4. Mistake nguy hiểm: giữ stale task/image trong reused cell hoặc ID bằng index.
5. Production lesson: kết luận bằng scroll trace, reuse log và diffable snapshot audit, rồi bảo vệ bằng test và metric.
...
- [UICollectionView và compositional layout](08-uicollectionview-va-compositional-layout.md)
