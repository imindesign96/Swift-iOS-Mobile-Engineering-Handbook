---
title: "UIView: hierarchy, layout và drawing"
phase: "iOS Platform"
difficulty: 3
importance: 5
...
levels:
  - L3
prerequisites:
  - "App lifecycle và scene lifecycle"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "uiview-hierarchy-layout-va-drawing"
  - "global-commerce"
---
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **UIView: hierarchy, layout và drawing**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [App lifecycle và scene lifecycle](01-app-lifecycle-va-scene-lifecycle.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [UIViewController lifecycle và ownership](03-uiviewcontroller-lifecycle-va-ownership.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: UIView: hierarchy, layout và drawing
                 ↓
...
Trọng tâm của **UIView: hierarchy, layout và drawing** là: ARC quản lý lifetime qua strong references trong ownership graph. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa UIView: hierarchy, layout và drawing?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **UIView: hierarchy, layout và drawing**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **UIView: hierarchy, layout và drawing** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **UIView: hierarchy, layout và drawing** là code an toàn.
>
...
**Hỏi:** UIView: hierarchy, layout và drawing giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **UIView: hierarchy, layout và drawing** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **UIView: hierarchy, layout và drawing**. Thu evidence bằng Memory Graph + deinit expectation, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [App lifecycle và scene lifecycle](01-app-lifecycle-va-scene-lifecycle.md)
