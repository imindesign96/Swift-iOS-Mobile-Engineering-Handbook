---
title: "View identity, ForEach và state lifetime"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
levels:
  - L3
prerequisites:
  - "Environment và dependency flow"
used_later:
  - "NavigationStack và typed navigation"
competencies:
  - "iOS Platform"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "view-identity-foreach-va-state-lifetime"
  - "global-commerce"
---

# View identity, ForEach và state lifetime
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **View identity, ForEach và state lifetime**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Environment và dependency flow](14-environment-va-dependency-flow.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [NavigationStack và typed navigation](16-navigationstack-va-typed-navigation.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: View identity, ForEach và state lifetime
                 ↓
...
Trọng tâm của **View identity, ForEach và state lifetime** là: SwiftUI state ownership và stable identity quyết định lifetime/update. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa View identity, ForEach và state lifetime?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **View identity, ForEach và state lifetime**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **View identity, ForEach và state lifetime** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **View identity, ForEach và state lifetime** là code an toàn.
>
...
**Hỏi:** View identity, ForEach và state lifetime giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **View identity, ForEach và state lifetime** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **View identity, ForEach và state lifetime**. Thu evidence bằng SwiftUI Instruments và state-transition log, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Environment và dependency flow](14-environment-va-dependency-flow.md)
- [NavigationStack và typed navigation](16-navigationstack-va-typed-navigation.md)
