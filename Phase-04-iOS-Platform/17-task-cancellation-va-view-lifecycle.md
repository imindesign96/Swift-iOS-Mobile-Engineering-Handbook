---
title: "`.task`, cancellation và view lifecycle"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "task-cancellation-va-view-lifecycle"
  - "global-commerce"
---
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **`.task`, cancellation và view lifecycle**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [NavigationStack và typed navigation](16-navigationstack-va-typed-navigation.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Animation, transaction và rendering cost](18-animation-transaction-va-rendering-cost.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: `.task`, cancellation và view lifecycle
                 ↓
...
Trọng tâm của **`.task`, cancellation và view lifecycle** là: cancellation là cooperative signal, không cưỡng bức dừng mọi code. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa `.task`, cancellation và view lifecycle?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **`.task`, cancellation và view lifecycle**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **`.task`, cancellation và view lifecycle** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **`.task`, cancellation và view lifecycle** là code an toàn.
>
...
**Hỏi:** `.task`, cancellation và view lifecycle giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **`.task`, cancellation và view lifecycle** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **`.task`, cancellation và view lifecycle**. Thu evidence bằng cancel-to-stop latency và state after cancel, vẽ owner/state graph, xác định root cause và thêm regression test.
