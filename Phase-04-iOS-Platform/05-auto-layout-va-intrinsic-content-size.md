---
title: "Auto Layout và intrinsic content size"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
levels:
  - L3
prerequisites:
  - "Navigation: push, present và coordinator boundary"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "auto-layout-va-intrinsic-content-size"
  - "global-commerce"
---
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **Auto Layout và intrinsic content size**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Navigation: push, present và coordinator boundary](04-navigation-push-present-va-coordinator-boundary.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Hugging, compression resistance và ambiguous layout](06-hugging-compression-resistance-va-ambiguous-layout.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: Auto Layout và intrinsic content size
                 ↓
...
Trọng tâm của **Auto Layout và intrinsic content size** là: constraint system giải quan hệ; intrinsic size cung cấp preference chứ không phải frame tuyệt đối. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Auto Layout và intrinsic content size?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **Auto Layout và intrinsic content size**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Auto Layout và intrinsic content size** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Auto Layout và intrinsic content size** là code an toàn.
>
...
**Hỏi:** Auto Layout và intrinsic content size giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Auto Layout và intrinsic content size** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Auto Layout và intrinsic content size**. Thu evidence bằng View Debugger và unsatisfiable-constraint log, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Navigation: push, present và coordinator boundary](04-navigation-push-present-va-coordinator-boundary.md)
- [Hugging, compression resistance và ambiguous layout](06-hugging-compression-resistance-va-ambiguous-layout.md)
