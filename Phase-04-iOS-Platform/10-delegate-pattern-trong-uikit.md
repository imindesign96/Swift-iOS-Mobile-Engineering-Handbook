---
title: "Delegate pattern trong UIKit"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **Delegate pattern trong UIKit**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng Delegate pattern trong UIKit phải được hiểu bằng behavior, constraint và trade-off quan sát được;
- nhận diện failure mode chính: dùng API/pattern theo tên gọi mà không xác định owner, state và failure mode;
- chọn giải pháp bằng rule: chọn giải pháp phục vụ mục tiêu: đặt UI state và work đúng lifecycle trong UIKit lẫn SwiftUI;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng View Debugger, constraint logs, SwiftUI Instruments, Time Profiler và lifecycle logging.;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Diffable Data Source và stable identity](09-diffable-data-source-va-stable-identity.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [SwiftUI declarative mental model](11-swiftui-declarative-mental-model.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: Delegate pattern trong UIKit
                 ↓
...
Trọng tâm của **Delegate pattern trong UIKit** là: Delegate pattern trong UIKit phải được hiểu bằng behavior, constraint và trade-off quan sát được. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng View Debugger, constraint logs, SwiftUI Instruments, Time Profiler và lifecycle logging..
...
Áp dụng khi chọn giải pháp phục vụ mục tiêu: đặt UI state và work đúng lifecycle trong UIKit lẫn SwiftUI. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
1. Observable behavior nào định nghĩa Delegate pattern trong UIKit?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **Delegate pattern trong UIKit**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Delegate pattern trong UIKit** trong flow có network, UI lifecycle và cache.
...
Dùng View Debugger, constraint logs, SwiftUI Instruments, Time Profiler và lifecycle logging.; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **chọn giải pháp phục vụ mục tiêu: đặt UI state và work đúng lifecycle trong UIKit lẫn SwiftUI**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **View Debugger, constraint logs, SwiftUI Instruments, Time Profiler và lifecycle logging.**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Delegate pattern trong UIKit** là code an toàn.
>
...
- Chọn giải pháp phục vụ mục tiêu: đặt ui state và work đúng lifecycle trong uikit lẫn swiftui.
...
**Hỏi:** Delegate pattern trong UIKit giải quyết vấn đề gì?

**30-second:** Delegate pattern trong uikit phải được hiểu bằng behavior, constraint và trade-off quan sát được. Chọn nó khi chọn giải pháp phục vụ mục tiêu: đặt UI state và work đúng lifecycle trong UIKit lẫn SwiftUI; rủi ro chính là dùng API/pattern theo tên gọi mà không xác định owner, state và failure mode.
...
Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **View Debugger, constraint logs, SwiftUI Instruments, Time Profiler và lifecycle logging.**.
...
Viết một ví dụ nhỏ minh họa **Delegate pattern trong UIKit** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Delegate pattern trong UIKit**. Thu evidence bằng View Debugger, constraint logs, SwiftUI Instruments, Time Profiler và lifecycle logging., vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → Delegate pattern trong UIKit phải được hiểu bằng behavior, constraint và trade-off quan sát được
Use when  → chọn giải pháp phục vụ mục tiêu: đặt UI state và work đúng lifecycle trong UIKit lẫn SwiftUI
...
3. Usage rule: chọn giải pháp phục vụ mục tiêu: đặt UI state và work đúng lifecycle trong UIKit lẫn SwiftUI.
4. Mistake nguy hiểm: dùng API/pattern theo tên gọi mà không xác định owner, state và failure mode.
5. Production lesson: kết luận bằng View Debugger, constraint logs, SwiftUI Instruments, Time Profiler và lifecycle logging., rồi bảo vệ bằng test và metric.
