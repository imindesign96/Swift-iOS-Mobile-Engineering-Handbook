---
title: "Hugging, compression resistance và ambiguous layout"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
levels:
  - L3
prerequisites:
  - "Auto Layout và intrinsic content size"
used_later:
  - "UITableView reuse và scrolling lifecycle"
competencies:
  - "iOS Platform"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "hugging-compression-resistance-va-ambiguous-layout"
  - "global-commerce"
---

# Hugging, compression resistance và ambiguous layout
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **Hugging, compression resistance và ambiguous layout**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng constraint system giải quan hệ; intrinsic size cung cấp preference chứ không phải frame tuyệt đối;
- nhận diện failure mode chính: constraint conflict/ambiguity hoặc ưu tiên hugging sai;
- chọn giải pháp bằng rule: mô tả intent bằng constraint tối thiểu và priority có lý do;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng View Debugger và unsatisfiable-constraint log;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Auto Layout và intrinsic content size](05-auto-layout-va-intrinsic-content-size.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [UITableView reuse và scrolling lifecycle](07-uitableview-reuse-va-scrolling-lifecycle.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: Hugging, compression resistance và ambiguous layout
                 ↓
Evidence: View Debugger và unsatisfiable-constraint log
```
...
Trọng tâm của **Hugging, compression resistance và ambiguous layout** là: constraint system giải quan hệ; intrinsic size cung cấp preference chứ không phải frame tuyệt đối. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng View Debugger và unsatisfiable-constraint log.
...
Áp dụng khi mô tả intent bằng constraint tối thiểu và priority có lý do. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là constraint conflict/ambiguity hoặc ưu tiên hugging sai. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Hugging, compression resistance và ambiguous layout?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Constraint conflict/ambiguity hoặc ưu tiên hugging sai tạo evidence gì?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **Hugging, compression resistance và ambiguous layout**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Hugging, compression resistance và ambiguous layout** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy constraint conflict/ambiguity hoặc ưu tiên hugging sai.
...
Dùng View Debugger và unsatisfiable-constraint log; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **mô tả intent bằng constraint tối thiểu và priority có lý do**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **View Debugger và unsatisfiable-constraint log**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Hugging, compression resistance và ambiguous layout** là code an toàn.
>
...
- Constraint conflict/ambiguity hoặc ưu tiên hugging sai → behavior chỉ sai ở scale/lifecycle edge.
...
- Mô tả intent bằng constraint tối thiểu và priority có lý do.
...
**Hỏi:** Hugging, compression resistance và ambiguous layout giải quyết vấn đề gì?

**30-second:** Constraint system giải quan hệ; intrinsic size cung cấp preference chứ không phải frame tuyệt đối. Chọn nó khi mô tả intent bằng constraint tối thiểu và priority có lý do; rủi ro chính là constraint conflict/ambiguity hoặc ưu tiên hugging sai.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao constraint conflict/ambiguity hoặc ưu tiên hugging sai phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **View Debugger và unsatisfiable-constraint log**.
...
Viết một ví dụ nhỏ minh họa **Hugging, compression resistance và ambiguous layout** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **constraint conflict/ambiguity hoặc ưu tiên hugging sai** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Hugging, compression resistance và ambiguous layout**. Thu evidence bằng View Debugger và unsatisfiable-constraint log, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → constraint system giải quan hệ; intrinsic size cung cấp preference chứ không phải frame tuyệt đối
Use when  → mô tả intent bằng constraint tối thiểu và priority có lý do
Risk      → constraint conflict/ambiguity hoặc ưu tiên hugging sai
Evidence  → View Debugger và unsatisfiable-constraint log
...
3. Usage rule: mô tả intent bằng constraint tối thiểu và priority có lý do.
4. Mistake nguy hiểm: constraint conflict/ambiguity hoặc ưu tiên hugging sai.
5. Production lesson: kết luận bằng View Debugger và unsatisfiable-constraint log, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Auto Layout và intrinsic content size](05-auto-layout-va-intrinsic-content-size.md)
- [UITableView reuse và scrolling lifecycle](07-uitableview-reuse-va-scrolling-lifecycle.md)
