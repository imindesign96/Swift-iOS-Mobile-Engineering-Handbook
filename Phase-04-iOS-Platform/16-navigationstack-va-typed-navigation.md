---
title: "NavigationStack và typed navigation"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
used_later:
  - "`.task`, cancellation và view lifecycle"
competencies:
  - "iOS Platform"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "navigationstack-va-typed-navigation"
  - "global-commerce"
---
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **NavigationStack và typed navigation**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [View identity, ForEach và state lifetime](15-view-identity-foreach-va-state-lifetime.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [`.task`, cancellation và view lifecycle](17-task-cancellation-va-view-lifecycle.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: NavigationStack và typed navigation
                 ↓
...
Trọng tâm của **NavigationStack và typed navigation** là: SwiftUI state ownership và stable identity quyết định lifetime/update. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa NavigationStack và typed navigation?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **NavigationStack và typed navigation**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **NavigationStack và typed navigation** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **NavigationStack và typed navigation** là code an toàn.
>
...
**Hỏi:** NavigationStack và typed navigation giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **NavigationStack và typed navigation** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **NavigationStack và typed navigation**. Thu evidence bằng SwiftUI Instruments và state-transition log, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [View identity, ForEach và state lifetime](15-view-identity-foreach-va-state-lifetime.md)
- [`.task`, cancellation và view lifecycle](17-task-cancellation-va-view-lifecycle.md)
