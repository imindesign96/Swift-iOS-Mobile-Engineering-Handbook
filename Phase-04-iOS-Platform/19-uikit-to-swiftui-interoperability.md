---
title: "UIKit ↔ SwiftUI interoperability"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "uikit-to-swiftui-interoperability"
  - "global-commerce"
---
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **UIKit ↔ SwiftUI interoperability**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Animation, transaction và rendering cost](18-animation-transaction-va-rendering-cost.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: UIKit ↔ SwiftUI interoperability
                 ↓
...
Trọng tâm của **UIKit ↔ SwiftUI interoperability** là: SwiftUI state ownership và stable identity quyết định lifetime/update. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa UIKit ↔ SwiftUI interoperability?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **UIKit ↔ SwiftUI interoperability**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **UIKit ↔ SwiftUI interoperability** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **UIKit ↔ SwiftUI interoperability** là code an toàn.
>
...
**Hỏi:** UIKit ↔ SwiftUI interoperability giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **UIKit ↔ SwiftUI interoperability** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **UIKit ↔ SwiftUI interoperability**. Thu evidence bằng SwiftUI Instruments và state-transition log, vẽ owner/state graph, xác định root cause và thêm regression test.
