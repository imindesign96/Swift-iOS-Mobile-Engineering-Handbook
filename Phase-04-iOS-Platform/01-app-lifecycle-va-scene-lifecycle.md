---
title: "App lifecycle và scene lifecycle"
phase: "iOS Platform"
difficulty: 3
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "app-lifecycle-va-scene-lifecycle"
  - "global-commerce"
---
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **App lifecycle và scene lifecycle**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [UIView: hierarchy, layout và drawing](02-uiview-hierarchy-layout-va-drawing.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: App lifecycle và scene lifecycle
                 ↓
...
Trọng tâm của **App lifecycle và scene lifecycle** là: app/scene state là external lifecycle signal, không phải bảo đảm thời lượng. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa App lifecycle và scene lifecycle?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **App lifecycle và scene lifecycle**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **App lifecycle và scene lifecycle** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **App lifecycle và scene lifecycle** là code an toàn.
>
...
**Hỏi:** App lifecycle và scene lifecycle giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **App lifecycle và scene lifecycle** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **App lifecycle và scene lifecycle**. Thu evidence bằng lifecycle logs và background task expiration, vẽ owner/state graph, xác định root cause và thêm regression test.
