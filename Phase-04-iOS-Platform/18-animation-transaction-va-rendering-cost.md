---
title: "Animation, transaction và rendering cost"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "animation-transaction-va-rendering-cost"
  - "global-commerce"
---
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **Animation, transaction và rendering cost**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [`.task`, cancellation và view lifecycle](17-task-cancellation-va-view-lifecycle.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [UIKit ↔ SwiftUI interoperability](19-uikit-to-swiftui-interoperability.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: Animation, transaction và rendering cost
                 ↓
...
Trọng tâm của **Animation, transaction và rendering cost** là: animation mô tả transition của state trong transaction và vẫn chịu layout/render cost. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Animation, transaction và rendering cost?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **Animation, transaction và rendering cost**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Animation, transaction và rendering cost** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Animation, transaction và rendering cost** là code an toàn.
>
...
**Hỏi:** Animation, transaction và rendering cost giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Animation, transaction và rendering cost** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Animation, transaction và rendering cost**. Thu evidence bằng animation hitches và Time Profiler, vẽ owner/state graph, xác định root cause và thêm regression test.
