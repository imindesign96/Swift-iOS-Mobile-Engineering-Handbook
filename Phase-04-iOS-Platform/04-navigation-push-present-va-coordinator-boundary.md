---
title: "Navigation: push, present và coordinator boundary"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
levels:
  - L3
prerequisites:
  - "UIViewController lifecycle và ownership"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "navigation-push-present-va-coordinator-boundary"
  - "global-commerce"
---
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **Navigation: push, present và coordinator boundary**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng Coordinator sở hữu navigation flow khi flow vượt một screen;
- nhận diện failure mode chính: coordinator giữ controller mãi hoặc business logic trôi vào routing;
- chọn giải pháp bằng rule: child coordinator phải được release khi flow kết thúc;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng ownership graph và route tests;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [UIViewController lifecycle và ownership](03-uiviewcontroller-lifecycle-va-ownership.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Auto Layout và intrinsic content size](05-auto-layout-va-intrinsic-content-size.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: Navigation: push, present và coordinator boundary
                 ↓
Evidence: ownership graph và route tests
```
...
Trọng tâm của **Navigation: push, present và coordinator boundary** là: Coordinator sở hữu navigation flow khi flow vượt một screen. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng ownership graph và route tests.
...
Áp dụng khi child coordinator phải được release khi flow kết thúc. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là coordinator giữ controller mãi hoặc business logic trôi vào routing. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Navigation: push, present và coordinator boundary?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Coordinator giữ controller mãi hoặc business logic trôi vào routing tạo evidence gì?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **Navigation: push, present và coordinator boundary**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Navigation: push, present và coordinator boundary** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy coordinator giữ controller mãi hoặc business logic trôi vào routing.
...
Dùng ownership graph và route tests; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **child coordinator phải được release khi flow kết thúc**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **ownership graph và route tests**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Navigation: push, present và coordinator boundary** là code an toàn.
>
...
- Coordinator giữ controller mãi hoặc business logic trôi vào routing → behavior chỉ sai ở scale/lifecycle edge.
...
- Child coordinator phải được release khi flow kết thúc.
...
**Hỏi:** Navigation: push, present và coordinator boundary giải quyết vấn đề gì?

**30-second:** Coordinator sở hữu navigation flow khi flow vượt một screen. Chọn nó khi child coordinator phải được release khi flow kết thúc; rủi ro chính là coordinator giữ controller mãi hoặc business logic trôi vào routing.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao coordinator giữ controller mãi hoặc business logic trôi vào routing phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **ownership graph và route tests**.
...
Viết một ví dụ nhỏ minh họa **Navigation: push, present và coordinator boundary** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **coordinator giữ controller mãi hoặc business logic trôi vào routing** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Navigation: push, present và coordinator boundary**. Thu evidence bằng ownership graph và route tests, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → Coordinator sở hữu navigation flow khi flow vượt một screen
Use when  → child coordinator phải được release khi flow kết thúc
Risk      → coordinator giữ controller mãi hoặc business logic trôi vào routing
Evidence  → ownership graph và route tests
...
3. Usage rule: child coordinator phải được release khi flow kết thúc.
4. Mistake nguy hiểm: coordinator giữ controller mãi hoặc business logic trôi vào routing.
5. Production lesson: kết luận bằng ownership graph và route tests, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [UIViewController lifecycle và ownership](03-uiviewcontroller-lifecycle-va-ownership.md)
