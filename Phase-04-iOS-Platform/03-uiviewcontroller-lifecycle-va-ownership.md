---
title: "UIViewController lifecycle và ownership"
phase: "iOS Platform"
difficulty: 3
importance: 5
...
levels:
  - L3
prerequisites:
  - "UIView: hierarchy, layout và drawing"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "uiviewcontroller-lifecycle-va-ownership"
  - "global-commerce"
---
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **UIViewController lifecycle và ownership**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [UIView: hierarchy, layout và drawing](02-uiview-hierarchy-layout-va-drawing.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Navigation: push, present và coordinator boundary](04-navigation-push-present-va-coordinator-boundary.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: UIViewController lifecycle và ownership
                 ↓
...
Trọng tâm của **UIViewController lifecycle và ownership** là: ARC quản lý lifetime qua strong references trong ownership graph. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa UIViewController lifecycle và ownership?
...
```swift
@MainActor func publish(_ products: [Product]) { model.products = products }
```
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **UIViewController lifecycle và ownership**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **UIViewController lifecycle và ownership** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **UIViewController lifecycle và ownership** là code an toàn.
>
...
**Hỏi:** UIViewController lifecycle và ownership giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **UIViewController lifecycle và ownership** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **UIViewController lifecycle và ownership**. Thu evidence bằng Memory Graph + deinit expectation, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [UIView: hierarchy, layout và drawing](02-uiview-hierarchy-layout-va-drawing.md)
