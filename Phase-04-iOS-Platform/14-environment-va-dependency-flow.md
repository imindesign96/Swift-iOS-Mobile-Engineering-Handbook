---
title: "Environment và dependency flow"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
levels:
  - L3
prerequisites:
  - "Observation và observable model ownership"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "environment-va-dependency-flow"
  - "global-commerce"
---
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **Environment và dependency flow**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Observation và observable model ownership](13-observation-va-observable-model-ownership.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [View identity, ForEach và state lifetime](15-view-identity-foreach-va-state-lifetime.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: Environment và dependency flow
                 ↓
...
Trọng tâm của **Environment và dependency flow** là: SwiftUI state ownership và stable identity quyết định lifetime/update. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Environment và dependency flow?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **Environment và dependency flow**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Environment và dependency flow** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Environment và dependency flow** là code an toàn.
>
...
**Hỏi:** Environment và dependency flow giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Environment và dependency flow** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Environment và dependency flow**. Thu evidence bằng SwiftUI Instruments và state-transition log, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Observation và observable model ownership](13-observation-va-observable-model-ownership.md)
