---
title: "State, Binding và source of truth"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
levels:
  - L3
prerequisites:
  - "SwiftUI declarative mental model"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "state-binding-va-source-of-truth"
  - "global-commerce"
---

# State, Binding và source of truth
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **State, Binding và source of truth**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [SwiftUI declarative mental model](11-swiftui-declarative-mental-model.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Observation và observable model ownership](13-observation-va-observable-model-ownership.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: State, Binding và source of truth
                 ↓
...
Trọng tâm của **State, Binding và source of truth** là: SwiftUI state ownership và stable identity quyết định lifetime/update. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa State, Binding và source of truth?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **State, Binding và source of truth**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **State, Binding và source of truth** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **State, Binding và source of truth** là code an toàn.
>
...
**Hỏi:** State, Binding và source of truth giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **State, Binding và source of truth** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **State, Binding và source of truth**. Thu evidence bằng SwiftUI Instruments và state-transition log, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [SwiftUI declarative mental model](11-swiftui-declarative-mental-model.md)
