---
title: "UICollectionView và compositional layout"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
used_later:
  - "Diffable Data Source và stable identity"
competencies:
  - "iOS Platform"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "uicollectionview-va-compositional-layout"
  - "global-commerce"
---
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **UICollectionView và compositional layout**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng collection choice mã hóa ordering, uniqueness và lookup semantics;
- nhận diện failure mode chính: dựa vào iteration order không được contract hoặc O(n) trong hot path;
- chọn giải pháp bằng rule: chọn Array/Set/Dictionary theo operation chủ đạo và domain;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng complexity measurement và invariant tests;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [UITableView reuse và scrolling lifecycle](07-uitableview-reuse-va-scrolling-lifecycle.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Diffable Data Source và stable identity](09-diffable-data-source-va-stable-identity.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: UICollectionView và compositional layout
                 ↓
Evidence: complexity measurement và invariant tests
```
...
Trọng tâm của **UICollectionView và compositional layout** là: collection choice mã hóa ordering, uniqueness và lookup semantics. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng complexity measurement và invariant tests.
...
Áp dụng khi chọn Array/Set/Dictionary theo operation chủ đạo và domain. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là dựa vào iteration order không được contract hoặc O(n) trong hot path. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa UICollectionView và compositional layout?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Dựa vào iteration order không được contract hoặc o(n) trong hot path tạo evidence gì?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **UICollectionView và compositional layout**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **UICollectionView và compositional layout** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy dựa vào iteration order không được contract hoặc O(n) trong hot path.
...
Dùng complexity measurement và invariant tests; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **chọn Array/Set/Dictionary theo operation chủ đạo và domain**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **complexity measurement và invariant tests**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **UICollectionView và compositional layout** là code an toàn.
>
...
- Dựa vào iteration order không được contract hoặc o(n) trong hot path → behavior chỉ sai ở scale/lifecycle edge.
...
- Chọn array/set/dictionary theo operation chủ đạo và domain.
...
**Hỏi:** UICollectionView và compositional layout giải quyết vấn đề gì?

**30-second:** Collection choice mã hóa ordering, uniqueness và lookup semantics. Chọn nó khi chọn Array/Set/Dictionary theo operation chủ đạo và domain; rủi ro chính là dựa vào iteration order không được contract hoặc O(n) trong hot path.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao dựa vào iteration order không được contract hoặc O(n) trong hot path phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **complexity measurement và invariant tests**.
...
Viết một ví dụ nhỏ minh họa **UICollectionView và compositional layout** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **dựa vào iteration order không được contract hoặc O(n) trong hot path** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **UICollectionView và compositional layout**. Thu evidence bằng complexity measurement và invariant tests, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → collection choice mã hóa ordering, uniqueness và lookup semantics
Use when  → chọn Array/Set/Dictionary theo operation chủ đạo và domain
Risk      → dựa vào iteration order không được contract hoặc O(n) trong hot path
Evidence  → complexity measurement và invariant tests
...
3. Usage rule: chọn Array/Set/Dictionary theo operation chủ đạo và domain.
4. Mistake nguy hiểm: dựa vào iteration order không được contract hoặc O(n) trong hot path.
5. Production lesson: kết luận bằng complexity measurement và invariant tests, rồi bảo vệ bằng test và metric.
...
- [Diffable Data Source và stable identity](09-diffable-data-source-va-stable-identity.md)
