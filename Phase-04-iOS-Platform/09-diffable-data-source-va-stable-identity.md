---
title: "Diffable Data Source và stable identity"
phase: "iOS Platform"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "diffable-data-source-va-stable-identity"
  - "global-commerce"
---
...
Trong Global Commerce, screen Commerce hiển thị sai, cập nhật sau khi dismiss hoặc giật vì identity/lifecycle bị hiểu sai. Chapter này tập trung vào **Diffable Data Source và stable identity**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [UICollectionView và compositional layout](08-uicollectionview-va-compositional-layout.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Delegate pattern trong UIKit](10-delegate-pattern-trong-uikit.md) dùng contract của chapter này làm building block.
...
Event/state mutation → lifecycle/observation → UI description/layout → visible frame
                 ↓
Focus: Diffable Data Source và stable identity
                 ↓
...
Trọng tâm của **Diffable Data Source và stable identity** là: reuse tách cell identity khỏi item identity; snapshot cần stable domain ID. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Diffable Data Source và stable identity?
...
UIKit vận hành qua object lifecycle; SwiftUI đánh giá description và reconcile phần UI bị ảnh hưởng, không đơn giản là vẽ lại mọi thứ. Với **Diffable Data Source và stable identity**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Diffable Data Source và stable identity** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Diffable Data Source và stable identity** là code an toàn.
>
...
**Hỏi:** Diffable Data Source và stable identity giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Diffable Data Source và stable identity** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Diffable Data Source và stable identity**. Thu evidence bằng scroll trace, reuse log và diffable snapshot audit, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Delegate pattern trong UIKit](10-delegate-pattern-trong-uikit.md)
