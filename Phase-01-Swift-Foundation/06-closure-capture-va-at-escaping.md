---
title: "Closure, capture và `@escaping`"
...
levels:
  - L1
prerequisites:
  - "Function, parameter label và method"
used_later:
  - "Enum và state modeling"
competencies:
  - "Swift Foundation"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "closure-capture-va-at-escaping"
  - "global-commerce"
---

# Closure, capture và `@escaping`
...
Trong Global Commerce, một thay đổi nhỏ trong model Commerce lan thành crash hoặc state không hợp lệ vì semantics của Swift bị hiểu sai. Chapter này tập trung vào **Closure, capture và `@escaping`**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Function, parameter label và method](05-function-parameter-label-va-method.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Enum và state modeling](07-enum-va-state-modeling.md) dùng contract của chapter này làm building block.
...
Input domain → type/operation phù hợp → compiler kiểm tra → state hợp lệ
                 ↓
Focus: Closure, capture và `@escaping`
                 ↓
...
Trọng tâm của **Closure, capture và `@escaping`** là: closure là behavior value có capture và lifetime. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Closure, capture và `@escaping`?
...
Theo dõi phần compiler kiểm tra tĩnh và phần behavior chỉ xuất hiện khi chương trình chạy. Với **Closure, capture và `@escaping`**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Closure, capture và `@escaping`** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Closure, capture và `@escaping`** là code an toàn.
>
...
**Hỏi:** Closure, capture và `@escaping` giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Closure, capture và `@escaping`** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Closure, capture và `@escaping`**. Thu evidence bằng ownership graph và thời điểm closure được release, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Function, parameter label và method](05-function-parameter-label-va-method.md)
- [Enum và state modeling](07-enum-va-state-modeling.md)
