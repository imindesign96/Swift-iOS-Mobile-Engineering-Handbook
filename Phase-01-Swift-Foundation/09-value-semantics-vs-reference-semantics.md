...
levels:
  - L1
prerequisites:
  - "Struct, class, equality và identity"
used_later:
  - "Property, initialization và access control"
competencies:
  - "Swift Foundation"
  - "Production"
  - "Debugging"
...
Trong Global Commerce, một thay đổi nhỏ trong model Commerce lan thành crash hoặc state không hợp lệ vì semantics của Swift bị hiểu sai. Chapter này tập trung vào **Value semantics vs reference semantics**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Struct, class, equality và identity](08-struct-class-equality-va-identity.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Property, initialization và access control](10-property-initialization-va-access-control.md) dùng contract của chapter này làm building block.
...
Input domain → type/operation phù hợp → compiler kiểm tra → state hợp lệ
                 ↓
Focus: Value semantics vs reference semantics
                 ↓
...
Trọng tâm của **Value semantics vs reference semantics** là: value mutation không làm thay đổi value độc lập; reference chia sẻ identity. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Value semantics vs reference semantics?
...
Theo dõi phần compiler kiểm tra tĩnh và phần behavior chỉ xuất hiện khi chương trình chạy. Với **Value semantics vs reference semantics**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Value semantics vs reference semantics** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Value semantics vs reference semantics** là code an toàn.
>
...
**Hỏi:** Value semantics vs reference semantics giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Value semantics vs reference semantics** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Value semantics vs reference semantics**. Thu evidence bằng mutation isolation test và profiling khi copy cost đáng kể, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Struct, class, equality và identity](08-struct-class-equality-va-identity.md)
- [Property, initialization và access control](10-property-initialization-va-access-control.md)
