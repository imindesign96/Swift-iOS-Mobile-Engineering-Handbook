...
Trong Global Commerce, một thay đổi nhỏ trong model Commerce lan thành crash hoặc state không hợp lệ vì semantics của Swift bị hiểu sai. Chapter này tập trung vào **Codable fundamentals**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [String, Unicode và indexing](17-string-unicode-va-indexing.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Phase Review: Swift Foundation](99-phase-review.md) dùng contract của chapter này làm building block.
...
Trọng tâm của **Codable fundamentals** là: Codable map schema bên ngoài vào model với boundary và evolution policy. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Codable fundamentals?
...
Theo dõi phần compiler kiểm tra tĩnh và phần behavior chỉ xuất hiện khi chương trình chạy. Với **Codable fundamentals**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Codable fundamentals** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Codable fundamentals** là code an toàn.
>
...
**Hỏi:** Codable fundamentals giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Codable fundamentals** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Codable fundamentals**. Thu evidence bằng fixture versioned và decode error path, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Phase Review: Swift Foundation](99-phase-review.md)
