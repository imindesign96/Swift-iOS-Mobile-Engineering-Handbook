---
title: "Property, initialization và access control"
...
competencies:
  ...
  - "Interview"
tags:
  - "property-initialization-va-access-control"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi nhỏ trong model Commerce lan thành crash hoặc state không hợp lệ vì semantics của Swift bị hiểu sai. Chapter này tập trung vào **Property, initialization và access control**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng object phải hoàn tất invariant trước khi được quan sát;
- nhận diện failure mode chính: partially initialized state hoặc public mutation quá rộng;
- chọn giải pháp bằng rule: để access hẹp nhất vẫn phục vụ collaboration/test;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng compiler initialization rules và API surface review;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Value semantics vs reference semantics](09-value-semantics-vs-reference-semantics.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Extension và tổ chức capability](11-extension-va-to-chuc-capability.md) dùng contract của chapter này làm building block.
...
Input domain → type/operation phù hợp → compiler kiểm tra → state hợp lệ
                 ↓
Focus: Property, initialization và access control
                 ↓
Evidence: compiler initialization rules và API surface review
```
...
Trọng tâm của **Property, initialization và access control** là: object phải hoàn tất invariant trước khi được quan sát. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng compiler initialization rules và API surface review.
...
Áp dụng khi để access hẹp nhất vẫn phục vụ collaboration/test. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là partially initialized state hoặc public mutation quá rộng. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Property, initialization và access control?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Partially initialized state hoặc public mutation quá rộng tạo evidence gì?
4. Rule chọn giải pháp là gì, và constraint nào khiến rule đổi?

## Runnable Swift Example

```swift
let focus = "Model domain bằng type và invariant"
print(focus)
```
...
Theo dõi phần compiler kiểm tra tĩnh và phần behavior chỉ xuất hiện khi chương trình chạy. Với **Property, initialization và access control**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Property, initialization và access control** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy partially initialized state hoặc public mutation quá rộng.
...
Dùng compiler initialization rules và API surface review; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **để access hẹp nhất vẫn phục vụ collaboration/test**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **compiler initialization rules và API surface review**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Property, initialization và access control** là code an toàn.
>
...
- Partially initialized state hoặc public mutation quá rộng → behavior chỉ sai ở scale/lifecycle edge.
...
- Để access hẹp nhất vẫn phục vụ collaboration/test.
...
**Hỏi:** Property, initialization và access control giải quyết vấn đề gì?

**30-second:** Object phải hoàn tất invariant trước khi được quan sát. Chọn nó khi để access hẹp nhất vẫn phục vụ collaboration/test; rủi ro chính là partially initialized state hoặc public mutation quá rộng.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao partially initialized state hoặc public mutation quá rộng phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **compiler initialization rules và API surface review**.
...
Viết một ví dụ nhỏ minh họa **Property, initialization và access control** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **partially initialized state hoặc public mutation quá rộng** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Property, initialization và access control**. Thu evidence bằng compiler initialization rules và API surface review, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → object phải hoàn tất invariant trước khi được quan sát
Use when  → để access hẹp nhất vẫn phục vụ collaboration/test
Risk      → partially initialized state hoặc public mutation quá rộng
Evidence  → compiler initialization rules và API surface review
...
3. Usage rule: để access hẹp nhất vẫn phục vụ collaboration/test.
4. Mistake nguy hiểm: partially initialized state hoặc public mutation quá rộng.
5. Production lesson: kết luận bằng compiler initialization rules và API surface review, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Value semantics vs reference semantics](09-value-semantics-vs-reference-semantics.md)
