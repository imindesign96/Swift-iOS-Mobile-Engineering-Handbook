---
title: "String, Unicode và indexing"
...
Trong Global Commerce, một thay đổi nhỏ trong model Commerce lan thành crash hoặc state không hợp lệ vì semantics của Swift bị hiểu sai. Chapter này tập trung vào **String, Unicode và indexing**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng String là collection của extended grapheme clusters, index không phải integer offset;
- nhận diện failure mode chính: index bằng byte/Int hoặc cắt Unicode sai boundary;
- chọn giải pháp bằng rule: dùng String.Index và normalization policy rõ khi persistence/search;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng test emoji, combining marks và locale-sensitive input;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Array, Set, Dictionary và collection semantics](16-array-set-dictionary-va-collection-semantics.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Codable fundamentals](18-codable-fundamentals.md) dùng contract của chapter này làm building block.
...
Input domain → type/operation phù hợp → compiler kiểm tra → state hợp lệ
                 ↓
Focus: String, Unicode và indexing
                 ↓
Evidence: test emoji, combining marks và locale-sensitive input
```
...
Trọng tâm của **String, Unicode và indexing** là: String là collection của extended grapheme clusters, index không phải integer offset. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng test emoji, combining marks và locale-sensitive input.
...
Áp dụng khi dùng String.Index và normalization policy rõ khi persistence/search. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là index bằng byte/Int hoặc cắt Unicode sai boundary. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa String, Unicode và indexing?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Index bằng byte/int hoặc cắt unicode sai boundary tạo evidence gì?
4. Rule chọn giải pháp là gì, và constraint nào khiến rule đổi?

## Runnable Swift Example

```swift
let name = "Cafe\u{301} ☕️"
...
Theo dõi phần compiler kiểm tra tĩnh và phần behavior chỉ xuất hiện khi chương trình chạy. Với **String, Unicode và indexing**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **String, Unicode và indexing** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy index bằng byte/Int hoặc cắt Unicode sai boundary.
...
Dùng test emoji, combining marks và locale-sensitive input; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **dùng String.Index và normalization policy rõ khi persistence/search**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **test emoji, combining marks và locale-sensitive input**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **String, Unicode và indexing** là code an toàn.
>
...
- Index bằng byte/int hoặc cắt unicode sai boundary → behavior chỉ sai ở scale/lifecycle edge.
...
- Dùng string.index và normalization policy rõ khi persistence/search.
...
**Hỏi:** String, Unicode và indexing giải quyết vấn đề gì?

**30-second:** String là collection của extended grapheme clusters, index không phải integer offset. Chọn nó khi dùng String.Index và normalization policy rõ khi persistence/search; rủi ro chính là index bằng byte/Int hoặc cắt Unicode sai boundary.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao index bằng byte/Int hoặc cắt Unicode sai boundary phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **test emoji, combining marks và locale-sensitive input**.
...
Viết một ví dụ nhỏ minh họa **String, Unicode và indexing** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **index bằng byte/Int hoặc cắt Unicode sai boundary** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **String, Unicode và indexing**. Thu evidence bằng test emoji, combining marks và locale-sensitive input, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → String là collection của extended grapheme clusters, index không phải integer offset
Use when  → dùng String.Index và normalization policy rõ khi persistence/search
Risk      → index bằng byte/Int hoặc cắt Unicode sai boundary
Evidence  → test emoji, combining marks và locale-sensitive input
...
3. Usage rule: dùng String.Index và normalization policy rõ khi persistence/search.
4. Mistake nguy hiểm: index bằng byte/Int hoặc cắt Unicode sai boundary.
5. Production lesson: kết luận bằng test emoji, combining marks và locale-sensitive input, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Array, Set, Dictionary và collection semantics](16-array-set-dictionary-va-collection-semantics.md)
- [Codable fundamentals](18-codable-fundamentals.md)
