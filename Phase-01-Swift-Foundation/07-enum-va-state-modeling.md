...
Trong Global Commerce, một thay đổi nhỏ trong model Commerce lan thành crash hoặc state không hợp lệ vì semantics của Swift bị hiểu sai. Chapter này tập trung vào **Enum và state modeling**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng enum mô hình hóa tập state hữu hạn và associated value đi cùng case;
- nhận diện failure mode chính: Boolean explosion tạo tổ hợp state bất hợp lệ;
- chọn giải pháp bằng rule: dùng enum khi state loại trừ nhau hoặc event mang payload khác nhau;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng switch exhaustive và transition test;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Closure, capture và `@escaping`](06-closure-capture-va-at-escaping.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Struct, class, equality và identity](08-struct-class-equality-va-identity.md) dùng contract của chapter này làm building block.
...
Input domain → type/operation phù hợp → compiler kiểm tra → state hợp lệ
                 ↓
Focus: Enum và state modeling
                 ↓
Evidence: switch exhaustive và transition test
```
...
Trọng tâm của **Enum và state modeling** là: enum mô hình hóa tập state hữu hạn và associated value đi cùng case. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng switch exhaustive và transition test.
...
Áp dụng khi dùng enum khi state loại trừ nhau hoặc event mang payload khác nhau. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là Boolean explosion tạo tổ hợp state bất hợp lệ. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Enum và state modeling?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Boolean explosion tạo tổ hợp state bất hợp lệ tạo evidence gì?
...
Theo dõi phần compiler kiểm tra tĩnh và phần behavior chỉ xuất hiện khi chương trình chạy. Với **Enum và state modeling**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Enum và state modeling** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy Boolean explosion tạo tổ hợp state bất hợp lệ.
...
Dùng switch exhaustive và transition test; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **dùng enum khi state loại trừ nhau hoặc event mang payload khác nhau**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **switch exhaustive và transition test**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Enum và state modeling** là code an toàn.
>
...
- Boolean explosion tạo tổ hợp state bất hợp lệ → behavior chỉ sai ở scale/lifecycle edge.
...
- Dùng enum khi state loại trừ nhau hoặc event mang payload khác nhau.
...
**Hỏi:** Enum và state modeling giải quyết vấn đề gì?

**30-second:** Enum mô hình hóa tập state hữu hạn và associated value đi cùng case. Chọn nó khi dùng enum khi state loại trừ nhau hoặc event mang payload khác nhau; rủi ro chính là Boolean explosion tạo tổ hợp state bất hợp lệ.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao Boolean explosion tạo tổ hợp state bất hợp lệ phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **switch exhaustive và transition test**.
...
Viết một ví dụ nhỏ minh họa **Enum và state modeling** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **Boolean explosion tạo tổ hợp state bất hợp lệ** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Enum và state modeling**. Thu evidence bằng switch exhaustive và transition test, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → enum mô hình hóa tập state hữu hạn và associated value đi cùng case
Use when  → dùng enum khi state loại trừ nhau hoặc event mang payload khác nhau
Risk      → Boolean explosion tạo tổ hợp state bất hợp lệ
Evidence  → switch exhaustive và transition test
...
3. Usage rule: dùng enum khi state loại trừ nhau hoặc event mang payload khác nhau.
4. Mistake nguy hiểm: Boolean explosion tạo tổ hợp state bất hợp lệ.
5. Production lesson: kết luận bằng switch exhaustive và transition test, rồi bảo vệ bằng test và metric.
