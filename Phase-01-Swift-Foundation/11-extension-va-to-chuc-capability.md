---
title: "Extension và tổ chức capability"
...
competencies:
  ...
  - "Interview"
tags:
  - "extension-va-to-chuc-capability"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi nhỏ trong model Commerce lan thành crash hoặc state không hợp lệ vì semantics của Swift bị hiểu sai. Chapter này tập trung vào **Extension và tổ chức capability**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng extension tổ chức capability quanh type mà không tạo owner mới;
- nhận diện failure mode chính: chia file theo syntax nhưng làm phân tán invariant;
- chọn giải pháp bằng rule: dùng extension cho conformance/capability gắn kết, không che god type;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng dependency direction và discoverability;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Property, initialization và access control](10-property-initialization-va-access-control.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Protocol và protocol-oriented design](12-protocol-va-protocol-oriented-design.md) dùng contract của chapter này làm building block.
...
Input domain → type/operation phù hợp → compiler kiểm tra → state hợp lệ
                 ↓
Focus: Extension và tổ chức capability
                 ↓
Evidence: dependency direction và discoverability
```
...
Trọng tâm của **Extension và tổ chức capability** là: extension tổ chức capability quanh type mà không tạo owner mới. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng dependency direction và discoverability.
...
Áp dụng khi dùng extension cho conformance/capability gắn kết, không che god type. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là chia file theo syntax nhưng làm phân tán invariant. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Extension và tổ chức capability?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Chia file theo syntax nhưng làm phân tán invariant tạo evidence gì?
...
Theo dõi phần compiler kiểm tra tĩnh và phần behavior chỉ xuất hiện khi chương trình chạy. Với **Extension và tổ chức capability**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Extension và tổ chức capability** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy chia file theo syntax nhưng làm phân tán invariant.
...
Dùng dependency direction và discoverability; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **dùng extension cho conformance/capability gắn kết, không che god type**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **dependency direction và discoverability**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Extension và tổ chức capability** là code an toàn.
>
...
- Chia file theo syntax nhưng làm phân tán invariant → behavior chỉ sai ở scale/lifecycle edge.
...
- Dùng extension cho conformance/capability gắn kết, không che god type.
...
**Hỏi:** Extension và tổ chức capability giải quyết vấn đề gì?

**30-second:** Extension tổ chức capability quanh type mà không tạo owner mới. Chọn nó khi dùng extension cho conformance/capability gắn kết, không che god type; rủi ro chính là chia file theo syntax nhưng làm phân tán invariant.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao chia file theo syntax nhưng làm phân tán invariant phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **dependency direction và discoverability**.
...
Viết một ví dụ nhỏ minh họa **Extension và tổ chức capability** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **chia file theo syntax nhưng làm phân tán invariant** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Extension và tổ chức capability**. Thu evidence bằng dependency direction và discoverability, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → extension tổ chức capability quanh type mà không tạo owner mới
Use when  → dùng extension cho conformance/capability gắn kết, không che god type
Risk      → chia file theo syntax nhưng làm phân tán invariant
Evidence  → dependency direction và discoverability
...
3. Usage rule: dùng extension cho conformance/capability gắn kết, không che god type.
4. Mistake nguy hiểm: chia file theo syntax nhưng làm phân tán invariant.
5. Production lesson: kết luận bằng dependency direction và discoverability, rồi bảo vệ bằng test và metric.
