---
title: "Error handling, `throws` và `Result`"
...
competencies:
  ...
  - "Interview"
tags:
  - "error-handling-throws-va-result"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi nhỏ trong model Commerce lan thành crash hoặc state không hợp lệ vì semantics của Swift bị hiểu sai. Chapter này tập trung vào **Error handling, `throws` và `Result`**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng failure là part của signature và cần taxonomy/recovery rõ;
- nhận diện failure mode chính: try! hoặc gom mọi lỗi thành unknown/string;
- chọn giải pháp bằng rule: throws cho control flow trực tiếp; Result khi cần lưu/truyền outcome như value;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng test từng error category và recovery path;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [`some` vs `any`: opaque và existential types](14-some-vs-any-opaque-va-existential-types.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Array, Set, Dictionary và collection semantics](16-array-set-dictionary-va-collection-semantics.md) dùng contract của chapter này làm building block.
...
Input domain → type/operation phù hợp → compiler kiểm tra → state hợp lệ
                 ↓
Focus: Error handling, `throws` và `Result`
                 ↓
Evidence: test từng error category và recovery path
```
...
Trọng tâm của **Error handling, `throws` và `Result`** là: failure là part của signature và cần taxonomy/recovery rõ. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng test từng error category và recovery path.
...
Áp dụng khi throws cho control flow trực tiếp; Result khi cần lưu/truyền outcome như value. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là try! hoặc gom mọi lỗi thành unknown/string. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Error handling, `throws` và `Result`?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Try! hoặc gom mọi lỗi thành unknown/string tạo evidence gì?
...
Theo dõi phần compiler kiểm tra tĩnh và phần behavior chỉ xuất hiện khi chương trình chạy. Với **Error handling, `throws` và `Result`**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Error handling, `throws` và `Result`** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy try! hoặc gom mọi lỗi thành unknown/string.
...
Dùng test từng error category và recovery path; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **throws cho control flow trực tiếp; Result khi cần lưu/truyền outcome như value**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **test từng error category và recovery path**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Error handling, `throws` và `Result`** là code an toàn.
>
...
- Try! hoặc gom mọi lỗi thành unknown/string → behavior chỉ sai ở scale/lifecycle edge.
...
- Throws cho control flow trực tiếp; result khi cần lưu/truyền outcome như value.
...
**Hỏi:** Error handling, `throws` và `Result` giải quyết vấn đề gì?

**30-second:** Failure là part của signature và cần taxonomy/recovery rõ. Chọn nó khi throws cho control flow trực tiếp; Result khi cần lưu/truyền outcome như value; rủi ro chính là try! hoặc gom mọi lỗi thành unknown/string.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao try! hoặc gom mọi lỗi thành unknown/string phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **test từng error category và recovery path**.
...
Viết một ví dụ nhỏ minh họa **Error handling, `throws` và `Result`** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **try! hoặc gom mọi lỗi thành unknown/string** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Error handling, `throws` và `Result`**. Thu evidence bằng test từng error category và recovery path, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → failure là part của signature và cần taxonomy/recovery rõ
Use when  → throws cho control flow trực tiếp; Result khi cần lưu/truyền outcome như value
Risk      → try! hoặc gom mọi lỗi thành unknown/string
Evidence  → test từng error category và recovery path
...
3. Usage rule: throws cho control flow trực tiếp; Result khi cần lưu/truyền outcome như value.
4. Mistake nguy hiểm: try! hoặc gom mọi lỗi thành unknown/string.
5. Production lesson: kết luận bằng test từng error category và recovery path, rồi bảo vệ bằng test và metric.
