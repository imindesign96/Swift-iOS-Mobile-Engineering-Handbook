---
title: "Protocol và protocol-oriented design"
...
levels:
  - L1
prerequisites:
  - "Extension và tổ chức capability"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "protocol-va-protocol-oriented-design"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi nhỏ trong model Commerce lan thành crash hoặc state không hợp lệ vì semantics của Swift bị hiểu sai. Chapter này tập trung vào **Protocol và protocol-oriented design**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Extension và tổ chức capability](11-extension-va-to-chuc-capability.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Generics, constraint và `associatedtype`](13-generics-constraint-va-associatedtype.md) dùng contract của chapter này làm building block.
...
Input domain → type/operation phù hợp → compiler kiểm tra → state hợp lệ
                 ↓
Focus: Protocol và protocol-oriented design
                 ↓
...
Trọng tâm của **Protocol và protocol-oriented design** là: protocol mô tả capability/contract; generic giữ type relationship tại compile time. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Protocol và protocol-oriented design?
...
Theo dõi phần compiler kiểm tra tĩnh và phần behavior chỉ xuất hiện khi chương trình chạy. Với **Protocol và protocol-oriented design**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Protocol và protocol-oriented design** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Protocol và protocol-oriented design** là code an toàn.
>
...
**Hỏi:** Protocol và protocol-oriented design giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Protocol và protocol-oriented design** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Protocol và protocol-oriented design**. Thu evidence bằng call-site substitutability và compile-time constraints, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Extension và tổ chức capability](11-extension-va-to-chuc-capability.md)
