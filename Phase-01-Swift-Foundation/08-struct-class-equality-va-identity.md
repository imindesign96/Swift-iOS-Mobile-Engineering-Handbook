---
title: "Struct, class, equality và identity"
...
competencies:
  ...
  - "Interview"
tags:
  - "struct-class-equality-va-identity"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi nhỏ trong model Commerce lan thành crash hoặc state không hợp lệ vì semantics của Swift bị hiểu sai. Chapter này tập trung vào **Struct, class, equality và identity**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng struct diễn tả value; class diễn tả shared identity/lifecycle;
- nhận diện failure mode chính: chọn theo performance myth thay vì semantics;
- chọn giải pháp bằng rule: bắt đầu từ domain: value độc lập hay một thực thể được chia sẻ;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng == cho equality và === cho object identity;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Enum và state modeling](07-enum-va-state-modeling.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Value semantics vs reference semantics](09-value-semantics-vs-reference-semantics.md) dùng contract của chapter này làm building block.
...
Input domain → type/operation phù hợp → compiler kiểm tra → state hợp lệ
                 ↓
Focus: Struct, class, equality và identity
                 ↓
Evidence: == cho equality và === cho object identity
```
...
Trọng tâm của **Struct, class, equality và identity** là: struct diễn tả value; class diễn tả shared identity/lifecycle. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng == cho equality và === cho object identity.
...
Áp dụng khi bắt đầu từ domain: value độc lập hay một thực thể được chia sẻ. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là chọn theo performance myth thay vì semantics. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Struct, class, equality và identity?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Chọn theo performance myth thay vì semantics tạo evidence gì?
...
Theo dõi phần compiler kiểm tra tĩnh và phần behavior chỉ xuất hiện khi chương trình chạy. Với **Struct, class, equality và identity**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Struct, class, equality và identity** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy chọn theo performance myth thay vì semantics.
...
Dùng == cho equality và === cho object identity; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **bắt đầu từ domain: value độc lập hay một thực thể được chia sẻ**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **== cho equality và === cho object identity**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Struct, class, equality và identity** là code an toàn.
>
...
- Chọn theo performance myth thay vì semantics → behavior chỉ sai ở scale/lifecycle edge.
...
- Bắt đầu từ domain: value độc lập hay một thực thể được chia sẻ.
...
**Hỏi:** Struct, class, equality và identity giải quyết vấn đề gì?

**30-second:** Struct diễn tả value; class diễn tả shared identity/lifecycle. Chọn nó khi bắt đầu từ domain: value độc lập hay một thực thể được chia sẻ; rủi ro chính là chọn theo performance myth thay vì semantics.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao chọn theo performance myth thay vì semantics phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **== cho equality và === cho object identity**.
...
Viết một ví dụ nhỏ minh họa **Struct, class, equality và identity** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **chọn theo performance myth thay vì semantics** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Struct, class, equality và identity**. Thu evidence bằng == cho equality và === cho object identity, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → struct diễn tả value; class diễn tả shared identity/lifecycle
Use when  → bắt đầu từ domain: value độc lập hay một thực thể được chia sẻ
Risk      → chọn theo performance myth thay vì semantics
Evidence  → == cho equality và === cho object identity
...
3. Usage rule: bắt đầu từ domain: value độc lập hay một thực thể được chia sẻ.
4. Mistake nguy hiểm: chọn theo performance myth thay vì semantics.
5. Production lesson: kết luận bằng == cho equality và === cho object identity, rồi bảo vệ bằng test và metric.
