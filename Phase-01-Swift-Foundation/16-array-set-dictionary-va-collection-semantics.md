---
title: "Array, Set, Dictionary và collection semantics"
...
competencies:
  ...
  - "Interview"
tags:
  - "array-set-dictionary-va-collection-semantics"
  - "global-commerce"
---

# Array, Set, Dictionary và collection semantics
...
Trong Global Commerce, một thay đổi nhỏ trong model Commerce lan thành crash hoặc state không hợp lệ vì semantics của Swift bị hiểu sai. Chapter này tập trung vào **Array, Set, Dictionary và collection semantics**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Error handling, `throws` và `Result`](15-error-handling-throws-va-result.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [String, Unicode và indexing](17-string-unicode-va-indexing.md) dùng contract của chapter này làm building block.
...
Input domain → type/operation phù hợp → compiler kiểm tra → state hợp lệ
                 ↓
Focus: Array, Set, Dictionary và collection semantics
                 ↓
...
Trọng tâm của **Array, Set, Dictionary và collection semantics** là: collection choice mã hóa ordering, uniqueness và lookup semantics. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Array, Set, Dictionary và collection semantics?
...
Theo dõi phần compiler kiểm tra tĩnh và phần behavior chỉ xuất hiện khi chương trình chạy. Với **Array, Set, Dictionary và collection semantics**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Array, Set, Dictionary và collection semantics** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Array, Set, Dictionary và collection semantics** là code an toàn.
>
...
**Hỏi:** Array, Set, Dictionary và collection semantics giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Array, Set, Dictionary và collection semantics** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Array, Set, Dictionary và collection semantics**. Thu evidence bằng complexity measurement và invariant tests, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [String, Unicode và indexing](17-string-unicode-va-indexing.md)
