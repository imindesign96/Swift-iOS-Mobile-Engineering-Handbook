---
title: "Control flow, `switch` và pattern matching"
...
competencies:
  ...
  - "Interview"
tags:
  - "control-flow-switch-va-pattern-matching"
  - "global-commerce"
---

# Control flow, `switch` và pattern matching
...
Trong Global Commerce, một thay đổi nhỏ trong model Commerce lan thành crash hoặc state không hợp lệ vì semantics của Swift bị hiểu sai. Chapter này tập trung vào **Control flow, `switch` và pattern matching**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng control flow nên làm domain cases exhaustive và dễ audit;
- nhận diện failure mode chính: default che case mới hoặc nested if làm mất invariant;
- chọn giải pháp bằng rule: ưu tiên switch/pattern khi các case domain loại trừ nhau;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng branch coverage và compiler exhaustiveness;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Optional và nil safety](03-optionals-and-nil-safety.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Function, parameter label và method](05-function-parameter-label-va-method.md) dùng contract của chapter này làm building block.
...
Input domain → type/operation phù hợp → compiler kiểm tra → state hợp lệ
                 ↓
Focus: Control flow, `switch` và pattern matching
                 ↓
Evidence: branch coverage và compiler exhaustiveness
```
...
Trọng tâm của **Control flow, `switch` và pattern matching** là: control flow nên làm domain cases exhaustive và dễ audit. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng branch coverage và compiler exhaustiveness.
...
Áp dụng khi ưu tiên switch/pattern khi các case domain loại trừ nhau. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là default che case mới hoặc nested if làm mất invariant. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Control flow, `switch` và pattern matching?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Default che case mới hoặc nested if làm mất invariant tạo evidence gì?
...
Theo dõi phần compiler kiểm tra tĩnh và phần behavior chỉ xuất hiện khi chương trình chạy. Với **Control flow, `switch` và pattern matching**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Control flow, `switch` và pattern matching** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy default che case mới hoặc nested if làm mất invariant.
...
Dùng branch coverage và compiler exhaustiveness; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **ưu tiên switch/pattern khi các case domain loại trừ nhau**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **branch coverage và compiler exhaustiveness**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Control flow, `switch` và pattern matching** là code an toàn.
>
...
- Default che case mới hoặc nested if làm mất invariant → behavior chỉ sai ở scale/lifecycle edge.
...
- Ưu tiên switch/pattern khi các case domain loại trừ nhau.
...
**Hỏi:** Control flow, `switch` và pattern matching giải quyết vấn đề gì?

**30-second:** Control flow nên làm domain cases exhaustive và dễ audit. Chọn nó khi ưu tiên switch/pattern khi các case domain loại trừ nhau; rủi ro chính là default che case mới hoặc nested if làm mất invariant.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao default che case mới hoặc nested if làm mất invariant phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **branch coverage và compiler exhaustiveness**.
...
Viết một ví dụ nhỏ minh họa **Control flow, `switch` và pattern matching** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **default che case mới hoặc nested if làm mất invariant** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Control flow, `switch` và pattern matching**. Thu evidence bằng branch coverage và compiler exhaustiveness, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → control flow nên làm domain cases exhaustive và dễ audit
Use when  → ưu tiên switch/pattern khi các case domain loại trừ nhau
Risk      → default che case mới hoặc nested if làm mất invariant
Evidence  → branch coverage và compiler exhaustiveness
...
3. Usage rule: ưu tiên switch/pattern khi các case domain loại trừ nhau.
4. Mistake nguy hiểm: default che case mới hoặc nested if làm mất invariant.
5. Production lesson: kết luận bằng branch coverage và compiler exhaustiveness, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Optional và nil safety](03-optionals-and-nil-safety.md)
