---
title: "Generics, constraint và `associatedtype`"
...
levels:
  - L1
prerequisites:
  - "Protocol và protocol-oriented design"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "generics-constraint-va-associatedtype"
  - "global-commerce"
---
...
Trong Global Commerce, một thay đổi nhỏ trong model Commerce lan thành crash hoặc state không hợp lệ vì semantics của Swift bị hiểu sai. Chapter này tập trung vào **Generics, constraint và `associatedtype`**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Protocol và protocol-oriented design](12-protocol-va-protocol-oriented-design.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [`some` vs `any`: opaque và existential types](14-some-vs-any-opaque-va-existential-types.md) dùng contract của chapter này làm building block.
...
Input domain → type/operation phù hợp → compiler kiểm tra → state hợp lệ
                 ↓
Focus: Generics, constraint và `associatedtype`
                 ↓
...
Trọng tâm của **Generics, constraint và `associatedtype`** là: protocol mô tả capability/contract; generic giữ type relationship tại compile time. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Generics, constraint và `associatedtype`?
...
Theo dõi phần compiler kiểm tra tĩnh và phần behavior chỉ xuất hiện khi chương trình chạy. Với **Generics, constraint và `associatedtype`**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Generics, constraint và `associatedtype`** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Generics, constraint và `associatedtype`** là code an toàn.
>
...
**Hỏi:** Generics, constraint và `associatedtype` giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Generics, constraint và `associatedtype`** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Generics, constraint và `associatedtype`**. Thu evidence bằng call-site substitutability và compile-time constraints, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Protocol và protocol-oriented design](12-protocol-va-protocol-oriented-design.md)
- [`some` vs `any`: opaque và existential types](14-some-vs-any-opaque-va-existential-types.md)
