---
title: "SwiftData vs Core Data theo constraint"
phase: "Persistence"
difficulty: 5
importance: 5
...
Trong Global Commerce, Commerce hiển thị dữ liệu account cũ hoặc mất cart offline vì storage được chọn theo thói quen thay vì policy. Chapter này tập trung vào **SwiftData vs Core Data theo constraint**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [SwiftData và ranh giới availability](08-swiftdata-va-ranh-gioi-availability.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Migration strategy và rollback thinking](10-migration-strategy-va-rollback-thinking.md) dùng contract của chapter này làm building block.
...
Domain intent → repository → local transaction/source of truth ↔ synchronization policy
                 ↓
Focus: SwiftData vs Core Data theo constraint
                 ↓
...
Trọng tâm của **SwiftData vs Core Data theo constraint** là: generic/opaque giữ concrete relationship; existential chứa một value bất kỳ conform protocol. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa SwiftData vs Core Data theo constraint?
...
Disk I/O, context/transaction và migration có lifecycle riêng; synchronous work sai chỗ có thể block UI. Với **SwiftData vs Core Data theo constraint**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **SwiftData vs Core Data theo constraint** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **SwiftData vs Core Data theo constraint** là code an toàn.
>
...
**Hỏi:** SwiftData vs Core Data theo constraint giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **SwiftData vs Core Data theo constraint** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **SwiftData vs Core Data theo constraint**. Thu evidence bằng compiler diagnostics, specialization/code-size measurement, vẽ owner/state graph, xác định root cause và thêm regression test.
