---
title: "Persistent cache và invalidation"
phase: "Persistence"
difficulty: 5
importance: 5
...
levels:
  - L4
prerequisites:
  - "Migration strategy và rollback thinking"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "persistent-cache-va-invalidation"
  - "global-commerce"
---

# Persistent cache và invalidation
...
Trong Global Commerce, Commerce hiển thị dữ liệu account cũ hoặc mất cart offline vì storage được chọn theo thói quen thay vì policy. Chapter này tập trung vào **Persistent cache và invalidation**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng Persistent cache và invalidation phải được hiểu bằng behavior, constraint và trade-off quan sát được;
...
- [Migration strategy và rollback thinking](10-migration-strategy-va-rollback-thinking.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Offline-first synchronization](12-offline-first-synchronization.md) dùng contract của chapter này làm building block.
...
Domain intent → repository → local transaction/source of truth ↔ synchronization policy
                 ↓
Focus: Persistent cache và invalidation
                 ↓
...
Trọng tâm của **Persistent cache và invalidation** là: Persistent cache và invalidation phải được hiểu bằng behavior, constraint và trade-off quan sát được. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Persistent cache và invalidation?
...
Disk I/O, context/transaction và migration có lifecycle riêng; synchronous work sai chỗ có thể block UI. Với **Persistent cache và invalidation**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Persistent cache và invalidation** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Persistent cache và invalidation** là code an toàn.
>
...
Viết một ví dụ nhỏ minh họa **Persistent cache và invalidation** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Persistent cache và invalidation**. Thu evidence bằng Database inspection, transaction logs, migration tests, file protection checks và memory/disk metrics., vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → Persistent cache và invalidation phải được hiểu bằng behavior, constraint và trade-off quan sát được
...
- [Migration strategy và rollback thinking](10-migration-strategy-va-rollback-thinking.md)
