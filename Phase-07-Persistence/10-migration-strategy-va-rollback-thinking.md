---
title: "Migration strategy và rollback thinking"
phase: "Persistence"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "migration-strategy-va-rollback-thinking"
  - "global-commerce"
---
...
Trong Global Commerce, Commerce hiển thị dữ liệu account cũ hoặc mất cart offline vì storage được chọn theo thói quen thay vì policy. Chapter này tập trung vào **Migration strategy và rollback thinking**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [SwiftData vs Core Data theo constraint](09-swiftdata-vs-core-data-theo-constraint.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Persistent cache và invalidation](11-persistent-cache-va-invalidation.md) dùng contract của chapter này làm building block.
...
Domain intent → repository → local transaction/source of truth ↔ synchronization policy
                 ↓
Focus: Migration strategy và rollback thinking
                 ↓
...
Trọng tâm của **Migration strategy và rollback thinking** là: migration tốt có seam, slice nhỏ, metric và rollback. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Migration strategy và rollback thinking?
...
Disk I/O, context/transaction và migration có lifecycle riêng; synchronous work sai chỗ có thể block UI. Với **Migration strategy và rollback thinking**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Migration strategy và rollback thinking** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Migration strategy và rollback thinking** là code an toàn.
>
...
**Hỏi:** Migration strategy và rollback thinking giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Migration strategy và rollback thinking** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Migration strategy và rollback thinking**. Thu evidence bằng parity tests, adoption metric và defect rate, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Persistent cache và invalidation](11-persistent-cache-va-invalidation.md)
