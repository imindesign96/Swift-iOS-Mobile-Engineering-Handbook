---
title: "UserDefaults: preference, không phải database"
phase: "Persistence"
difficulty: 4
importance: 5
...
levels:
  - L4
prerequisites:
  - "Chọn storage theo data, lifetime và security"
used_later:
...
competencies:
  ...
  - "Interview"
tags:
  - "userdefaults-preference-khong-phai-database"
  - "global-commerce"
---

# UserDefaults: preference, không phải database
...
Trong Global Commerce, Commerce hiển thị dữ liệu account cũ hoặc mất cart offline vì storage được chọn theo thói quen thay vì policy. Chapter này tập trung vào **UserDefaults: preference, không phải database**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng storage được chọn theo data shape, lifetime, query, security và migration;
- nhận diện failure mode chính: dùng preference store như database hoặc lưu token plaintext;
- chọn giải pháp bằng rule: tách storage adapter sau repository và xác định source of truth;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng migration/read-write/failure tests;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Chọn storage theo data, lifetime và security](01-chon-storage-theo-data-lifetime-va-security.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Keychain và sensitive data lifecycle](03-keychain-va-sensitive-data-lifecycle.md) dùng contract của chapter này làm building block.
...
Domain intent → repository → local transaction/source of truth ↔ synchronization policy
                 ↓
Focus: UserDefaults: preference, không phải database
                 ↓
Evidence: migration/read-write/failure tests
```
...
Trọng tâm của **UserDefaults: preference, không phải database** là: storage được chọn theo data shape, lifetime, query, security và migration. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
Concept tồn tại để giúp chọn storage/source of truth theo lifetime, query, security và synchronization requirement. Không có boundary này, rule domain bị nhân bản, behavior phụ thuộc call order và incident khó điều tra vì không biết layer nào chịu trách nhiệm.
...
5. Đo observable behavior bằng migration/read-write/failure tests.
...
Áp dụng khi tách storage adapter sau repository và xác định source of truth. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là dùng preference store như database hoặc lưu token plaintext. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa UserDefaults: preference, không phải database?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Dùng preference store như database hoặc lưu token plaintext tạo evidence gì?
...
Disk I/O, context/transaction và migration có lifecycle riêng; synchronous work sai chỗ có thể block UI. Với **UserDefaults: preference, không phải database**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Context/object graph/cache giữ resident data; fetch giới hạn và fault/batch strategy cần phù hợp workload.
...
Context/model access phải tuân isolation; sync cần ordering, idempotency và conflict policy.
...
Storage model không nên rò trực tiếp qua UI; repository map và bảo vệ account/data boundary. Dependency hướng vào contract ổn định; implementation details nằm phía ngoài. Composition root tạo concrete dependencies và quyết định lifetime.
...
Feature Commerce áp dụng **UserDefaults: preference, không phải database** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy dùng preference store như database hoặc lưu token plaintext.
...
Dùng migration/read-write/failure tests; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **tách storage adapter sau repository và xác định source of truth**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **migration/read-write/failure tests**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **UserDefaults: preference, không phải database** là code an toàn.
>
...
- Dùng preference store như database hoặc lưu token plaintext → behavior chỉ sai ở scale/lifecycle edge.
...
- Tách storage adapter sau repository và xác định source of truth.
...
**Hỏi:** UserDefaults: preference, không phải database giải quyết vấn đề gì?

**30-second:** Storage được chọn theo data shape, lifetime, query, security và migration. Chọn nó khi tách storage adapter sau repository và xác định source of truth; rủi ro chính là dùng preference store như database hoặc lưu token plaintext.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao dùng preference store như database hoặc lưu token plaintext phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **migration/read-write/failure tests**.
...
Viết một ví dụ nhỏ minh họa **UserDefaults: preference, không phải database** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **dùng preference store như database hoặc lưu token plaintext** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **UserDefaults: preference, không phải database**. Thu evidence bằng migration/read-write/failure tests, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → storage được chọn theo data shape, lifetime, query, security và migration
Use when  → tách storage adapter sau repository và xác định source of truth
Risk      → dùng preference store như database hoặc lưu token plaintext
Evidence  → migration/read-write/failure tests
...
2. Mental model: Domain intent → repository → local transaction/source of truth ↔ synchronization policy.
3. Usage rule: tách storage adapter sau repository và xác định source of truth.
4. Mistake nguy hiểm: dùng preference store như database hoặc lưu token plaintext.
5. Production lesson: kết luận bằng migration/read-write/failure tests, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Chọn storage theo data, lifetime và security](01-chon-storage-theo-data-lifetime-va-security.md)
- [Keychain và sensitive data lifecycle](03-keychain-va-sensitive-data-lifecycle.md)
