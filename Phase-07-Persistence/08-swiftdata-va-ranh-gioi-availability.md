---
title: "SwiftData và ranh giới availability"
phase: "Persistence"
difficulty: 5
importance: 5
...
levels:
  - L4
prerequisites:
  - "Managed Object Context và Core Data concurrency"
used_later:
  - "SwiftData vs Core Data theo constraint"
competencies:
  - "Persistence"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "swiftdata-va-ranh-gioi-availability"
  - "global-commerce"
---

# SwiftData và ranh giới availability
...
Trong Global Commerce, Commerce hiển thị dữ liệu account cũ hoặc mất cart offline vì storage được chọn theo thói quen thay vì policy. Chapter này tập trung vào **SwiftData và ranh giới availability**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [Managed Object Context và Core Data concurrency](07-managed-object-context-va-core-data-concurrency.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [SwiftData vs Core Data theo constraint](09-swiftdata-vs-core-data-theo-constraint.md) dùng contract của chapter này làm building block.
...
Domain intent → repository → local transaction/source of truth ↔ synchronization policy
                 ↓
Focus: SwiftData và ranh giới availability
                 ↓
...
Trọng tâm của **SwiftData và ranh giới availability** là: storage được chọn theo data shape, lifetime, query, security và migration. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa SwiftData và ranh giới availability?
...
Disk I/O, context/transaction và migration có lifecycle riêng; synchronous work sai chỗ có thể block UI. Với **SwiftData và ranh giới availability**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **SwiftData và ranh giới availability** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **SwiftData và ranh giới availability** là code an toàn.
>
...
**Hỏi:** SwiftData và ranh giới availability giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **SwiftData và ranh giới availability** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **SwiftData và ranh giới availability**. Thu evidence bằng migration/read-write/failure tests, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [Managed Object Context và Core Data concurrency](07-managed-object-context-va-core-data-concurrency.md)
- [SwiftData vs Core Data theo constraint](09-swiftdata-vs-core-data-theo-constraint.md)
