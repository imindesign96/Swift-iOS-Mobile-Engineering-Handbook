---
title: "Keychain và sensitive data lifecycle"
phase: "Persistence"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "keychain-va-sensitive-data-lifecycle"
  - "global-commerce"
---
...
Trong Global Commerce, Commerce hiển thị dữ liệu account cũ hoặc mất cart offline vì storage được chọn theo thói quen thay vì policy. Chapter này tập trung vào **Keychain và sensitive data lifecycle**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [UserDefaults: preference, không phải database](02-userdefaults-preference-khong-phai-database.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [FileManager, atomic write và file coordination](04-filemanager-atomic-write-va-file-coordination.md) dùng contract của chapter này làm building block.
...
Domain intent → repository → local transaction/source of truth ↔ synchronization policy
                 ↓
Focus: Keychain và sensitive data lifecycle
                 ↓
...
Trọng tâm của **Keychain và sensitive data lifecycle** là: storage được chọn theo data shape, lifetime, query, security và migration. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Keychain và sensitive data lifecycle?
...
Disk I/O, context/transaction và migration có lifecycle riêng; synchronous work sai chỗ có thể block UI. Với **Keychain và sensitive data lifecycle**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Keychain và sensitive data lifecycle** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Keychain và sensitive data lifecycle** là code an toàn.
>
...
**Hỏi:** Keychain và sensitive data lifecycle giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Keychain và sensitive data lifecycle** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Keychain và sensitive data lifecycle**. Thu evidence bằng migration/read-write/failure tests, vẽ owner/state graph, xác định root cause và thêm regression test.
...
- [UserDefaults: preference, không phải database](02-userdefaults-preference-khong-phai-database.md)
- [FileManager, atomic write và file coordination](04-filemanager-atomic-write-va-file-coordination.md)
