---
title: "Chọn storage theo data, lifetime và security"
phase: "Persistence"
difficulty: 4
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "chon-storage-theo-data-lifetime-va-security"
  - "global-commerce"
---
...
Trong Global Commerce, Commerce hiển thị dữ liệu account cũ hoặc mất cart offline vì storage được chọn theo thói quen thay vì policy. Chapter này tập trung vào **Chọn storage theo data, lifetime và security**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng transport security bảo vệ channel; pinning thêm operational risk;
- nhận diện failure mode chính: tắt ATS rộng hoặc pin không có rotation plan;
- chọn giải pháp bằng rule: dùng platform trust mặc định; pin chỉ khi threat model và rotation justify;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng trust failure telemetry không chứa secret;
...
- [UserDefaults: preference, không phải database](02-userdefaults-preference-khong-phai-database.md) dùng contract của chapter này làm building block.
...
Domain intent → repository → local transaction/source of truth ↔ synchronization policy
                 ↓
Focus: Chọn storage theo data, lifetime và security
                 ↓
Evidence: trust failure telemetry không chứa secret
```
...
Trọng tâm của **Chọn storage theo data, lifetime và security** là: transport security bảo vệ channel; pinning thêm operational risk. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng trust failure telemetry không chứa secret.
...
Áp dụng khi dùng platform trust mặc định; pin chỉ khi threat model và rotation justify. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là tắt ATS rộng hoặc pin không có rotation plan. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Chọn storage theo data, lifetime và security?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Tắt ats rộng hoặc pin không có rotation plan tạo evidence gì?
...
Disk I/O, context/transaction và migration có lifecycle riêng; synchronous work sai chỗ có thể block UI. Với **Chọn storage theo data, lifetime và security**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Chọn storage theo data, lifetime và security** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy tắt ATS rộng hoặc pin không có rotation plan.
...
Dùng trust failure telemetry không chứa secret; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **dùng platform trust mặc định; pin chỉ khi threat model và rotation justify**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **trust failure telemetry không chứa secret**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Chọn storage theo data, lifetime và security** là code an toàn.
>
...
- Tắt ats rộng hoặc pin không có rotation plan → behavior chỉ sai ở scale/lifecycle edge.
...
- Dùng platform trust mặc định; pin chỉ khi threat model và rotation justify.
...
**Hỏi:** Chọn storage theo data, lifetime và security giải quyết vấn đề gì?

**30-second:** Transport security bảo vệ channel; pinning thêm operational risk. Chọn nó khi dùng platform trust mặc định; pin chỉ khi threat model và rotation justify; rủi ro chính là tắt ATS rộng hoặc pin không có rotation plan.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao tắt ATS rộng hoặc pin không có rotation plan phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **trust failure telemetry không chứa secret**.
...
Viết một ví dụ nhỏ minh họa **Chọn storage theo data, lifetime và security** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **tắt ATS rộng hoặc pin không có rotation plan** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Chọn storage theo data, lifetime và security**. Thu evidence bằng trust failure telemetry không chứa secret, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → transport security bảo vệ channel; pinning thêm operational risk
Use when  → dùng platform trust mặc định; pin chỉ khi threat model và rotation justify
Risk      → tắt ATS rộng hoặc pin không có rotation plan
Evidence  → trust failure telemetry không chứa secret
...
3. Usage rule: dùng platform trust mặc định; pin chỉ khi threat model và rotation justify.
4. Mistake nguy hiểm: tắt ATS rộng hoặc pin không có rotation plan.
5. Production lesson: kết luận bằng trust failure telemetry không chứa secret, rồi bảo vệ bằng test và metric.
