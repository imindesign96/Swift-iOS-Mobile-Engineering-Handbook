...
Trong Global Commerce, Commerce hiển thị dữ liệu account cũ hoặc mất cart offline vì storage được chọn theo thói quen thay vì policy. Chapter này tập trung vào **Offline-first synchronization**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng cache cần key, freshness, validation, invalidation và source-of-truth policy;
- nhận diện failure mode chính: TTL tùy tiện hoặc stale overwrite fresh;
- chọn giải pháp bằng rule: chọn cache-first/network-first/offline-first theo UX consistency;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng hit rate, age, revalidation và generation;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Persistent cache và invalidation](11-persistent-cache-va-invalidation.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Conflict detection và resolution](13-conflict-detection-va-resolution.md) dùng contract của chapter này làm building block.
...
Domain intent → repository → local transaction/source of truth ↔ synchronization policy
                 ↓
Focus: Offline-first synchronization
                 ↓
Evidence: hit rate, age, revalidation và generation
```
...
Trọng tâm của **Offline-first synchronization** là: cache cần key, freshness, validation, invalidation và source-of-truth policy. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng hit rate, age, revalidation và generation.
...
Áp dụng khi chọn cache-first/network-first/offline-first theo UX consistency. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là TTL tùy tiện hoặc stale overwrite fresh. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Offline-first synchronization?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Ttl tùy tiện hoặc stale overwrite fresh tạo evidence gì?
...
Disk I/O, context/transaction và migration có lifecycle riêng; synchronous work sai chỗ có thể block UI. Với **Offline-first synchronization**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Offline-first synchronization** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy TTL tùy tiện hoặc stale overwrite fresh.
...
Dùng hit rate, age, revalidation và generation; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **chọn cache-first/network-first/offline-first theo UX consistency**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **hit rate, age, revalidation và generation**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Offline-first synchronization** là code an toàn.
>
...
- Ttl tùy tiện hoặc stale overwrite fresh → behavior chỉ sai ở scale/lifecycle edge.
...
- Chọn cache-first/network-first/offline-first theo ux consistency.
...
**Hỏi:** Offline-first synchronization giải quyết vấn đề gì?

**30-second:** Cache cần key, freshness, validation, invalidation và source-of-truth policy. Chọn nó khi chọn cache-first/network-first/offline-first theo UX consistency; rủi ro chính là TTL tùy tiện hoặc stale overwrite fresh.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao TTL tùy tiện hoặc stale overwrite fresh phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **hit rate, age, revalidation và generation**.
...
Viết một ví dụ nhỏ minh họa **Offline-first synchronization** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **TTL tùy tiện hoặc stale overwrite fresh** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Offline-first synchronization**. Thu evidence bằng hit rate, age, revalidation và generation, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → cache cần key, freshness, validation, invalidation và source-of-truth policy
Use when  → chọn cache-first/network-first/offline-first theo UX consistency
Risk      → TTL tùy tiện hoặc stale overwrite fresh
Evidence  → hit rate, age, revalidation và generation
...
3. Usage rule: chọn cache-first/network-first/offline-first theo UX consistency.
4. Mistake nguy hiểm: TTL tùy tiện hoặc stale overwrite fresh.
5. Production lesson: kết luận bằng hit rate, age, revalidation và generation, rồi bảo vệ bằng test và metric.
