...
Trong Global Commerce, Commerce hiển thị dữ liệu account cũ hoặc mất cart offline vì storage được chọn theo thói quen thay vì policy. Chapter này tập trung vào **Core Data mental model**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- [SQLite concepts, schema và transaction](05-sqlite-concepts-schema-va-transaction.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Managed Object Context và Core Data concurrency](07-managed-object-context-va-core-data-concurrency.md) dùng contract của chapter này làm building block.
...
Trọng tâm của **Core Data mental model** là: storage được chọn theo data shape, lifetime, query, security và migration. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
1. Observable behavior nào định nghĩa Core Data mental model?
...
Disk I/O, context/transaction và migration có lifecycle riêng; synchronous work sai chỗ có thể block UI. Với **Core Data mental model**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Core Data mental model** trong flow có network, UI lifecycle và cache.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Core Data mental model** là code an toàn.
>
...
**Hỏi:** Core Data mental model giải quyết vấn đề gì?
...
Viết một ví dụ nhỏ minh họa **Core Data mental model** và test happy path lẫn edge case.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Core Data mental model**. Thu evidence bằng migration/read-write/failure tests, vẽ owner/state graph, xác định root cause và thêm regression test.
