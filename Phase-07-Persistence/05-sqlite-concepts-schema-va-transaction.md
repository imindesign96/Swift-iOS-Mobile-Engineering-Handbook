---
title: "SQLite concepts, schema và transaction"
phase: "Persistence"
difficulty: 5
importance: 5
...
competencies:
  ...
  - "Interview"
tags:
  - "sqlite-concepts-schema-va-transaction"
  - "global-commerce"
---
...
Trong Global Commerce, Commerce hiển thị dữ liệu account cũ hoặc mất cart offline vì storage được chọn theo thói quen thay vì policy. Chapter này tập trung vào **SQLite concepts, schema và transaction**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng animation mô tả transition của state trong transaction và vẫn chịu layout/render cost;
- nhận diện failure mode chính: animate quá nhiều node hoặc decode/layout trên frame path;
- chọn giải pháp bằng rule: giảm invalidation, precompute và đo trên device;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng animation hitches và Time Profiler;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [FileManager, atomic write và file coordination](04-filemanager-atomic-write-va-file-coordination.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Core Data mental model](06-core-data-mental-model.md) dùng contract của chapter này làm building block.
...
Domain intent → repository → local transaction/source of truth ↔ synchronization policy
                 ↓
Focus: SQLite concepts, schema và transaction
                 ↓
Evidence: animation hitches và Time Profiler
```
...
Trọng tâm của **SQLite concepts, schema và transaction** là: animation mô tả transition của state trong transaction và vẫn chịu layout/render cost. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng animation hitches và Time Profiler.
...
Áp dụng khi giảm invalidation, precompute và đo trên device. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là animate quá nhiều node hoặc decode/layout trên frame path. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa SQLite concepts, schema và transaction?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Animate quá nhiều node hoặc decode/layout trên frame path tạo evidence gì?
...
Disk I/O, context/transaction và migration có lifecycle riêng; synchronous work sai chỗ có thể block UI. Với **SQLite concepts, schema và transaction**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **SQLite concepts, schema và transaction** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy animate quá nhiều node hoặc decode/layout trên frame path.
...
Dùng animation hitches và Time Profiler; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **giảm invalidation, precompute và đo trên device**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **animation hitches và Time Profiler**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **SQLite concepts, schema và transaction** là code an toàn.
>
...
- Animate quá nhiều node hoặc decode/layout trên frame path → behavior chỉ sai ở scale/lifecycle edge.
...
- Giảm invalidation, precompute và đo trên device.
...
**Hỏi:** SQLite concepts, schema và transaction giải quyết vấn đề gì?

**30-second:** Animation mô tả transition của state trong transaction và vẫn chịu layout/render cost. Chọn nó khi giảm invalidation, precompute và đo trên device; rủi ro chính là animate quá nhiều node hoặc decode/layout trên frame path.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao animate quá nhiều node hoặc decode/layout trên frame path phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **animation hitches và Time Profiler**.
...
Viết một ví dụ nhỏ minh họa **SQLite concepts, schema và transaction** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **animate quá nhiều node hoặc decode/layout trên frame path** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **SQLite concepts, schema và transaction**. Thu evidence bằng animation hitches và Time Profiler, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → animation mô tả transition của state trong transaction và vẫn chịu layout/render cost
Use when  → giảm invalidation, precompute và đo trên device
Risk      → animate quá nhiều node hoặc decode/layout trên frame path
Evidence  → animation hitches và Time Profiler
...
3. Usage rule: giảm invalidation, precompute và đo trên device.
4. Mistake nguy hiểm: animate quá nhiều node hoặc decode/layout trên frame path.
5. Production lesson: kết luận bằng animation hitches và Time Profiler, rồi bảo vệ bằng test và metric.
