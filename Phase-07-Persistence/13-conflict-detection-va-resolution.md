---
title: "Conflict detection và resolution"
phase: "Persistence"
difficulty: 5
importance: 5
...
used_later:
  - "Logout/login và data isolation giữa account"
competencies:
  - "Persistence"
  - "Production"
  - "Debugging"
  - "Interview"
tags:
  - "conflict-detection-va-resolution"
  - "global-commerce"
---
...
Trong Global Commerce, Commerce hiển thị dữ liệu account cũ hoặc mất cart offline vì storage được chọn theo thói quen thay vì policy. Chapter này tập trung vào **Conflict detection và resolution**: không dừng ở syntax/API mà nối behavior với ownership, failure mode và evidence production.
...
- giải thích chính xác rằng offline sync là state machine có version/order/conflict policy;
- nhận diện failure mode chính: last-write-wins ngầm hoặc retry mutation không idempotent;
- chọn giải pháp bằng rule: chọn merge policy theo domain và giữ pending operations durable;
- nối runtime, memory, concurrency và architecture implications;
- điều tra case production bằng sync log, revision và reconciliation metrics;
- trả lời câu hỏi interview ở ba độ sâu thay vì đọc thuộc định nghĩa.

## Prerequisites { data-search-exclude }

- [Offline-first synchronization](12-offline-first-synchronization.md); không lặp lại định nghĩa nền nếu chapter trước đã giải thích.
- [Glossary](../GLOSSARY.md) cho terminology canonical.

## Used Later { data-search-exclude }

- [Logout/login và data isolation giữa account](14-logout-login-va-data-isolation-giua-account.md) dùng contract của chapter này làm building block.
...
Domain intent → repository → local transaction/source of truth ↔ synchronization policy
                 ↓
Focus: Conflict detection và resolution
                 ↓
Evidence: sync log, revision và reconciliation metrics
```
...
Trọng tâm của **Conflict detection và resolution** là: offline sync là state machine có version/order/conflict policy. Hãy mô tả bằng state transition, lifetime hoặc output quan sát được thay vì chỉ bằng keyword.
...
5. Đo observable behavior bằng sync log, revision và reconciliation metrics.
...
Áp dụng khi chọn merge policy theo domain và giữ pending operations durable. Bắt đầu bằng giải pháp đơn giản nhất giữ được invariant; thêm abstraction/synchronization/cache chỉ khi requirement hoặc measurement chứng minh cần.
...
Failure mode quan trọng là last-write-wins ngầm hoặc retry mutation không idempotent. Consequence có thể là state stale, duplicate work, leak, crash, UI hitch hoặc test flaky tùy boundary.
...
1. Observable behavior nào định nghĩa Conflict detection và resolution?
2. Owner của state/resource là ai và lifetime kết thúc khi nào?
3. Last-write-wins ngầm hoặc retry mutation không idempotent tạo evidence gì?
...
Disk I/O, context/transaction và migration có lifecycle riêng; synchronous work sai chỗ có thể block UI. Với **Conflict detection và resolution**, hãy log hoặc đo transition ở boundary thay vì suy luận từ UI cuối cùng.
...
Feature Commerce áp dụng **Conflict detection và resolution** trong flow có network, UI lifecycle và cache.

### Symptom

User báo behavior không ổn định; telemetry cho thấy last-write-wins ngầm hoặc retry mutation không idempotent.
...
Dùng sync log, revision và reconciliation metrics; thêm correlation ID/generation an toàn, tái hiện repeated flow và so sánh expected transition với actual transition.
...
Áp dụng rule **chọn merge policy theo domain và giữ pending operations durable**, thu hẹp mutation/ownership và làm failure/cancellation explicit.
...
Primary evidence: **sync log, revision và reconciliation metrics**.
...
> **Myth:** Chỉ cần dùng đúng API/pattern tên **Conflict detection và resolution** là code an toàn.
>
...
- Last-write-wins ngầm hoặc retry mutation không idempotent → behavior chỉ sai ở scale/lifecycle edge.
...
- Chọn merge policy theo domain và giữ pending operations durable.
...
**Hỏi:** Conflict detection và resolution giải quyết vấn đề gì?

**30-second:** Offline sync là state machine có version/order/conflict policy. Chọn nó khi chọn merge policy theo domain và giữ pending operations durable; rủi ro chính là last-write-wins ngầm hoặc retry mutation không idempotent.

### Junior

**Hỏi:** Cho một ví dụ Commerce và common mistake.

**2–3 minute:** Nêu input/state, owner, code contract, failure path và test. Dùng ví dụ ở trên, sau đó giải thích vì sao last-write-wins ngầm hoặc retry mutation không idempotent phá invariant.

### Middle

**Hỏi:** Concept ảnh hưởng memory/concurrency/testability thế nào?

Trả lời bằng object/task graph, isolation/cancellation, boundary DI và evidence **sync log, revision và reconciliation metrics**.
...
Viết một ví dụ nhỏ minh họa **Conflict detection và resolution** và test happy path lẫn edge case.

### Medium

Refactor một call site đang gặp **last-write-wins ngầm hoặc retry mutation không idempotent** để contract/owner rõ hơn.
...
Bug report: repeated Commerce flow tạo symptom liên quan **Conflict detection và resolution**. Thu evidence bằng sync log, revision và reconciliation metrics, vẽ owner/state graph, xác định root cause và thêm regression test.
...
```text
Concept   → offline sync là state machine có version/order/conflict policy
Use when  → chọn merge policy theo domain và giữ pending operations durable
Risk      → last-write-wins ngầm hoặc retry mutation không idempotent
Evidence  → sync log, revision và reconciliation metrics
...
3. Usage rule: chọn merge policy theo domain và giữ pending operations durable.
4. Mistake nguy hiểm: last-write-wins ngầm hoặc retry mutation không idempotent.
5. Production lesson: kết luận bằng sync log, revision và reconciliation metrics, rồi bảo vệ bằng test và metric.

## Related Chapters { data-search-exclude }

- [Offline-first synchronization](12-offline-first-synchronization.md)
- [Logout/login và data isolation giữa account](14-logout-login-va-data-isolation-giua-account.md)
